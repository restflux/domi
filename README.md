...
在「设置 → 外观 → 界面风格」可选择「工作台 v2」：Work 模式左栏采用独立的项目/任务列表与收起 rail，右栏采用可拖动的 ZCode 式标签带，终端保活多个 xterm 视图，浏览器提供地址、响应式尺寸与错误状态控件。ZCode 的终端链接、主题、PowerShell 重绘和部分一方 UI 源码经过适配使用；**这不是将 ZCode 整套应用、原生终端/浏览器服务或全部面板原样复制到 Domi**。v2 右栏在没有工具标签时展示 ZCode 式打开标签页，而非默认显示旧文件/改动面板；已接入的预览、草稿等仍沿用 Domi 工作区面板。原有「现代」「经典」界面保留供回退；切换界面不会复制终端进程或浏览器会话。PTY、浏览器页面和安全策略仍由 Domi 原有主进程管理。
