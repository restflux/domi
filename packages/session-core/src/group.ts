 ...
 * 合并为可渲染/可导出的 Turn 分组。
 *
 * 这是 Domi 会话「快照去重」的唯一真源：Electron 渲染层、Domi CLI 以及
 ...
 * 3. 需要持久展示的 system 状态/操作卡 → 独立渲染，其他归入当前 turn
...
function assistantTurnCallsTool(turn: AssistantTurn, toolName: string): boolean {
  return turn.assistantMessages.some((message) => {
...
/**
 * ReadyForReview 在工具执行时先持久化验收卡，当前 assistant/tool_result/result 稍后才落盘，
 * 因而原始 JSONL 顺序会是 user → ready card → assistant process。产品语义上验收卡是该段内
 * 最后一次 ReadyForReview 的完成态，必须显示在其执行过程之后。
 * 移动只允许发生在卡片自己的段落内：一旦遇到真正的用户输入即停止扫描。后续迭代的
 * ReadyForReview 会追加自己的新卡并同样跟随各自的过程；若放行跨过用户输入，上一轮的
 * 旧卡会被搬到新一轮过程之后，导致正在输出的正文/过程跑到上一轮验收卡上面。
 */
function moveReadyForReviewAfterLatestTerminatingTurn(groups: MessageGroup[]): MessageGroup[] {
  if (groups.length <= 1) return groups

  const reviewCardIndexes = groups.flatMap((group, index) => (
    group.type === 'system' && group.message.subtype === 'worktree_ready_for_review' ? [index] : []
  ))
  if (reviewCardIndexes.length === 0) return groups

  const cardsAfterTurn = new Map<number, MessageGroup[]>()
  const movedCardIndexes = new Set<number>()
  reviewCardIndexes.forEach((cardIndex, cardPosition) => {
    const nextCardIndex = reviewCardIndexes[cardPosition + 1] ?? groups.length
    let targetTurnIndex = -1
    for (let index = cardIndex + 1; index < nextCardIndex; index++) {
      const group = groups[index]
      // 真正的用户输入开启新的迭代段落：验收卡不跨越它追赶后续 turn。
      if (group?.type === 'user') break
      if (group?.type === 'assistant-turn' && assistantTurnCallsTool(group, 'ReadyForReview')) {
        targetTurnIndex = index
      }
    }
    if (targetTurnIndex < 0) return
    movedCardIndexes.add(cardIndex)
    const cards = cardsAfterTurn.get(targetTurnIndex) ?? []
    cards.push(groups[cardIndex]!)
    cardsAfterTurn.set(targetTurnIndex, cards)
  })

  if (movedCardIndexes.size === 0) return groups
  const result: MessageGroup[] = []
  groups.forEach((group, index) => {
    if (!movedCardIndexes.has(index)) result.push(group)
    const cards = cardsAfterTurn.get(index)
...
export function getGroupPreview(group: MessageGroup): string {
  ...
  if (group.type === 'system') {
    ...
    if (group.message.subtype === 'worktree_ready_for_review') {
      const summary = typeof group.message.summary === 'string' ? group.message.summary.trim() : ''
      return (summary ? `已完成：${summary}` : '修改已完成，等待验收').slice(0, 200)
    }
    return ''
  }
  // assistant-turn：优先收集正文；纯工具轮回退为执行摘要，避免消息导航出现空白节点。
  const texts: string[] = []
  let toolCount = 0
  ...
  const textPreview = texts.join(' ').slice(0, 200)
  if (textPreview) return textPreview
  return toolCount > 0 ? `执行过程：${toolCount} 次工具调用` : ''
}
