...
/** Session Target 文件检查结果（仅暴露授权后的项目相对路径）。 */
export interface SessionTargetFileInspection {
  /** 相对于 Session Target 项目根的规范化路径。 */
  relativePath: string
  /** 检查时文件是否已经存在。 */
  exists: boolean
  /** Session Target 是否属于 Git 仓库。 */
  isGitRepo: boolean
}
...
/** Active Pi 文件请求。路径始终相对于会话绑定的 Session Target。 */
export interface SessionTargetFileRequest {
  sessionId: string
  relativePath: string
}

/** 获取文件 Diff 的输入（兼容旧手动 Worktree）。 */
...
export interface FileAccessOptions {
  ...
  /** Active Pi 文件路径所属空间；缺省时按 Session Target 相对路径处理。 */
  pathSpace?: 'session-target' | 'session-workbench' | 'session-local-project'
  /** 路径解析候选目录；主进程会先过滤到已授权目录内再使用 */
  candidateBasePaths?: string[]
}

/** 已授权本地文件的 domi-file URL */
...
export const IPC_CHANNELS = {
  ...
  /** 按当前 Session Target 获取只读 Git 工作区快照。 */
  GET_GIT_WORKSPACE_SNAPSHOT: 'git:get-workspace-snapshot',
  /** 按仓库与层次获取只读 Git Diff 内容。 */
  GET_GIT_WORKSPACE_DIFF: 'git:get-workspace-diff',
  /** Git 面板：提交历史（含 tag 徽章）。 */
  GET_GIT_WORKSPACE_HISTORY: 'git:get-workspace-history',
  /** Git 面板：本地分支列表。 */
  GET_GIT_WORKSPACE_BRANCHES: 'git:get-workspace-branches',
  /** Git 面板：提交的文件清单。 */
  GET_GIT_WORKSPACE_COMMIT_FILES: 'git:get-workspace-commit-files',
  /** Git 面板：单提交内文件 Diff。 */
  GET_GIT_WORKSPACE_COMMIT_DIFF: 'git:get-workspace-commit-diff',
  /** Git 面板：暂存/取消暂存文件。 */
  STAGE_GIT_WORKSPACE_FILES: 'git:stage-workspace-files',
  /** Git 面板：丢弃改动（含未追踪删除）。 */
  DISCARD_GIT_WORKSPACE_FILES: 'git:discard-workspace-files',
  /** Git 面板：提交（可选推送）。 */
  COMMIT_GIT_WORKSPACE: 'git:commit-workspace',
  /** Git 面板：切换本地分支。 */
  CHECKOUT_GIT_WORKSPACE_BRANCH: 'git:checkout-workspace-branch',
  /** Git 面板：fetch / pull（--ff-only）/ push / sync。 */
  GIT_WORKSPACE_PULL_PUSH: 'git:pull-push-workspace',
  /** 检查并投影 Session Target 中的文件路径。 */
  INSPECT_SESSION_TARGET_FILE: 'session-target:inspect-file',
  /** 获取单个文件的 diff */
  ...
  /** 将授权文件或文件夹作为系统文件对象写入剪贴板。 */
  COPY_FILE_SYSTEM_ITEM: 'clipboard:copy-file-system-item',
  /** 将授权文件或文件夹的真实路径作为文本写入剪贴板。 */
  COPY_FILE_SYSTEM_PATH: 'clipboard:copy-file-system-path',
  /** 用系统默认应用打开任意文件 */
