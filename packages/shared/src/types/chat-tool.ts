...
export interface ChatToolHttpConfig {
  ...
  /** HTTP 方法 */
  method: 'GET' | 'POST'
  /** 请求头；凭据只能使用 {{credential.<key>}} 引用，真实值存于 toolCredentials */
  headers?: Record<string, string>
  /** 请求体模板（支持 {{param}} 与 {{credential.<key>}}；凭据禁止放入 URL） */
  bodyTemplate?: string
...
export interface ChatToolInfo {
  ...
  /** 工具是否可用（凭据已配置） */
  available: boolean
  /** 自定义 HTTP 工具声明的 credential reference keys；不含真实值 */
  credentialKeys?: string[]
  /** 已配置非空值的 credential keys；只暴露 presence，不暴露真实值 */
