 ...
 * │ [language]                     [📋 复制] │  ← 头部栏
 * ├─────────────────────────────────────────┤
 * │  1  const foo = 'bar'                   │  ← 高亮代码区（行号 + 逐行渲染）
 * │  2  console.log(foo)                    │
 * │  …展开全部 120 行                        │  ← 超过 20 行默认折叠
 * └─────────────────────────────────────────┘
 */

import * as React from 'react'
import { getDisplayName, highlightToTokens, onHighlighterReady } from '@domi/core'
import type { HighlightToken, HighlightTokensResult } from '@domi/core'
...
interface CodeLineProps {
  ...
  /** 行号（从 1 开始）；不传则不渲染行号槽 */
  lineNumber?: number
  /** 行号槽宽度（ch 单位，按最大行号位数对齐所有行） */
  numberWidth?: string
}

/** 单行代码渲染（memo 避免已稳定行重复渲染） */
const CodeLine = React.memo(function CodeLine({ tokens, rawLine, lineNumber, numberWidth }: CodeLineProps): React.ReactElement {
  // token 覆盖的字符数
  const tokenLen = tokens.reduce((sum, t) => sum + t.content.length, 0)

  return (
    <span className="line">
      {/* 行号槽：右对齐 + 不可选中，宽度按最大行号位数统一 */}
...
export function CodeBlock({ children, onCopy }: CodeBlockProps): React.ReactElement {
  ...
  const collapsed = rawLines.length > COLLAPSE_THRESHOLD && !expanded
  const visibleLines = collapsed ? rawLines.slice(0, COLLAPSE_THRESHOLD) : rawLines
  /** 行号槽宽度按最大行号位数对齐 */
  const numberWidth = `${String(rawLines.length).length}ch`
        ...
        {collapsed && (
          <div
            ...
            style={{
              ...
              bottom: 0,
              height: 40,
              background: 'linear-gradient(to bottom, transparent, hsl(var(--code-bg)))',
            }}
          />
        )}
      </div>
