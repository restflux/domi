...
describe('OpenAIResponsesAdapter', () => {
  ...
  test('Given 多个工具调用参数交错 When streamSSE Then 按 output_index 分别累积参数', async () => {
    const result = await streamSSE({
      ...
      fetchFn: createSSEFetch([
        ...
        { type: 'response.function_call_arguments.delta', output_index: 0, delta: ':"Domi"}' },
    ...
    expect(result.toolCalls).toEqual([
      { id: 'call_1|fc_1', name: 'search', arguments: { query: 'Domi' }, metadata: { itemId: 'fc_1', outputIndex: 0 } },
  ...
  test('Given done 事件携带完整参数 When streamSSE Then 使用 finalArguments 替换增量兜底', async () => {
    const result = await streamSSE({
      ...
      fetchFn: createSSEFetch([
        ...
        { type: 'response.function_call_arguments.done', output_index: 0, arguments: '{"query":"Domi"}' },
        { type: 'response.completed', response: { status: 'completed' } },
      ]),
      onEvent: () => {},
    })
  ...
  test('Given 仅 output_item.done 携带工具调用 When streamSSE Then 仍可恢复工具参数', async () => {
    const result = await streamSSE({
      ...
      adapter,
      fetchFn: createSSEFetch([
        { type: 'response.output_item.done', output_index: 0, item: { type: 'function_call', id: 'fc_1', call_id: 'call_1', name: 'search', arguments: '{"query":"Domi"}' } },
        { type: 'response.completed', response: { status: 'completed' } },
      ]),
      onEvent: () => {},
    })
