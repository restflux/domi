...
import type { ProviderAdapter, ProviderRequest, StreamEventCallback, ThinkingBlock, ToolCall, StreamUsageEvent } from './types.ts'
...
async function runStreamAttempt(options: StreamSSEOptions): Promise<StreamSSEResult> {
  ...
  // 用量追踪：供应商通常在流末尾返回 usage（以最后一条为准）
  let streamUsage: StreamUsageEvent | undefined
      ...
      for (const line of lines) {
        ...
        for (const event of events) {
          if (event.type === 'chunk') {
            ...
            // 记录最后一条 usage（供应商口径：输入含缓存命中，缓存量单独标记）
            streamUsage = event
          }
          onEvent(event)
        }
  ...
  return { content, reasoning, thinkingBlocks, toolCalls, stopReason, usage: streamUsage }
}
