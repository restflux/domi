...
const adapterRegistry = new Map<ProviderType, ProviderAdapter>([
  ...
  ['qwen-token-plan', new AnthropicAdapter('qwen-token-plan')],     // 通义千问 Token Plan 中国区 Anthropic 兼容协议
  ['qwen-token-plan-individual', new OpenAIAdapter('qwen-token-plan-individual')], // 国际 Individual OpenAI 兼容协议
...
export function getAdapter(provider: ProviderType): ProviderAdapter {
  ...
  if (!adapter) {
    throw new Error(`不支持的供应商: ${provider}。你可能过去使用的是 Domi 商业版，请重新下载商业版覆盖安装，当前版本为开源版本。`)
  }
  return adapter
}
