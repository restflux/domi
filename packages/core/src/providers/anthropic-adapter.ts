...
import { extractZhipuCodingTeamApiToken, type ProviderType } from '@domi/shared'
import type {
  ProviderAdapter,
  ProviderRequest,
  StreamRequestInput,
...
import { getKimiCodingPlanUserAgent } from './user-agent.ts'
...
export class AnthropicAdapter implements ProviderAdapter {
   ...
   * - User-Agent 使用 Proma 自有标识（通过 setDomiCompatibilityVersion 初始化）
   */
  private buildHeaders(apiKey: string): Record<string, string> {
    ...
    if (this.providerType === 'kimi-coding' || this.providerType === 'zhipu-coding' || this.providerType === 'zhipu-coding-team') {
      ...
      base['User-Agent'] = getKimiCodingPlanUserAgent()
      return base
    }
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    ...
    // 调试：开启 DOMI_DEBUG_REQUEST 时打印请求体，便于排查思考+工具场景的消息结构
    const procReq = (globalThis as { process?: { env?: Record<string, string | undefined> } }).process
    if (procReq?.env?.DOMI_DEBUG_REQUEST) {
  ...
  parseSSELine(jsonLine: string): StreamEvent[] {
    try {
      ...
      // 调试：开启 DOMI_DEBUG_SSE 时打印原始事件，便于排查 Provider 的 SSE 格式差异
      const proc = (globalThis as { process?: { env?: Record<string, string | undefined> } }).process
      if (proc?.env?.DOMI_DEBUG_SSE) {
      ...
      // Anthropic 将非缓存输入、缓存读取和缓存写入分列；统一转换为“总输入含缓存”的内部口径。
      if (event.type === 'message_delta' && event.usage) {
        ...
        events.push({
          type: 'usage',
          inputTokens: (event.usage.input_tokens ?? 0) + cacheReadTokens + cacheCreationTokens,
