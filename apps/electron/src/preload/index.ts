...
import {
  ...
  type BrowserElementSelectionInput,
  type BrowserElementSelectionResult,
  type BrowserFitToWidthInput,
  type BrowserFocusEscapeRequest,
...
import type {
  ...
  SessionCapabilityGrantsChangedPayload,
  DomiPermissionMode,
...
import { EDITABLE_CONTEXT_MENU_CHANNELS } from '../types/editable-context-menu'
import type {
  UserProfile,
  AppSettings,
 ...
 * 暴露给渲染进程的 API 接口定义
 */
export interface TerminalRendererApi {
  create: (input: TerminalCreateInput) => Promise<TerminalSessionView>
  list: (input: Pick<TerminalOwnerInput, 'ownerSessionId'>) => Promise<TerminalSessionView[]>
  inspect: (input: TerminalOwnerInput) => Promise<TerminalSessionView>
  snapshot: (input: TerminalOwnerInput) => Promise<TerminalSnapshot>
  input: (input: TerminalInput) => Promise<void>
  resize: (input: TerminalResizeInput) => Promise<void>
  interrupt: (input: TerminalOwnerInput) => Promise<boolean>
  close: (input: TerminalOwnerInput) => Promise<boolean>
  onOutput: (callback: (event: TerminalOutputEvent) => void) => () => void
  onStateChanged: (callback: (event: TerminalStateChange) => void) => () => void
}

export interface BrowserRendererApi {
  open: (input: BrowserOpenInput) => Promise<BrowserSessionView>
  activate: (input: BrowserActivateInput) => Promise<BrowserSessionView>
  inspect: (input: BrowserInspectInput) => Promise<BrowserSessionView>
  navigate: (input: BrowserNavigateInput) => Promise<BrowserSessionView>
  goBack: (input: BrowserPageInput) => Promise<void>
  goForward: (input: BrowserPageInput) => Promise<void>
  reload: (input: BrowserPageInput) => Promise<void>
  stop: (input: BrowserPageInput) => Promise<void>
  setZoom: (input: BrowserZoomInput) => Promise<BrowserSessionView>
  setFitToWidth: (input: BrowserFitToWidthInput) => Promise<BrowserSessionView>
  setLayout: (input: BrowserLayoutInput) => Promise<boolean>
  selectElement: (input: BrowserElementSelectionInput) => Promise<BrowserElementSelectionResult>
  cancelElementSelection: (input: BrowserElementSelectionCancelInput) => Promise<boolean>
  close: (input: BrowserCloseInput) => Promise<boolean>
  onStateChanged: (callback: (state: BrowserStateChange) => void) => () => void
...
export interface ElectronAPI {
  ...
  terminal: TerminalRendererApi
  /** Pi Extension 候选预览、显式批准与撤销。 */
  piExtensionTrust: PiExtensionTrustApi
  /** 按会话检查、绑定和操作 Session Target。 */
  sessionCheckout: SessionCheckoutRendererApi
  /** 只读查询 main-owned Pi run timing 证据。 */
  piRunTiming: PiRunTimingRendererApi
  ...
  /** 获取当前 Session Target 的只读 Git 工作区快照。 */
  getGitWorkspaceSnapshot: (input: GitWorkspaceInspectInput) => Promise<GitWorkspaceSnapshot>
  /** 获取一个已列出 Git 变更层的 Diff 内容。 */
  getGitWorkspaceDiff: (input: GitWorkspaceDiffRequest) => Promise<GitWorkspaceDiffContents | null>
  /** Git 面板：提交历史（含 tag 徽章）。 */
  getGitWorkspaceHistory: (input: GitWorkspaceHistoryRequest) => Promise<GitWorkspaceHistoryResult>
  /** Git 面板：本地分支列表。 */
  getGitWorkspaceBranches: (input: GitWorkspaceBranchesRequest) => Promise<GitWorkspaceBranchesResult>
  /** Git 面板：提交的文件清单。 */
  getGitWorkspaceCommitFiles: (input: GitWorkspaceCommitFilesRequest) => Promise<GitWorkspaceCommitFilesResult>
  /** Git 面板：单提交内文件 Diff。 */
  getGitWorkspaceCommitDiff: (input: GitWorkspaceCommitDiffRequest) => Promise<GitWorkspaceDiffContents | null>
  /** Git 面板：暂存/取消暂存文件。 */
  stageGitWorkspaceFiles: (input: GitWorkspaceStageRequest) => Promise<GitWorkspaceOperationResult>
  /** Git 面板：丢弃改动（含未追踪删除）。 */
  discardGitWorkspaceFiles: (input: GitWorkspaceDiscardRequest) => Promise<GitWorkspaceOperationResult>
  /** Git 面板：提交（可选推送）。 */
  commitGitWorkspace: (input: GitWorkspaceCommitRequest) => Promise<GitWorkspaceOperationResult>
  /** Git 面板：切换本地分支。 */
  checkoutGitWorkspaceBranch: (input: GitWorkspaceCheckoutRequest) => Promise<GitWorkspaceOperationResult>
  /** Git 面板：fetch / pull（--ff-only）/ push / sync。 */
  gitWorkspacePullPush: (input: GitWorkspacePullPushRequest) => Promise<GitWorkspaceOperationResult>
  /** 获取未暂存的变更文件列表（旧手动浏览）。 */
  getUnstagedChanges: (dirPath: string, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => Promise<import('@domi/shared').UnstagedChangesResult>
  /** Active Diff 仅提交 sessionId，root 与 base 由主进程 Session Target 决定。 */
  getSessionTargetChanges: (sessionId: string) => Promise<import('@domi/shared').UnstagedChangesResult>
  /** 检查 Agent 写入路径并投影为经过授权的 Session Target 相对路径。 */
  inspectSessionTargetFile: (sessionId: string, filePath: string, allowMissingLeaf?: boolean) => Promise<import('@domi/shared').SessionTargetFileInspection | null>
  /** 获取单个文件的 diff（兼容旧手动 Worktree）。 */
  getFileDiff: (input: import('@domi/shared').GetFileDiffInput) => Promise<string>
  /** Active Pi 仅提交 sessionId 与 Session Target 相对路径。 */
  getSessionTargetFileDiff: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<string>
  getUntrackedContent: (input: import('@domi/shared').GetFileDiffInput) => Promise<string>
  getSessionTargetUntrackedContent: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<string>
  revertFile: (input: import('@domi/shared').RevertFileInput) => Promise<void>
  revertSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<void>
  getDiffContents: (input: import('@domi/shared').GetFileDiffInput) => Promise<{ oldContent: string; newContent: string } | null>
  getSessionTargetDiffContents: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<{ oldContent: string; newContent: string } | null>
  /** 列出 Git Worktree */
  listWorktrees: (repoPath: string, sessionId: string) => Promise<import('@domi/shared').WorktreeInfo[]>
  /** 获取 Worktree 相对于基准分支的全量变更 */
  getWorktreeChanges: (worktreePath: string, baseBranch: string, sessionId: string) => Promise<import('@domi/shared').UnstagedChangesResult>
  /** 在独立窗口打开当前文件预览 */
  ...
  /** 订阅可编辑区域右键菜单请求 */
  onEditableContextMenu: (callback: (request: EditableContextMenuRequest) => void) => () => void
  /** 执行标准编辑命令 */
  executeEditableContextMenuAction: (action: EditableContextMenuAction) => void
  ...
  /** 列出会话的生成图片（Chat 按会话 ID，Agent 按会话 ID） */
  listGeneratedImages: (request: GeneratedImagesRequest) => Promise<GeneratedImageItem[]>

  /** 读取当前会话画廊内的一张图片（base64） */
  readGeneratedImage: (request: GeneratedImagesRequest, localPath: string) => Promise<string>

  /** 将当前会话画廊内的一张图片另存为 */
  saveGeneratedImageAs: (request: GeneratedImagesRequest, localPath: string, defaultFilename: string) => Promise<boolean>
  ...
  getPiReasoningCapability: (channelId: string, modelId: string) => Promise<import('@domi/shared').ReasoningCapability | undefined>

  /** 查询当前渠道模型是否已被 Pi catalog 收录。 */
  getPiModelCatalogStatus: (channelId: string, modelId: string) => Promise<import('@domi/shared').PiModelCatalogStatus>

  /** 列出可作为 Vision Relay 目标的、catalog 明确支持图片的模型。 */
  listVisionRelayModels: () => Promise<ModelOption[]>

  /** 更新当前会话的推理深度 */
  updateSessionReasoningLevel: (sessionId: string, thinkingLevel: AgentThinkingLevel) => Promise<AgentSessionMeta>

  /** 更新当前会话的模型呈现预设（极简/标准，下一轮生效） */
  updateSessionModelPresentationPreset: (sessionId: string, preset: ModelPresentationPreset) => Promise<AgentSessionMeta>
  ...
  /** 切换 Agent 会话待继续状态 */
  toggleFollowUpAgentSession: (id: string) => Promise<AgentSessionMeta>
  ...
  /** 只读预览回退将影响的受控文件。 */
  previewRewindSession: (input: RewindSessionInput) => Promise<RewindSessionPreview>

  /** 快照回退（同一会话内回退到指定点，恢复文件 + 截断对话） */
  rewindSession: (input: RewindSessionInput) => Promise<RewindSessionResult>

  /** 查询最近一次回退是否仍可撤销。 */
  getRewindUndoState: (input: RewindUndoInput) => Promise<RewindUndoState>

  /** 撤销最近一次回退。 */
  undoRewindSession: (input: RewindUndoInput) => Promise<UndoRewindSessionResult>

  /** 读取 Pi 会话树 */
  getSessionTree: (sessionId: string) => Promise<SessionTreeResult>

  /** 在同一 Pi session 文件内导航 */
  navigateSessionTree: (input: NavigateSessionTreeInput) => Promise<NavigateSessionTreeResult>
  ...
  /** 中止 Agent 执行 */
  stopAgent: (sessionId: string, source?: 'renderer-stop-control' | 'renderer-ask-user-dismiss' | 'renderer-plan-dismiss' | 'renderer-permission-dismiss') => Promise<void>

  /** 跳过自动重试等待，立即执行当前已经安排的恢复 */
  retryAgentNow: (sessionId: string) => Promise<boolean>

  // ===== Agent 队列消息 =====

  /** 流式追加发送 Agent 消息（Agent 运行中） */
  queueAgentMessage: (input: AgentQueueMessageInput) => Promise<string>
  /** main 原子决定注入当前 run 或接管为 deferred queue。 */
  submitOrEnqueueAgentMessage: (input: AgentSubmitOrEnqueueInput) => Promise<AgentSubmitOrEnqueueResult>
  cancelDeferredAgentMessage: (input: AgentQueuedMessageControlInput) => Promise<boolean>
  moveDeferredAgentMessage: (input: AgentMoveQueuedMessageInput) => Promise<boolean>
  /** clearQueue + replay 整体重建 Pi 原生队列 */
  replaceAgentMessageQueue: (input: AgentReplaceMessageQueueInput) => Promise<AgentReplaceMessageQueueResult>
  /** 清空 Pi 原生队列，可与 abort 组合用于 Escape 恢复 */
  clearAgentMessageQueue: (input: AgentClearMessageQueueInput) => Promise<AgentClearMessageQueueResult>
  ...
  /** 获取检测到的外部用户级 Skills / Pi MCP */
  getGlobalAgentCapabilities: () => Promise<GlobalAgentCapabilities>

  /** 启用或关闭外部用户级 Skills 接入 */
  setGlobalSkillsEnabled: (enabled: boolean) => Promise<GlobalAgentCapabilities>

  /** 启用或关闭 Pi 用户级 MCP 接入 */
  setPiGlobalMcpEnabled: (enabled: boolean) => Promise<GlobalAgentCapabilities>
  ...
  /** 测试 MCP 服务器连接 */
  testMcpServer: (name: string, entry: import('@domi/shared').McpServerEntry) => Promise<{ success: boolean; message: string }>
  ...
  /** 获取会话级 Skill 触发明细 */
  getSessionSkillTriggers: (sessionId: string) => Promise<SkillTriggerEvent[]>

  /** 获取工作区级 Skill 使用统计 */
  getWorkspaceSkillUsage: (workspaceSlug: string) => Promise<SkillUsageStats[]>
  ...
  listSkillFiles: (workspaceSlug: string, skillSlug: string) => Promise<import('@domi/shared').SkillFileNode[]>

  /** 读取 Skill 目录下的子文件内容 */
  readSkillFile: (workspaceSlug: string, skillSlug: string, relativePath: string) => Promise<import('@domi/shared').SkillFileContent>
  ...
  /** 读取工作区 AGENTS.md */
  readWorkspaceAgentsMd: (workspaceSlug: string) => Promise<import('@domi/shared').SkillFileContent>

  /** 写入工作区 AGENTS.md */
  writeWorkspaceAgentsMd: (workspaceSlug: string, content: string) => Promise<void>

  /** @deprecated 兼容旧 renderer；代理到 AGENTS.md。 */
  readWorkspaceClaudeMd: (workspaceSlug: string) => Promise<import('@domi/shared').SkillFileContent>

  /** @deprecated 兼容旧 renderer；代理到 AGENTS.md。 */
  writeWorkspaceClaudeMd: (workspaceSlug: string, content: string) => Promise<void>

  /** 列出工作区 auto memory 文件树 */
  listWorkspaceAutoMemoryFiles: (workspaceSlug: string) => Promise<import('@domi/shared').SkillFileNode[]>

  /** 读取工作区 auto memory 文件 */
  readWorkspaceAutoMemoryFile: (workspaceSlug: string, relativePath: string) => Promise<import('@domi/shared').SkillFileContent>

  /** 写入工作区 auto memory 文件 */
  writeWorkspaceAutoMemoryFile: (workspaceSlug: string, relativePath: string, content: string) => Promise<void>

  /** 上报当前真正展示对话转录的 Agent 会话，仅用于流式频率调度 */
  setVisibleAgentSession: (sessionId: string | null, notificationPresenceSessionId?: string | null) => Promise<void>
  ...
  /** 响应权限请求 */
  respondPermission: (response: PermissionResponse) => Promise<PermissionResponseResult>

  /** Legacy Runtime 权限模式兼容入口。 */
  updateSessionPermissionMode: (sessionId: string, mode: DomiPermissionMode) => Promise<void>

  /** Pi 会话独立更新 Execution Policy / Workflow。 */
  updateSessionExecutionControls: (
    sessionId: string,
    controls: AgentExecutionControlsUpdate,
  ) => Promise<AgentSessionMeta>

  /** 查询当前进程内、当前会话的短期能力授权。 */
  getSessionCapabilityGrants: (sessionId: string) => Promise<GitPushSessionTrustView[]>

  /** 按 grantId 撤销会话能力；Renderer 不能创建或修改授权内容。 */
  revokeSessionCapabilityGrant: (sessionId: string, grantId: string) => Promise<GitPushSessionTrustView[]>

  /** 订阅会话能力授权变化。 */
  onSessionCapabilityGrantsChanged: (callback: (payload: SessionCapabilityGrantsChangedPayload) => void) => () => void
  ...
  /** 获取宿主权威的全局 Work Activity 投影。 */
  getWorkActivity: () => Promise<WorkActivityProjection>
  markWorkActivityViewed: (rootSessionId: string) => Promise<void>
  acknowledgeWorkActivityOutcome: (rootSessionId: string) => Promise<void>
  removeWorkActivityCompleted: (rootSessionId: string) => Promise<void>
  removeAllWorkActivityCompleted: () => Promise<number>
  stopWorkActivitySession: (rootSessionId: string) => Promise<WorkActivityStopResult>
  onWorkActivityChanged: (callback: () => void) => () => void
  onWorkActivityNotification: (callback: (event: WorkActivityNotificationEvent) => void) => () => void
  onWorkActivityNotificationNavigate: (callback: (target: WorkActivityNotificationTarget) => void) => () => void
  ...
  openFileOrFolderDialog: (sessionId: string, kind: ComposerAttachmentKind) => Promise<FileOrFolderDialogResult>
  ...
  getWorktreeRepos: (workspaceSlug: string) => Promise<import('@domi/shared').WorkspaceWorktreeRepo[]>
  /** 添加 worktree 仓库到工作区配置 */
  addWorktreeRepo: (workspaceSlug: string, repo: import('@domi/shared').WorkspaceWorktreeRepo) => Promise<import('@domi/shared').WorkspaceWorktreeRepo[]>
  /** 从工作区配置移除 worktree 仓库 */
  removeWorktreeRepo: (workspaceSlug: string, repoPath: string) => Promise<import('@domi/shared').WorkspaceWorktreeRepo[]>
  ...
  /** 兼容通用文件树的绝对路径 API。 */
  listDirectory: (dirPath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<FileEntry[]>
  /** Active Pi 只提交 Session Target 相对路径，结果也只含相对路径。 */
  listSessionTargetDirectory: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<FileEntry[]>
  /** 列出当前会话在项目中产生或修改、且当前仍存在的文件。 */
  listSessionProjectArtifacts: (sessionId: string) => Promise<import('@domi/shared').SessionProjectArtifact[]>

  deleteFile: (filePath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>
  deleteSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<void>

  openFile: (filePath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>
  openSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<void>

  /** 将剪贴板文本写入临时预览文件并返回绝对路径 */
  writeClipboardPreview: (filename: string, content: string) => Promise<string>

  /** 用系统默认应用打开任意文件（无工作区限制） */
  systemOpenFile: (filePath: string, appName?: string, access?: import('@domi/shared').FileAccessOptions) => Promise<void>

  /** 扫描系统中可用的编辑器应用（仅 macOS） */
  scanEditors: () => Promise<import('@domi/shared').EditorApp[]>

  /** 查询本机为该文件类型注册的默认打开应用（含图标 dataURL） */
  getDefaultAppForFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<import('@domi/shared').DefaultAppInfo | null>

  /** 在系统文件管理器中显示文件 */
  showInFolder: (filePath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>
  showSessionTargetInFolder: (input: import('@domi/shared').SessionTargetFileRequest) => Promise<void>

  /** 使用系统终端打开文件夹；Agent UI 应携带 Session 上下文，设置页保留托管目录入口。 */
  openFolderInTerminal: (folderPath: string, access?: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>

  /** 在系统文件管理器中显示文件（无工作区限制，支持候选基础目录） */
  showItemInFolder: (filePath: string, candidateBasePaths?: string[]) => Promise<boolean>

  /** 复制授权文件或文件夹本身，可直接粘贴到资源管理器等目标。 */
  copyFileSystemItem: (filePaths: string[], access?: import('@domi/shared').FileAccessOptions) => Promise<void>

  /** 复制授权文件或文件夹的真实路径文本。 */
  copyFileSystemPath: (filePaths: string[], access?: import('@domi/shared').FileAccessOptions) => Promise<void>

  /** 解析文件路径并读取内容（供内联预览使用） */
  resolveAndReadFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<{ resolvedPath: string; content: string } | null>

  /** 写入文本文件（供 Markdown 内联编辑使用） */
  writeTextFile: (filePath: string, content: string, access?: import('@domi/shared').FileAccessOptions) => Promise<boolean>

  /** 解析本地文件或目录；文件返回预览 URL，目录返回目录类型。 */
  resolveFilePath: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<ResolvedPreviewPath | null>

  /** 解析 HTML 预览路径，并授权加载同目录相对资源。 */
  resolveHtmlPreviewPath: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<ResolvedPreviewPath | null>

  /** 为内联 PDF 预览生成临时 HTML 文件，返回文件路径 */
  preparePdfPreview: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<{ tmpHtmlUrl: string } | null>

  /** 读取文件为 base64（带路径校验，供内联图片预览等） */
  readBinaryBase64: (filePath: string, access?: import('@domi/shared').FileAccessOptions, maxSize?: number) => Promise<string | null>

  /** DOCX 转 HTML（内联预览） */
  docxToHtml: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<{ resolvedPath: string; html: string } | null>

  /** XLSX/PPTX 转 HTML（内联预览） */
  officeToHtml: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => Promise<import('@domi/shared').OfficePreviewResult | null>
  ...
  /** 重命名文件/目录 */
  renameFile: (filePath: string, newName: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>
  renameSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest, newName: string) => Promise<void>

  /** 移动文件/目录到目标目录 */
  moveFile: (filePath: string, targetDir: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>

  /** 列出附加目录内容 */
  listAttachedDirectory: (dirPath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<FileEntry[]>

  /** 读取附加目录文件内容为 base64（限制在已附加目录范围内） */
  readAttachedFile: (filePath: string, sessionId: string, workspaceSlug?: string) => Promise<string>

  /** 在文件管理器中显示附加目录文件 */
  showAttachedInFolder: (filePath: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>

  /** 重命名附加目录文件/目录 */
  renameAttachedFile: (filePath: string, newName: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>

  /** 移动附加目录文件/目录 */
  moveAttachedFile: (filePath: string, targetDir: string, access: import('@domi/shared').FileAccessOptions & { sessionId: string }) => Promise<void>
  ...
  /** 搜索绝对路径文件区；Active Pi 仅允许当前会话 workbench。 */
  searchWorkspaceFiles: (rootPath: string, query: string, limit: number | undefined, additionalPaths: string[] | undefined, sessionPaths: string[] | undefined, sessionId: string, pathSpace?: import('@domi/shared').FileAccessOptions['pathSpace']) => Promise<FileSearchResult>
  /** Active Pi 搜索不接受 renderer root。 */
  searchSessionTargetFiles: (sessionId: string, query: string, limit?: number) => Promise<FileSearchResult>
  ...
  /** 启用或停用单条 Work 附加提示词 */
  updateWorkPromptActivation: (id: string, enabled: boolean) => Promise<void>

  /** 设置 Chat 默认提示词 */
  setDefaultPrompt: (id: string | null, scope?: SystemPromptScope) => Promise<void>
  ...
  /** 获取多 Bot 配置 */
  getFeishuMultiConfig: () => Promise<import('@domi/shared').FeishuMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveFeishuBotConfig: (input: import('@domi/shared').FeishuBotConfigInput) => Promise<import('@domi/shared').FeishuBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getFeishuMultiStatus: () => Promise<import('@domi/shared').FeishuMultiBridgeState>

  // --- 扫码注册 ---

  /** 启动扫码注册流程，等待用户扫码 + 飞书确认后返回 App ID/Secret */
  registerFeishuApp: () => Promise<import('@domi/shared').FeishuRegisterAppResult>
  /** 取消正在进行的扫码注册流程 */
  cancelFeishuRegistration: () => Promise<void>
  /** 监听二维码 URL 生成 */
  onFeishuRegisterQrcode: (callback: (payload: import('@domi/shared').FeishuRegisterAppQRCode) => void) => () => void
  /** 监听注册流程状态变化 */
  onFeishuRegisterStatus: (callback: (payload: import('@domi/shared').FeishuRegisterAppStatus) => void) => () => void
  ...
  /** 获取多 Bot 配置 */
  getDingTalkMultiConfig: () => Promise<import('@domi/shared').DingTalkMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveDingTalkBotConfig: (input: import('@domi/shared').DingTalkBotConfigInput) => Promise<import('@domi/shared').DingTalkBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getDingTalkMultiStatus: () => Promise<import('@domi/shared').DingTalkMultiBridgeState>
  ...
  /** 执行导出 */
  migrationExport: (options: MigrationExportOptions) => Promise<MigrationExportResult>
  /** 执行 v2 多工作区导出 */
  migrationExportV2: (options: MigrationExportOptionsV2) => Promise<MigrationExportResult>
  /** 解析导入文件，返回预览信息 */
  migrationParseImportFile: (filePath: string) => Promise<unknown>
  /** 确认导入 */
  migrationConfirmImport: (options: unknown) => Promise<{ success: boolean }>
  /** 打开文件选择对话框（支持 Domi 当前格式与旧 Proma 格式） */
  migrationOpenFileDialog: () => Promise<string | null>
  /** 打开文件保存对话框（选择导出路径） */
  migrationSaveFileDialog: (mode: MigrationFileMode) => Promise<string | null>
  /** 订阅双击迁移文件触发的导入事件 */
  ...
  /** 查询 token 用量统计（条目 + 摘要 + 按天/渠道/模型分组） */
  getUsageStats: (options?: UsageQueryOptions) => Promise<UsageQueryResult>
...
interface MigrationExportOptions {
...
interface MigrationExportOptionsV2 {
  mode: MigrationFileMode
  components: MigrationComponent[]
  outputPath: string
  sessionIds?: string[]
...
const electronAPI: ElectronAPI = {
  browser: {
    ...
    inspect: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.INSPECT, input),
    navigate: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.NAVIGATE, input),
    goBack: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.GO_BACK, input),
    goForward: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.GO_FORWARD, input),
    reload: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.RELOAD, input),
    stop: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.STOP, input),
    setZoom: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.SET_ZOOM, input),
    setFitToWidth: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.SET_FIT_TO_WIDTH, input),
    setLayout: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.SET_LAYOUT, input),
    selectElement: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.SELECT_ELEMENT, input),
    cancelElementSelection: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.CANCEL_ELEMENT_SELECTION, input),
    close: (input) => ipcRenderer.invoke(BROWSER_IPC_CHANNELS.CLOSE, input),
    onStateChanged: (callback) => {
      const listener = (_event: Electron.IpcRendererEvent, state: BrowserStateChange): void => callback(state)
      ipcRenderer.on(BROWSER_IPC_CHANNELS.STATE_CHANGED, listener)
      return () => ipcRenderer.removeListener(BROWSER_IPC_CHANNELS.STATE_CHANGED, listener)
    },
    onFocusEscapeRequested: (callback) => {
      const listener = (_event: Electron.IpcRendererEvent, request: BrowserFocusEscapeRequest): void => callback(request)
      ipcRenderer.on(BROWSER_IPC_CHANNELS.FOCUS_ESCAPE_REQUESTED, listener)
      return () => ipcRenderer.removeListener(BROWSER_IPC_CHANNELS.FOCUS_ESCAPE_REQUESTED, listener)
    },
  },
  terminal: {
    create: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.CREATE, input),
    list: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.LIST, input),
    inspect: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.INSPECT, input),
    snapshot: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.SNAPSHOT, input),
    input: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.INPUT, input),
    resize: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.RESIZE, input),
    interrupt: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.INTERRUPT, input),
    close: (input) => ipcRenderer.invoke(TERMINAL_IPC_CHANNELS.CLOSE, input),
    onOutput: (callback) => {
      const listener = (_event: Electron.IpcRendererEvent, output: TerminalOutputEvent): void => callback(output)
    ...
    onStateChanged: (callback) => {
      const listener = (_event: Electron.IpcRendererEvent, state: TerminalStateChange): void => callback(state)
      ipcRenderer.on(TERMINAL_IPC_CHANNELS.STATE_CHANGED, listener)
      return () => ipcRenderer.removeListener(TERMINAL_IPC_CHANNELS.STATE_CHANGED, listener)
    },
  },
  piExtensionTrust: {
    pickCandidate: (input) => ipcRenderer.invoke(PI_EXTENSION_TRUST_IPC_CHANNELS.PICK_CANDIDATE, input),
    list: (input) => ipcRenderer.invoke(PI_EXTENSION_TRUST_IPC_CHANNELS.LIST, input),
    approve: (input) => ipcRenderer.invoke(PI_EXTENSION_TRUST_IPC_CHANNELS.APPROVE, input),
    revoke: (input) => ipcRenderer.invoke(PI_EXTENSION_TRUST_IPC_CHANNELS.REVOKE, input),
  },
  piRunTiming: {
    query: (input) => ipcRenderer.invoke(PI_RUN_TIMING_IPC_CHANNELS.QUERY, input),
  },
  sessionCheckout: {
    ...
    operate: (input) => ipcRenderer.invoke(SESSION_CHECKOUT_IPC_CHANNELS.OPERATE, input),
  ...
  getGitWorkspaceSnapshot: (input: GitWorkspaceInspectInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_SNAPSHOT, input)
  },

  getGitWorkspaceDiff: (input: GitWorkspaceDiffRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_DIFF, input)
  },

  getGitWorkspaceHistory: (input: GitWorkspaceHistoryRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_HISTORY, input)
  },

  getGitWorkspaceBranches: (input: GitWorkspaceBranchesRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_BRANCHES, input)
  },

  getGitWorkspaceCommitFiles: (input: GitWorkspaceCommitFilesRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_COMMIT_FILES, input)
  },

  getGitWorkspaceCommitDiff: (input: GitWorkspaceCommitDiffRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_GIT_WORKSPACE_COMMIT_DIFF, input)
  },

  stageGitWorkspaceFiles: (input: GitWorkspaceStageRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.STAGE_GIT_WORKSPACE_FILES, input)
  },

  discardGitWorkspaceFiles: (input: GitWorkspaceDiscardRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.DISCARD_GIT_WORKSPACE_FILES, input)
  },

  commitGitWorkspace: (input: GitWorkspaceCommitRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.COMMIT_GIT_WORKSPACE, input)
  },

  checkoutGitWorkspaceBranch: (input: GitWorkspaceCheckoutRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.CHECKOUT_GIT_WORKSPACE_BRANCH, input)
  },

  gitWorkspacePullPush: (input: GitWorkspacePullPushRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GIT_WORKSPACE_PULL_PUSH, input)
  },
  ...
  getSessionTargetChanges: (sessionId: string) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNSTAGED_CHANGES, undefined, undefined, undefined, undefined, sessionId)
  },

  inspectSessionTargetFile: (sessionId: string, filePath: string, allowMissingLeaf?: boolean) => {
    return ipcRenderer.invoke(IPC_CHANNELS.INSPECT_SESSION_TARGET_FILE, sessionId, filePath, allowMissingLeaf)
  },

  getFileDiff: (input: import('@domi/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_FILE_DIFF, input)
  },

  getSessionTargetFileDiff: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_FILE_DIFF, input)
  },

  getUntrackedContent: (input: import('@domi/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNTRACKED_CONTENT, input)
  },

  getSessionTargetUntrackedContent: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNTRACKED_CONTENT, input)
  },

  revertFile: (input: import('@domi/shared').RevertFileInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.REVERT_FILE, input)
  },

  revertSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.REVERT_FILE, input)
  },

  getDiffContents: (input: import('@domi/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DIFF_CONTENTS, input)
  },

  getSessionTargetDiffContents: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DIFF_CONTENTS, input)
  },
  ...
  onEditableContextMenu: (callback: (request: EditableContextMenuRequest) => void) => {
    const listener = (_event: unknown, request: EditableContextMenuRequest): void => callback(request)
    ipcRenderer.on(EDITABLE_CONTEXT_MENU_CHANNELS.SHOW, listener)
    return () => ipcRenderer.removeListener(EDITABLE_CONTEXT_MENU_CHANNELS.SHOW, listener)
  },

  executeEditableContextMenuAction: (action: EditableContextMenuAction) => {
    ipcRenderer.send(EDITABLE_CONTEXT_MENU_CHANNELS.EXECUTE, action)
  },
  ...
  listGeneratedImages: (request: GeneratedImagesRequest) => {
    return ipcRenderer.invoke(GALLERY_IPC_CHANNELS.LIST_GENERATED_IMAGES, request)
  },

  readGeneratedImage: (request: GeneratedImagesRequest, localPath: string) => {
    return ipcRenderer.invoke(GALLERY_IPC_CHANNELS.READ_GENERATED_IMAGE, request, localPath)
  },

  saveGeneratedImageAs: (request: GeneratedImagesRequest, localPath: string, defaultFilename: string) => {
    return ipcRenderer.invoke(GALLERY_IPC_CHANNELS.SAVE_GENERATED_IMAGE_AS, request, localPath, defaultFilename)
  },
  ...
  getPiModelCatalogStatus: (channelId: string, modelId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_PI_MODEL_CATALOG_STATUS, channelId, modelId)
  },

  listVisionRelayModels: () => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_VISION_RELAY_MODELS)
  },
  ...
  updateSessionModelPresentationPreset: (sessionId: string, preset: ModelPresentationPreset) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.UPDATE_SESSION_MODEL_PRESENTATION_PRESET, sessionId, preset)
  },
  ...
  toggleFollowUpAgentSession: (id: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.TOGGLE_FOLLOW_UP, id)
  },
  ...
  previewRewindSession: (input: RewindSessionInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.PREVIEW_REWIND_SESSION, input)
  },
  ...
  getRewindUndoState: (input: RewindUndoInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_REWIND_UNDO_STATE, input)
  },

  undoRewindSession: (input: RewindUndoInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.UNDO_REWIND_SESSION, input)
  },

  getSessionTree: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_SESSION_TREE, sessionId)
  },

  navigateSessionTree: (input: NavigateSessionTreeInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.NAVIGATE_SESSION_TREE, input)
  },
  ...
  stopAgent: (sessionId: string, source = 'renderer-stop-control') => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.STOP_AGENT, sessionId, source)
  },

  retryAgentNow: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RETRY_NOW, sessionId)
  },
  ...
  cancelDeferredAgentMessage: (input: AgentQueuedMessageControlInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.CANCEL_QUEUED_MESSAGE, input)
  },
  moveDeferredAgentMessage: (input: AgentMoveQueuedMessageInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.MOVE_QUEUED_MESSAGE, input)
  },
  replaceAgentMessageQueue: (input: AgentReplaceMessageQueueInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.REPLACE_MESSAGE_QUEUE, input)
  },
  clearAgentMessageQueue: (input: AgentClearMessageQueueInput) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.CLEAR_MESSAGE_QUEUE, input)
  },
  ...
  getGlobalAgentCapabilities: () => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_GLOBAL_CAPABILITIES)
  },

  setGlobalSkillsEnabled: (enabled: boolean) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SET_GLOBAL_SKILLS_ENABLED, enabled)
  },

  setPiGlobalMcpEnabled: (enabled: boolean) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SET_PI_GLOBAL_MCP_ENABLED, enabled)
  },
  ...
  testMcpServer: (name: string, entry: import('@domi/shared').McpServerEntry) => {
  ...
  getSessionSkillTriggers: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_SESSION_SKILL_TRIGGERS, sessionId)
  },

  getWorkspaceSkillUsage: (workspaceSlug: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_WORKSPACE_SKILL_USAGE, workspaceSlug)
  },
  ...
  setVisibleAgentSession: (sessionId: string | null, notificationPresenceSessionId?: string | null) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SET_VISIBLE_SESSION, sessionId, notificationPresenceSessionId)
  },
  ...
  updateSessionPermissionMode: (sessionId: string, mode: DomiPermissionMode) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.UPDATE_SESSION_PERMISSION_MODE, sessionId, mode)
  },

  updateSessionExecutionControls: (sessionId: string, controls: AgentExecutionControlsUpdate) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.UPDATE_SESSION_EXECUTION_CONTROLS, sessionId, controls)
  },

  getSessionCapabilityGrants: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_SESSION_CAPABILITY_GRANTS, sessionId)
  },

  revokeSessionCapabilityGrant: (sessionId: string, grantId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.REVOKE_SESSION_CAPABILITY_GRANT, sessionId, grantId)
  },

  onSessionCapabilityGrantsChanged: (callback: (payload: SessionCapabilityGrantsChangedPayload) => void) => {
    const listener = (_: unknown, payload: SessionCapabilityGrantsChangedPayload): void => callback(payload)
    ipcRenderer.on(AGENT_IPC_CHANNELS.SESSION_CAPABILITY_GRANTS_CHANGED, listener)
    return () => { ipcRenderer.removeListener(AGENT_IPC_CHANNELS.SESSION_CAPABILITY_GRANTS_CHANGED, listener) }
  },
  ...
  // 全局 Work Activity
  getWorkActivity: () => ipcRenderer.invoke(AGENT_IPC_CHANNELS.GET_WORK_ACTIVITY),
  markWorkActivityViewed: (rootSessionId: string) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.MARK_WORK_ACTIVITY_VIEWED, rootSessionId),
  acknowledgeWorkActivityOutcome: (rootSessionId: string) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.ACKNOWLEDGE_WORK_ACTIVITY_OUTCOME, rootSessionId),
  removeWorkActivityCompleted: (rootSessionId: string) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.REMOVE_WORK_ACTIVITY_COMPLETED, rootSessionId),
  removeAllWorkActivityCompleted: () => ipcRenderer.invoke(AGENT_IPC_CHANNELS.REMOVE_ALL_WORK_ACTIVITY_COMPLETED),
  stopWorkActivitySession: (rootSessionId: string) => ipcRenderer.invoke(AGENT_IPC_CHANNELS.STOP_WORK_ACTIVITY_SESSION, rootSessionId),
  onWorkActivityChanged: (callback: () => void) => {
    const listener = (): void => callback()
    ipcRenderer.on(AGENT_IPC_CHANNELS.WORK_ACTIVITY_CHANGED, listener)
    return () => { ipcRenderer.removeListener(AGENT_IPC_CHANNELS.WORK_ACTIVITY_CHANGED, listener) }
  },
  onWorkActivityNotification: (callback: (event: WorkActivityNotificationEvent) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, payload: WorkActivityNotificationEvent): void => callback(payload)
    ipcRenderer.on(AGENT_IPC_CHANNELS.WORK_ACTIVITY_NOTIFICATION, listener)
    return () => { ipcRenderer.removeListener(AGENT_IPC_CHANNELS.WORK_ACTIVITY_NOTIFICATION, listener) }
  },
  onWorkActivityNotificationNavigate: (callback: (target: WorkActivityNotificationTarget) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, payload: WorkActivityNotificationTarget): void => callback(payload)
    ipcRenderer.on(AGENT_IPC_CHANNELS.WORK_ACTIVITY_NOTIFICATION_NAVIGATE, listener)
    return () => { ipcRenderer.removeListener(AGENT_IPC_CHANNELS.WORK_ACTIVITY_NOTIFICATION_NAVIGATE, listener) }
  },
  ...
  openFileOrFolderDialog: (sessionId: string, kind: ComposerAttachmentKind) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.OPEN_FILE_OR_FOLDER_DIALOG, sessionId, kind)
  },
  ...
  addWorktreeRepo: (workspaceSlug: string, repo: import('@domi/shared').WorkspaceWorktreeRepo) => {
  ...
  listDirectory: (dirPath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_DIRECTORY, dirPath, access)
  },

  listSessionTargetDirectory: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_DIRECTORY, input.relativePath, { sessionId: input.sessionId })
  },

  listSessionProjectArtifacts: (sessionId: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_SESSION_PROJECT_ARTIFACTS, sessionId)
  },

  deleteFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.DELETE_FILE, filePath, access)
  },

  deleteSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.DELETE_FILE, input.relativePath, { sessionId: input.sessionId })
  },

  openFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.OPEN_FILE, filePath, access)
  },

  openSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.OPEN_FILE, input.relativePath, { sessionId: input.sessionId })
  },
  ...
  systemOpenFile: (filePath: string, appName?: string, access?: import('@domi/shared').FileAccessOptions) => {
  ...
  getDefaultAppForFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DEFAULT_APP_FOR_FILE, filePath, access) as Promise<import('@domi/shared').DefaultAppInfo | null>
  },

  showInFolder: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_IN_FOLDER, filePath, access)
  },

  showSessionTargetInFolder: (input: import('@domi/shared').SessionTargetFileRequest) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_IN_FOLDER, input.relativePath, { sessionId: input.sessionId })
  },

  openFolderInTerminal: (folderPath: string, access?: import('@domi/shared').FileAccessOptions & { sessionId: string }) => {
  ...
  copyFileSystemItem: (filePaths: string[], access?: import('@domi/shared').FileAccessOptions): Promise<void> => {
    return ipcRenderer.invoke(IPC_CHANNELS.COPY_FILE_SYSTEM_ITEM, filePaths, access)
  },

  copyFileSystemPath: (filePaths: string[], access?: import('@domi/shared').FileAccessOptions): Promise<void> => {
    return ipcRenderer.invoke(IPC_CHANNELS.COPY_FILE_SYSTEM_PATH, filePaths, access)
  },

  resolveAndReadFile: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-and-read', filePath, access) as Promise<{ resolvedPath: string; content: string } | null>
  },

  writeTextFile: (filePath: string, content: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:write-text', filePath, content, access) as Promise<boolean>
  },

  resolveFilePath: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-path', filePath, access) as Promise<ResolvedPreviewPath | null>
  },

  resolveHtmlPreviewPath: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-html-preview-path', filePath, access) as Promise<ResolvedPreviewPath | null>
  },

  preparePdfPreview: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-pdf-preview', filePath, access) as Promise<{ tmpHtmlUrl: string } | null>
  },

  readBinaryBase64: (filePath: string, access?: import('@domi/shared').FileAccessOptions, maxSize?: number) => {
    return ipcRenderer.invoke('file:read-binary-base64', filePath, access, maxSize) as Promise<string | null>
  },

  docxToHtml: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:docx-to-html', filePath, access) as Promise<{ resolvedPath: string; html: string } | null>
  },

  officeToHtml: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:office-to-html', filePath, access) as Promise<import('@domi/shared').OfficePreviewResult | null>
  },
  ...
  renameFile: (filePath: string, newName: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_FILE, filePath, newName, access)
  },

  renameSessionTargetFile: (input: import('@domi/shared').SessionTargetFileRequest, newName: string) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_FILE, input.relativePath, newName, { sessionId: input.sessionId })
  },

  moveFile: (filePath: string, targetDir: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.MOVE_FILE, filePath, targetDir, access)
  },

  listAttachedDirectory: (dirPath: string, access?: import('@domi/shared').FileAccessOptions) => {
  ...
  showAttachedInFolder: (filePath: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_ATTACHED_IN_FOLDER, filePath, access)
  },

  renameAttachedFile: (filePath: string, newName: string, access?: import('@domi/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_ATTACHED_FILE, filePath, newName, access)
  },

  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@domi/shared').FileAccessOptions) => {
  ...
  searchWorkspaceFiles: (rootPath: string, query: string, limit = 20, additionalPaths?: string[], sessionPaths?: string[], sessionId?: string, pathSpace?: import('@domi/shared').FileAccessOptions['pathSpace']) => {
    return ipcRenderer.invoke(
  ...
  searchSessionTargetFiles: (sessionId: string, query: string, limit = 20) => {
    return ipcRenderer.invoke(
      AGENT_IPC_CHANNELS.SEARCH_WORKSPACE_FILES,
      undefined,
      query,
  ...
  updateWorkPromptActivation: (id: string, enabled: boolean) => {
    return ipcRenderer.invoke(SYSTEM_PROMPT_IPC_CHANNELS.UPDATE_WORK_ACTIVATION, id, enabled)
  },

  setDefaultPrompt: (id: string | null, scope?: SystemPromptScope) => {
    return ipcRenderer.invoke(SYSTEM_PROMPT_IPC_CHANNELS.SET_DEFAULT, id, scope)
  },
  ...
  saveFeishuBotConfig: (input: import('@domi/shared').FeishuBotConfigInput) => {
  ...
  onFeishuRegisterQrcode: (callback: (payload: import('@domi/shared').FeishuRegisterAppQRCode) => void) => {
    const listener = (_: unknown, payload: import('@domi/shared').FeishuRegisterAppQRCode) => callback(payload)
  ...
  onFeishuRegisterStatus: (callback: (payload: import('@domi/shared').FeishuRegisterAppStatus) => void) => {
    const listener = (_: unknown, payload: import('@domi/shared').FeishuRegisterAppStatus) => callback(payload)
  ...
  saveDingTalkBotConfig: (input: import('@domi/shared').DingTalkBotConfigInput) => {
  ...
  migrationExport: (options: MigrationExportOptions) => {
    return ipcRenderer.invoke('migration:export', options)
  },

  migrationExportV2: (options: MigrationExportOptionsV2) => {
  ...
  migrationSaveFileDialog: (mode: MigrationFileMode) => {
  ...
  getUsageStats: (options?: UsageQueryOptions) => {
    return ipcRenderer.invoke(USAGE_IPC_CHANNELS.QUERY, options)
  },
