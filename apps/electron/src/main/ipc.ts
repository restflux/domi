...
import { ipcMain, nativeTheme, shell, dialog, BrowserWindow, app, clipboard, nativeImage, type OpenDialogOptions } from 'electron'
...
import { IPC_CHANNELS, CHANNEL_IPC_CHANNELS, CHAT_IPC_CHANNELS, AGENT_IPC_CHANNELS, ENVIRONMENT_IPC_CHANNELS, INSTALLER_IPC_CHANNELS, PROXY_IPC_CHANNELS, GITHUB_RELEASE_IPC_CHANNELS, SYSTEM_PROMPT_IPC_CHANNELS, CHAT_TOOL_IPC_CHANNELS, GALLERY_IPC_CHANNELS, FEISHU_IPC_CHANNELS, DINGTALK_IPC_CHANNELS, WECHAT_IPC_CHANNELS, AUTOMATION_IPC_CHANNELS, PLANNING_IPC_CHANNELS, PLANNING_CONFLICT_ERROR, MIGRATION_IMPORT_EXTENSIONS, PREVIEW_TEMP_DIR_NAME, USAGE_IPC_CHANNELS, assertMigrationFileMode, getMigrationExportExtension, isSupportedMigrationImportFile, isDomiPermissionMode, normalizePathForCompare } from '@domi/shared'
...
import type {
  ...
  ForkSessionInput,
  RewindSessionInput,
  RewindSessionPreview,
  RewindSessionResult,
  RewindUndoInput,
  RewindUndoState,
  UndoRewindSessionResult,
  NavigateSessionTreeInput,
  ...
  SkillTriggerEvent,
  SkillUsageStats,
  GeneratedImagesRequest,
  GeneratedImageItem,
} from '@domi/shared'
import type { UserProfile, AppSettings } from '../types'
import { getRuntimeStatus, getGitRepoStatus, reinitializeRuntime } from './lib/runtime-init'
import { getUnstagedChanges, getFileDiff, getUntrackedContent, revertFile, getDiffContents, listWorktrees, getWorktreeChanges, getMainRepoRoot, findGitRoot } from './lib/git-diff-service'
import { registerDomiDirectoryPath, registerDomiFilePath } from './lib/local-file-protocol'
import {
  createResolvedHtmlPreviewPath,
  createResolvedPreviewPath,
  type ResolvedPreviewPath,
} from './lib/resolved-preview-path.ts'
import { assertMigrationExportRequest } from './lib/migration-boundary.ts'
import {
  listChannels,
  createChannel,
  updateChannel,
...
} from './lib/adapters/pi-model-registry'
import { collectVisionRelayModelOptions } from './lib/vision-relay-model-options'
...
import { getGlobalAgentCapabilities } from './lib/global-agent-capabilities'
import { refreshAgentIslandConfiguration } from './lib/agent-island-service'
import { reconcileLinkedImageToolStates, updateLinkedBuiltinMcpState, updateLinkedChatToolState } from './lib/linked-image-tool-state'
import { normalizeAgentStopSource } from './lib/agent-stop-source.ts'
...
import {
  ...
  assertAgentWorkspaceSessionLifecycle,
} from './lib/agent-session-manager'
import { runAgent, reserveAgentSessionStart, consumeCancelledAgentSessionStart, stopAgent, retryAgentNow, generateAgentTitle, saveFilesToAgentSession, saveFilesToWorkspaceFiles, isAgentSessionActive, queueAgentMessage, submitOrEnqueueAgentMessage, cancelDeferredAgentMessage, moveDeferredAgentMessage, replaceAgentMessageQueue, clearAgentMessageQueue, updateAgentPermissionMode, updateSessionExecutionControls, previewAgentSessionRewind, rewindAgentSession, getAgentRewindUndoState, undoAgentSessionRewind, getAgentSessionTree, navigateAgentSessionTree, setAgentStreamForegroundSession, respondAgentAskUser, respondAgentExitPlan, notifyAgentPermissionResolved } from './lib/agent-service'
import {
  acknowledgeWorkActivityOutcome,
  getWorkActivityProjection,
  markWorkActivityViewed,
  removeAllWorkActivityCompleted,
  removeWorkActivityCompleted,
  stopWorkActivitySession,
} from './lib/work-activity-service.ts'
import { describeWorkActivityHostFailure } from './lib/work-activity-host-facts.ts'
import { broadcastWorkActivityChanged } from './lib/work-activity-events.ts'
import { permissionService } from './lib/agent-permission-service'
import { configureLocalMaintenanceTransactionService } from './lib/local-maintenance-transaction.ts'
import {
  confirmAgentWorktreeIterationContinuation,
  worktreeContinuationAuthorizationRegistry,
} from './lib/agent-worktree-continuation-authorization.ts'
import { askUserService } from './lib/agent-ask-user-service'
import { exitPlanService } from './lib/agent-exit-plan-service'
import { mergeAskUserAttachments, mergeExitPlanFeedbackAttachments } from './lib/user-input-attachments'
import { getAgentSessionWorkspacePath, getAgentWorkspacesDir, getWorkspaceSkillsDir, getScratchPadPath, getExtensionTrustPath, getPiRunTimingAuditPath, getAgentSessionSkillTriggersPath, getWorkspaceSkillUsagePath } from './lib/config-paths.ts'
import { FileExtensionTrustStore } from './lib/adapters/pi-extension-trust.ts'
...
import { SessionTargetDiffService } from './lib/session-target-diff-service.ts'
import { SessionGitWorkspaceService } from './lib/git-workspace/session-git-workspace-service.ts'
import { SessionCheckoutOperationGuard } from './lib/session-checkout/session-checkout-operation-guard.ts'
import { resolveSessionTargetRoot } from './lib/agent-session-target.ts'
import { getSessionCheckoutModule } from './lib/session-checkout/production.ts'
import { getAgentFileCheckpointStore } from './lib/agent-file-checkpoint-production.ts'
import { SessionProjectArtifactService } from './lib/session-project-artifact-service.ts'
import { getCachedDefaultAppInfo, saveCachedDefaultAppInfo } from './lib/default-app-cache'
import { calculateStorageStats, cleanupStorage, cleanupTempFiles } from './lib/storage-service'
import type { CleanupOptions } from './lib/storage-service'
import { queryUsage } from './lib/usage-record-service'
...
import {
  ...
  getAgentWorkspace,
  getProjectFilesPath,
  resolveAgentWorkspaceProjectFolder,
...
import { readSessionSkillTriggers, readWorkspaceSkillUsage } from './lib/skill-trigger-recorder'
...
function getAuthorizedRoots(options?: FileAccessOptions): string[] {
  const roots: string[] = [
    ...
    join(tmpdir(), PREVIEW_TEMP_DIR_NAME),
  ]
...
function normalizeFileAccessOptions(value?: FileAccessOptions | string[]): FileAccessOptions | undefined {
  ...
  const rawPathSpace: unknown = (value as Record<string, unknown>).pathSpace
  if (rawPathSpace !== undefined
    && rawPathSpace !== 'session-target'
    && rawPathSpace !== 'session-workbench'
    && rawPathSpace !== 'session-local-project') {
    throw new Error('Agent 文件 IPC 收到无效 pathSpace')
  }
  return {
    sessionId: typeof value.sessionId === 'string' ? value.sessionId : undefined,
    workspaceSlug: typeof value.workspaceSlug === 'string' ? value.workspaceSlug : undefined,
    pathSpace: rawPathSpace as FileAccessOptions['pathSpace'],
...
function normalizeSessionFileAccessOptions(
  value?: FileAccessOptions | string[],
): (FileAccessOptions & { sessionId: string }) | null {
  const options = normalizeFileAccessOptions(value)
  return options?.sessionId ? { ...options, sessionId: options.sessionId } : null
}
 ...
 *
 * worktree 常被放在主仓库之外（如 ~/domi-dev/worktrees/xxx），其路径不在任何
 * 授权根下，会被 ensurePathAllowed 拒绝。但只要它回溯到的主仓库已被授权，就应放行。
 * 用 git 自身背书（--git-common-dir），避免粗暴跳过安全检查。
 */
async function isPathAllowedForSession(
  filePath: string,
  options: FileAccessOptions | undefined,
  sessionTargetAccess: SessionTargetFileAccessService,
  allowMissingLeaf = false,
): Promise<boolean> {
  if (sessionTargetAccess.isActivePiSession(options)) {
    return sessionTargetAccess.authorizeFileRequest(
      filePath,
      options,
      { allowMissingLeaf },
    )
  }
  return isPathAllowed(filePath, options)
}
...
async function ensurePathAllowedWithWorktree(
  ...
  if (sessionTargetAccess.isActivePiSession(options)) {
    const allowed = await sessionTargetAccess.authorizeFileRequest(filePath, options)
    if (!allowed) console.warn('[IPC] 拒绝非当前 Agent 文件路径空间:', filePath)
    return allowed
  }
  if (isPathAllowed(filePath, options)) return true
  ...
  if (mainRepo) {
    ...
    for (const workspaceSlug of getWorkspaceSlugsForAccess(options)) {
      let repos: import('@domi/shared').WorkspaceWorktreeRepo[]
      try {
...
/** 统一解析只读预览路径；所有候选路径最终仍由当前会话授权边界校验。 */
async function resolvePreviewReadPath(
  filePath: string,
  options: FileAccessOptions | undefined,
  sessionTargetAccess: SessionTargetFileAccessService,
): Promise<string | null> {
  return resolveAuthorizedPreviewReadPath(filePath, options, sessionTargetAccess, {
    async resolveCandidatePath(candidatePath, candidateBasePaths) {
      const { resolveFilePath } = await import('./lib/file-preview-service.ts')
      return resolveFilePath(candidatePath, candidateBasePaths)
    },
    async authorizeResolvedPath(resolvedPath, accessOptions) {
      return isPathAllowedForSession(resolvedPath, accessOptions, sessionTargetAccess)
    },
  })
}
 ...
 */
const defaultAppCache = new Map<string, import('@domi/shared').DefaultAppInfo>()
...
async function getMacAppIconViaSips(appPath: string): Promise<string> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'domi-icon-'))
...
): Promise<import('@domi/shared').DefaultAppInfo | null> {
...
if let appUrl = NSWorkspace.shared.urlForApplication(toOpen: url) {
  ...
  const info: import('@domi/shared').DefaultAppInfo = { name: appName, appPath, iconDataUrl }
...
const visionDirectoryGrants = new VisionRelayDirectoryGrantRegistry()

export interface IpcRuntimeModules {
  browser?: { service: BrowserSessionService; guard: BrowserIpcGuard }
  terminal?: { service: TerminalSessionService; guard: TerminalIpcGuard }
}

export function registerIpcHandlers(modules: IpcRuntimeModules = {}): void {
  const { browser, terminal } = modules
  console.log('[IPC] 正在注册 IPC 处理器...')
  try {
    reconcileLinkedImageToolStates()
  } catch (error) {
    // 配置迁移失败不应阻断应用启动；后续从任一设置入口切换时仍会再次同步。
    console.error('[IPC] 生图工具开关迁移失败:', error)
  }

  const extensionTrustService = new PiExtensionTrustService({
    store: new FileExtensionTrustStore(getExtensionTrustPath()),
    picker: async (kind) => {
    ...
    workspaceResolver: (workspaceId) => {
      ...
      return {
        ...
        projectRoot: getProjectFilesPath(workspace.slug),
      }
    },
  })
  ...
  const sessionCheckoutOperationGuard = new SessionCheckoutOperationGuard({
    listSessionIds: () => listAgentSessions().map((session) => session.id),
    isSessionActive: isAgentSessionActive,
    inspect: (sessionId) => sessionCheckout.inspect(sessionId),
    stopSession: stopAgent,
  })
  const localMaintenance = configureLocalMaintenanceTransactionService({
    inspect: (sessionId) => sessionCheckout.inspect(sessionId),
    lease: (sessionId) => sessionCheckout.lease(sessionId),
  })
  permissionService.configureDeferredWorktreeApprovals({
    inspect: (sessionId) => sessionCheckout.inspect(sessionId),
    assertIdle: (sessionId, options) => sessionCheckoutOperationGuard.assertIdle(sessionId, options),
    preflight: async (sessionId, expectedRevision) => {
      const preflight = sessionCheckout.preflight
      if (!preflight) throw new Error('Worktree Apply 预检执行器尚未就绪')
      return preflight(sessionId, expectedRevision)
    },
    operate: (input) => sessionCheckout.operate(input),
    captureLocalMaintenance: (sessionId) => localMaintenance.captureRequestSnapshot(sessionId),
    startLocalMaintenance: (sessionId, goal, snapshot) => localMaintenance.start(sessionId, goal, snapshot),
  })
  if (browser) registerBrowserIpc(ipcMain, browser.service, browser.guard)
  if (terminal) registerTerminalIpc(ipcMain, terminal.service, terminal.guard)
  registerSessionCheckoutIpc(ipcMain, sessionCheckout, (sessionId, sessionTarget) => {
    updateAgentSessionMeta(sessionId, { sessionTarget })
  ...
  async (input) => {
    ...
    const prepared = await prepareAgentSessionHandoff({
      originSessionId: input.sessionId,
      expectedRevision: input.expectedRevision,
    ...
    worktreeContinuationAuthorizationRegistry,
    {
      ...
      assertIdle: (sessionId) => sessionCheckoutOperationGuard.assertIdle(sessionId),
      inspectTarget: (sessionId) => sessionCheckout.inspect(sessionId),
      beginNextIteration: (sessionId) => sessionCheckout.beginNextIteration(sessionId),
      createToken: randomUUID,
    },
  ))
  const sessionTargetAccess = new SessionTargetFileAccessService({
    getSession: getAgentSessionMeta,
    getWorkspace: getAgentWorkspace,
    getProjectRoot: getProjectFilesPath,
    getSessionWorkbenchRoot: (sessionId) => {
      const session = getAgentSessionMeta(sessionId)
      if (!session?.workspaceId) return null
      const workspace = getAgentWorkspace(session.workspaceId)
      return workspace ? getAgentSessionWorkspacePath(workspace.slug, sessionId) : null
    },
    getWorkspaceAttachedDirectories,
    getWorkspaceAttachedFiles,
    resolveTargetRoot: resolveSessionTargetRoot,
    inspectTarget: async (sessionId) => {
      const target = await sessionCheckout.inspect(sessionId)
      return { baseOid: target.source.oid, kind: target.checkout.kind }
    },
    resolveGitRoot: findGitRoot,
  })
  const sessionTargetDiff = new SessionTargetDiffService({
    access: sessionTargetAccess,
    getUnstagedChanges,
    getWorktreeChanges,
    getFileDiff,
    getUntrackedContent,
  ...
  const sessionProjectArtifacts = new SessionProjectArtifactService({
    inspectTarget: (sessionId) => sessionCheckout.inspect(sessionId),
    readPersistedChangedFiles: (sessionId) => sessionCheckout.readSessionChangedFiles(sessionId),
    readCheckpointPaths: (sessionId) => getAgentFileCheckpointStore().listTrackedPaths(sessionId),
    getCurrentChanges: async (sessionId) => {
      const route = await sessionTargetDiff.getChanges(sessionId)
      return route.handled
        ? route.value
        : { isGitRepo: false, files: [], untrackedFiles: [], gitRootNames: [] }
    },
    resolveExistingFiles: (sessionId, relativePaths) => (
      sessionTargetAccess.resolveExistingRelativeFiles(sessionId, relativePaths)
    ),
    statFile: statSync,
  })
  const sessionGitWorkspace = new SessionGitWorkspaceService({
    resolveTarget: async (sessionId) => {
      const [root, target] = await Promise.all([
        resolveSessionTargetRoot(sessionId),
        sessionCheckout.inspect(sessionId),
      ])
      return { root, kind: target.checkout.kind }
    },
  })
  ...
  // 轻量 Git 面板只接受 sessionId，由主进程解析权威 Session Target。
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_SNAPSHOT,
    async (_, input: GitWorkspaceInspectInput): Promise<GitWorkspaceSnapshot> => {
      return sessionGitWorkspace.inspect(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_DIFF,
    async (_, input: GitWorkspaceDiffRequest): Promise<GitWorkspaceDiffContents | null> => {
      return sessionGitWorkspace.getDiff(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_HISTORY,
    async (_, input: GitWorkspaceHistoryRequest): Promise<GitWorkspaceHistoryResult> => {
      return sessionGitWorkspace.getHistory(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_BRANCHES,
    async (_, input: GitWorkspaceBranchesRequest): Promise<GitWorkspaceBranchesResult> => {
      return sessionGitWorkspace.getBranches(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_COMMIT_FILES,
    async (_, input: GitWorkspaceCommitFilesRequest): Promise<GitWorkspaceCommitFilesResult> => {
      return sessionGitWorkspace.getCommitFiles(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GET_GIT_WORKSPACE_COMMIT_DIFF,
    async (_, input: GitWorkspaceCommitDiffRequest): Promise<GitWorkspaceDiffContents | null> => {
      return sessionGitWorkspace.getCommitDiff(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.STAGE_GIT_WORKSPACE_FILES,
    async (_, input: GitWorkspaceStageRequest): Promise<GitWorkspaceOperationResult> => {
      return sessionGitWorkspace.stage(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.DISCARD_GIT_WORKSPACE_FILES,
    async (_, input: GitWorkspaceDiscardRequest): Promise<GitWorkspaceOperationResult> => {
      return sessionGitWorkspace.discard(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.COMMIT_GIT_WORKSPACE,
    async (_, input: GitWorkspaceCommitRequest): Promise<GitWorkspaceOperationResult> => {
      return sessionGitWorkspace.commit(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.CHECKOUT_GIT_WORKSPACE_BRANCH,
    async (_, input: GitWorkspaceCheckoutRequest): Promise<GitWorkspaceOperationResult> => {
      return sessionGitWorkspace.checkout(input)
    },
  )
  ipcMain.handle(
    IPC_CHANNELS.GIT_WORKSPACE_PULL_PUSH,
    async (_, input: GitWorkspacePullPushRequest): Promise<GitWorkspaceOperationResult> => {
      return sessionGitWorkspace.pullPush(input)
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, dirPath: unknown, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => {
      const access = normalizeFileAccessOptions({ sessionId })
      const activeRoute = await sessionTargetDiff.getChanges(sessionId, dirPath)
      if (activeRoute.handled) return activeRoute.value
  ...
  // 检查 Agent 写工具目标，并只返回当前 Session Target 内的相对路径。
  ipcMain.handle(
    IPC_CHANNELS.INSPECT_SESSION_TARGET_FILE,
    async (_, sessionId: unknown, filePath: unknown, allowMissingLeaf?: boolean): Promise<SessionTargetFileInspection | null> => {
      if (typeof sessionId !== 'string' || !sessionId || typeof filePath !== 'string' || !filePath) return null
      return sessionTargetAccess.inspectTargetFile(sessionId, filePath, allowMissingLeaf === true)
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput | SessionTargetFileRequest) => {
      const sessionId = input?.sessionId
      const access = normalizeFileAccessOptions({ sessionId })
      if (sessionTargetAccess.isActivePiSession(access)) {
        const relativePath = (input as SessionTargetFileRequest).relativePath
        if (typeof relativePath !== 'string' || !relativePath) return ''
        const route = await sessionTargetDiff.getFileDiff({ sessionId: access.sessionId, relativePath })
        return route.handled ? route.value : ''
      }
      const { dirPath, filePath, gitRoot } = input as GetFileDiffInput
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string'
        || typeof sessionId !== 'string' || !sessionId) {
        console.warn('[IPC] git:get-file-diff 缺少有效 Session 上下文或文件参数')
        return ''
      }
      if (!(await ensurePathAllowedWithWorktree(dirPath, access, sessionTargetAccess))
        || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access, sessionTargetAccess)))) return ''
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput | SessionTargetFileRequest) => {
      ...
      if (sessionTargetAccess.isActivePiSession(access)) {
        const relativePath = (input as SessionTargetFileRequest).relativePath
        if (typeof relativePath !== 'string' || !relativePath) return ''
        const route = await sessionTargetDiff.getUntrackedContent({ sessionId: access.sessionId, relativePath })
        return route.handled ? route.value : ''
      }
      const { dirPath, filePath, gitRoot } = input as GetFileDiffInput
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string'
        || typeof sessionId !== 'string' || !sessionId) {
        console.warn('[IPC] git:get-untracked-content 缺少有效 Session 上下文或文件参数')
        return ''
      }
      if (!(await ensurePathAllowedWithWorktree(dirPath, access, sessionTargetAccess))
        || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access, sessionTargetAccess)))) return ''
  ...
  ipcMain.handle(
    ...
    async (_, input: RevertFileInput | SessionTargetFileRequest) => {
      const sessionId = input?.sessionId
      const access = normalizeFileAccessOptions({ sessionId })
      if (sessionTargetAccess.isActivePiSession(access)) {
        const relativePath = (input as SessionTargetFileRequest).relativePath
        if (typeof relativePath !== 'string' || !relativePath) return
        await sessionTargetDiff.revertFile({ sessionId: access.sessionId, relativePath })
        return
      }
      const { dirPath, filePath, gitRoot } = input as RevertFileInput
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string'
        || typeof sessionId !== 'string' || !sessionId) {
        console.warn('[IPC] git:revert-file 缺少有效 Session 上下文或文件参数')
        return
      }
      if (!(await ensurePathAllowedWithWorktree(dirPath, access, sessionTargetAccess))
        || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access, sessionTargetAccess)))) return
  ...
  ipcMain.handle(
    ...
    async (_, input: GetFileDiffInput | SessionTargetFileRequest) => {
      ...
      if (sessionTargetAccess.isActivePiSession(access)) {
        ...
        const route = await sessionTargetDiff.getDiffContents({ sessionId: access.sessionId, relativePath })
        return route.handled ? route.value : null
      }
      const { dirPath, filePath, gitRoot, baseRef } = input as GetFileDiffInput
      if (!dirPath || !filePath || typeof dirPath !== 'string' || typeof filePath !== 'string'
        || typeof sessionId !== 'string' || !sessionId) {
        console.warn('[IPC] git:get-diff-contents 缺少有效 Session 上下文或文件参数')
        return null
      }
      if (!(await ensurePathAllowedWithWorktree(dirPath, access, sessionTargetAccess))
        || (gitRoot && !(await ensurePathAllowedWithWorktree(gitRoot, access, sessionTargetAccess)))) return null
      return getDiffContents(dirPath, filePath, gitRoot, baseRef)
    }
  )

  // 旧手动 Worktree 浏览保留，但 active Pi 会话不能用它扩大 Session Target。
  ipcMain.handle(
    IPC_CHANNELS.LIST_WORKTREES,
    async (_, repoPath: string, sessionId: string) => {
      if (!repoPath || typeof repoPath !== 'string') return []
      const activeRoute = await sessionTargetDiff.listWorktrees(sessionId, repoPath)
      if (activeRoute.handled) return activeRoute.value
  ...
  ipcMain.handle(
    ...
    async (_, worktreePath: string, baseBranch: string, sessionId: string) => {
      ...
      const access = normalizeFileAccessOptions({ sessionId })
      const activeRoute = await sessionTargetDiff.getWorktreeChanges(sessionId, worktreePath)
      if (activeRoute.handled) return activeRoute.value
      if (!(await ensurePathAllowedWithWorktree(worktreePath, access, sessionTargetAccess))) {
        return { isGitRepo: false, files: [], untrackedFiles: [], gitRootNames: [] }
      }
  ...
  ipcMain.handle(
    ...
    async (event, input: DetachedPreviewWindowInput): Promise<string | null> => {
      ...
      const access = normalizeFileAccessOptions({
        sessionId: input.sessionId,
        pathSpace: input.pathSpace,
        candidateBasePaths: [input.dirPath, ...(input.basePaths ?? [])],
      })
      const previewPath = input.previewOnly
        ? await resolvePreviewReadPath(input.filePath, access, sessionTargetAccess)
        : isAbsolute(input.filePath) ? input.filePath : resolve(input.dirPath, input.filePath)
      const dirAllowed = input.previewOnly
        || await ensurePathAllowedWithWorktree(input.dirPath, access, sessionTargetAccess)
      const previewAllowed = input.previewOnly
        ? previewPath !== null
        : Boolean(previewPath && await ensurePathAllowedWithWorktree(previewPath, access, sessionTargetAccess))
      if (!dirAllowed || !previewAllowed) {
        console.warn('[IPC] preview:open-detached 拒绝非当前会话授权路径')
        return null
      }
  ...
  // 在系统剪贴板中写入纯文本。Windows renderer 的 Clipboard API 可能受权限限制，
  // 统一由受信 preload 桥接到主进程 Electron clipboard。
  ipcMain.handle(
  ...
  const resolveClipboardFilePaths = async (
    filePaths: string[],
    access?: FileAccessOptions | string[],
  ): Promise<string[]> => {
    if (!Array.isArray(filePaths) || filePaths.length === 0 || filePaths.length > 100) {
      throw new TypeError('文件剪贴板需要 1 到 100 个路径')
    }
    const options = normalizeFileAccessOptions(access)
    const resolvedPaths: string[] = []
    for (const filePath of filePaths) {
      if (typeof filePath !== 'string' || !filePath) throw new TypeError('文件路径必须是非空字符串')
      const resolvedPath = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!resolvedPath) throw new Error('访问路径超出当前会话的授权范围，或文件已不存在')
      if (!resolvedPaths.includes(resolvedPath)) resolvedPaths.push(resolvedPath)
    }
    return resolvedPaths
  }

  // 把文件/文件夹本身写入系统文件剪贴板，可直接粘贴到资源管理器、桌面等目标。
  ipcMain.handle(
    IPC_CHANNELS.COPY_FILE_SYSTEM_ITEM,
    async (_, filePaths: string[], access?: FileAccessOptions | string[]): Promise<void> => {
      const resolvedPaths = await resolveClipboardFilePaths(filePaths, access)
      const { copyFileSystemEntriesToClipboard } = await import('./lib/file-clipboard-service.ts')
      await copyFileSystemEntriesToClipboard(resolvedPaths)
    },
  )

  // 复制真实物理路径文本；解析与授权留在主进程，renderer 不接触 managed checkout 根。
  ipcMain.handle(
    IPC_CHANNELS.COPY_FILE_SYSTEM_PATH,
    async (_, filePaths: string[], access?: FileAccessOptions | string[]): Promise<void> => {
      const resolvedPaths = await resolveClipboardFilePaths(filePaths, access)
      clipboard.writeText(resolvedPaths.join('\n'))
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, appName?: string, access?: FileAccessOptions | string[]): Promise<void> => {
      const options = normalizeFileAccessOptions(access)
      const absPath = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!absPath) {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@domi/shared').EditorApp[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@domi/shared').DefaultAppInfo | null> => {
      if (!filePath || typeof filePath !== 'string') return null
      try {
        const options = normalizeFileAccessOptions(access)
        const resolvedPath = options
          ? await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
          : filePath
        if (!resolvedPath) {
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string): Promise<import('@domi/shared').ChannelPlanQuotaResult> => {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@domi/shared').CodexOAuthLoginResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      if (isAgentSessionActive(id) || terminal?.service.hasRunningOwner(id)) {
        throw Object.assign(new Error('Agent 或终端仍在运行，请先停止后再删除'), {
          code: 'operation_not_allowed',
        })
      }
      await deleteAgentSession(id)
      await browser?.service.closeOwner(id)
      await terminal?.service.closeOwner(id)
      // 只有生命周期与持久化删除成功后才清理运行态服务，避免拒绝删除时留下半清理状态。
      permissionService.clearSessionCapabilities(id)
      permissionService.clearSessionPending(id)
      permissionService.removeSessionDeferred(id)
  ...
  // 切换 Agent 会话待继续状态
  ipcMain.handle(
    AGENT_IPC_CHANNELS.TOGGLE_FOLLOW_UP,
    async (_, id: string): Promise<AgentSessionMeta> => {
      const sessions = listAgentSessions()
      const current = sessions.find((s) => s.id === id)
      if (!current) throw new Error(`Agent session not found: ${id}`)
      return updateAgentSessionMeta(id, { needsFollowUp: !current.needsFollowUp })
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: ForkSessionInput): Promise<AgentSessionMeta> => {
      if (isAgentSessionActive(input.sessionId)) {
        throw Object.assign(new Error('源会话正在运行，请停止后再 Fork 当前 Worktree'), {
          code: 'operation_not_allowed',
        })
      }
  ...
  ipcMain.handle(
    AGENT_IPC_CHANNELS.PREVIEW_REWIND_SESSION,
    async (_, input: RewindSessionInput): Promise<RewindSessionPreview> => {
      return previewAgentSessionRewind(input.sessionId, input.assistantMessageUuid)
    },
  )
  ...
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_REWIND_UNDO_STATE,
    async (_, input: RewindUndoInput): Promise<RewindUndoState> => getAgentRewindUndoState(input.sessionId),
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.UNDO_REWIND_SESSION,
    async (_, input: RewindUndoInput): Promise<UndoRewindSessionResult> => undoAgentSessionRewind(input.sessionId),
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_SESSION_TREE,
    async (_, sessionId: string): Promise<SessionTreeResult> => getAgentSessionTree(sessionId),
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.NAVIGATE_SESSION_TREE,
    async (_, input: NavigateSessionTreeInput): Promise<NavigateSessionTreeResult> => (
      navigateAgentSessionTree(input.sessionId, input.entryId)
    ),
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@domi/shared').CreateAgentWorkspaceInput): Promise<AgentWorkspace> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@domi/shared').CreateAgentWorkspaceInput, channelId?: string, modelId?: string): Promise<import('@domi/shared').CreateAgentProjectResult> => {
      const workspace = createAgentWorkspace(input)
      if (workspace.projectRootPath) watchAttachedDirectory(workspace.projectRootPath)
  ...
  ipcMain.handle(
    ...
    async (_, id: string, projectRootPath: string): Promise<AgentWorkspace> => {
      ...
      const sameRoot = previousRoot
        ? normalizePathForCompare(resolve(previousRoot)) === normalizePathForCompare(resolve(projectRootPath))
        : false
      if (!sameRoot) await assertAgentWorkspaceSessionLifecycle(id, 'move')
  ...
  ipcMain.handle(
    ...
    async (_, id: string): Promise<void> => {
      ...
      const activeSessionId = affectedSessionIds.find((sessionId) => isAgentSessionActive(sessionId))
      if (activeSessionId) {
        throw Object.assign(new Error('项目仍有运行中或等待后台任务的 Agent，请先停止后再删除'), {
          code: 'operation_not_allowed',
        })
      }
      if (affectedSessionIds.some((sessionId) => terminal?.service.hasRunningOwner(sessionId))) {
        throw Object.assign(new Error('工作区仍有终端在运行，请先停止后再删除'), { code: 'operation_not_allowed' })
      }
      // 所有 Checkout 与 session 必须先通过无副作用预检；任何 owner 未 Discard 时不删除 Bridge 或会话。
      await assertAgentWorkspaceSessionLifecycle(id, 'delete')
      for (const sessionId of affectedSessionIds) {
        await deleteAgentSession(sessionId)
        await browser?.service.closeOwner(sessionId)
        await terminal?.service.closeOwner(sessionId)
      }
      ...
      // 机器人默认项目与桌面端当前项目已分离；删除其默认项目时在主进程
      // 立即切到仍存在的默认/首个项目，避免设置页和远程新会话保留悬空 ID。
      const settings = getSettings()
      if (settings.agentRemoteDefaultWorkspaceId === id) {
        const remainingWorkspaces = listAgentWorkspaces()
        const fallbackWorkspace = remainingWorkspaces.find((workspace) => workspace.slug === 'default')
          ?? remainingWorkspaces[0]
        updateSettings({ agentRemoteDefaultWorkspaceId: fallbackWorkspace?.id })
      }
  ...
  // 获取外部用户级 Skills / Pi MCP 检测摘要
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_GLOBAL_CAPABILITIES,
    async (): Promise<GlobalAgentCapabilities> => getGlobalAgentCapabilities(),
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.SET_GLOBAL_SKILLS_ENABLED,
    async (event, enabled: boolean): Promise<GlobalAgentCapabilities> => {
      updateSettings({ externalGlobalSkillsEnabled: enabled === true })
      event.sender.send(AGENT_IPC_CHANNELS.CAPABILITIES_CHANGED)
      return getGlobalAgentCapabilities()
    },
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.SET_PI_GLOBAL_MCP_ENABLED,
    async (event, enabled: boolean): Promise<GlobalAgentCapabilities> => {
      updateSettings({ piGlobalMcpEnabled: enabled === true })
      event.sender.send(AGENT_IPC_CHANNELS.CAPABILITIES_CHANGED)
      return getGlobalAgentCapabilities()
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, name: string, entry: import('@domi/shared').McpServerEntry): Promise<{ success: boolean; message: string }> => {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, id: string, enabled: boolean): Promise<WorkspaceCapabilities> => {
      updateLinkedBuiltinMcpState(id, enabled)
      return getWorkspaceCapabilities(workspaceSlug)
    }
  )
  ...
  // 获取会话级 Skill 触发明细
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_SESSION_SKILL_TRIGGERS,
    async (_, sessionId: string): Promise<SkillTriggerEvent[]> => {
      return readSessionSkillTriggers(getAgentSessionSkillTriggersPath(sessionId))
    }
  )

  // 获取工作区级 Skill 使用统计
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_WORKSPACE_SKILL_USAGE,
    async (_, workspaceSlug: string): Promise<SkillUsageStats[]> => {
      return readWorkspaceSkillUsage(getWorkspaceSkillUsagePath(workspaceSlug))
    }
  )
  ...
  // 获取默认 Skills 的 slug 列表（来自 ~/.domi/default-skills/）
  ipcMain.handle(
  ...
  // 历史 channel 保留，统一代理到 AGENTS.md 主文件。
  ipcMain.handle(
  ...
  ipcMain.handle(
    ...
    async (event, input: AgentSendInput): Promise<void> => {
      ...
      try {
        ...
        if (consumeCancelledAgentSessionStart(input.sessionId)) {
          throw new Error('会话启动期间已被用户停止')
        }
        // runAgent/orchestrator 在首次异步让出前同步抢占 run generation；随后即可释放启动槽。
        const runPromise = runAgent({ ...input, triggeredBy: 'user' }, event.sender)
        releaseStart()
        await runPromise
      } finally {
        releaseStart()
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, source?: unknown): Promise<void> => {
      feishuBridgeManager.stopSessionMirrorRun(sessionId)
      stopAgent(sessionId, normalizeAgentStopSource(source ?? 'renderer-stop-control'))
    }
  )

  // 跳过自动重试等待，立即执行当前已经安排的恢复。
  ipcMain.handle(
    AGENT_IPC_CHANNELS.RETRY_NOW,
    async (_, sessionId: string): Promise<boolean> => retryAgentNow(sessionId),
  )

  // renderer 当前可见 Agent 上下文：对话 Tab 用于流式调度，对话/预览 Tab 用于通知抑制；不修改会话状态。
  ipcMain.handle(
    AGENT_IPC_CHANNELS.SET_VISIBLE_SESSION,
    async (event, sessionId: string | null, notificationPresenceSessionId?: string | null): Promise<void> => {
      setAgentStreamForegroundSession(
        event.sender,
        typeof sessionId === 'string' ? sessionId : null,
        notificationPresenceSessionId === undefined
          ? undefined
          : typeof notificationPresenceSessionId === 'string' ? notificationPresenceSessionId : null,
      )
    }
  )
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@domi/shared').AgentQueueMessageInput): Promise<string> => {
      return queueAgentMessage(input, event.sender)
    }
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.SUBMIT_OR_ENQUEUE_MESSAGE,
    async (event, input: import('@domi/shared').AgentSubmitOrEnqueueInput): Promise<import('@domi/shared').AgentSubmitOrEnqueueResult> => {
      return submitOrEnqueueAgentMessage(input, event.sender)
    },
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.CANCEL_QUEUED_MESSAGE,
    async (_event, input: import('@domi/shared').AgentQueuedMessageControlInput): Promise<boolean> => {
      return cancelDeferredAgentMessage(input)
    },
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.MOVE_QUEUED_MESSAGE,
    async (_event, input: import('@domi/shared').AgentMoveQueuedMessageInput): Promise<boolean> => {
      return moveDeferredAgentMessage(input)
    },
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.REPLACE_MESSAGE_QUEUE,
    async (_event, input: import('@domi/shared').AgentReplaceMessageQueueInput): Promise<import('@domi/shared').AgentReplaceMessageQueueResult> => {
      return replaceAgentMessageQueue(input)
    }
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.CLEAR_MESSAGE_QUEUE,
    async (_event, input: import('@domi/shared').AgentClearMessageQueueInput): Promise<import('@domi/shared').AgentClearMessageQueueResult> => {
      return clearAgentMessageQueue(input)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, response: PermissionResponse): Promise<import('@domi/shared').PermissionResponseResult> => {
      const { requestId, behavior, alwaysAllow, updatedInput } = response
      const result = await permissionService.respondToPermission(requestId, behavior, alwaysAllow, updatedInput)
      if (result.sessionId && result.consumed !== false) broadcastWorkActivityChanged()

      // 发送 permission_resolved 事件给渲染进程；snapshot 失效等确定性失败会消费旧卡，
      // 只有短暂 busy 的 Local 维修确认保留原卡，等待当前 Agent 结束后重试。
      if (result.sessionId && result.consumed !== false) {
        notifyAgentPermissionResolved(result.sessionId, requestId, behavior)
      }
      return result
    }
  )
  ...
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_SESSION_CAPABILITY_GRANTS,
    (_, sessionId: string): import('@domi/shared').GitPushSessionTrustView[] => {
      if (!getAgentSessionMeta(sessionId)) throw new Error(`Agent 会话不存在: ${sessionId}`)
      return permissionService.listSessionCapabilityGrants(sessionId)
    },
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.REVOKE_SESSION_CAPABILITY_GRANT,
    (_, sessionId: string, grantId: string): import('@domi/shared').GitPushSessionTrustView[] => {
      if (!getAgentSessionMeta(sessionId)) throw new Error(`Agent 会话不存在: ${sessionId}`)
      if (typeof grantId !== 'string' || !grantId) throw new Error('会话授权 ID 不能为空')
      permissionService.revokeSessionCapabilityGrant(sessionId, grantId)
      return permissionService.listSessionCapabilityGrants(sessionId)
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, mode: DomiPermissionMode): Promise<void> => {
      if (!isDomiPermissionMode(mode)) {
  ...
  // Pi 新路径：Execution Policy 与 Workflow 独立验证、持久化并热更新。
  ipcMain.handle(
    AGENT_IPC_CHANNELS.UPDATE_SESSION_EXECUTION_CONTROLS,
    async (_, sessionId: string, controls: AgentExecutionControlsUpdate): Promise<AgentSessionMeta> => {
      return updateSessionExecutionControls(sessionId, controls)
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string, modelId: string) => {
      ...
      const channelModel = channel.models.find((model) => model.id === modelId)
      return resolvePiReasoningCapability(channel.provider, modelId, channelModel)
    }
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_PI_MODEL_CATALOG_STATUS,
    async (_, channelId: string, modelId: string) => {
      if (!channelId || !modelId) return 'missing'
      const channel = getChannelById(channelId)
      if (!channel) return 'missing'
      return resolvePiModelCatalogStatus(channel.provider, modelId)
    }
  )

  ipcMain.handle(
    AGENT_IPC_CHANNELS.LIST_VISION_RELAY_MODELS,
    async () => collectVisionRelayModelOptions(listChannels(), resolvePiImageInputCapability),
  )
  ...
  ipcMain.handle(
    AGENT_IPC_CHANNELS.UPDATE_SESSION_MODEL_PRESENTATION_PRESET,
    async (_, sessionId: string, preset: ModelPresentationPreset): Promise<AgentSessionMeta> => {
      const validPresets: ModelPresentationPreset[] = ['standard', 'minimal']
      if (!validPresets.includes(preset)) {
        throw new Error(`无效的模型呈现预设: ${String(preset)}`)
      }
      if (!getAgentSessionMeta(sessionId)) {
        throw new Error(`Agent 会话不存在: ${sessionId}`)
      }
      // 当前运行已在启动时读取预设；此处只更新会话的下一轮配置。
      return updateAgentSessionMeta(sessionId, { modelPresentationPreset: preset })
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, toolId: string, state: ChatToolState): Promise<void> => {
      updateLinkedChatToolState(toolId, state)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, toolId: string): Promise<{ success: boolean; message: string }> => {
      ...
      // GPT Image 生图工具测试（验证凭据但不消耗生图额度）
      if (toolId === 'gpt-image') {
        const { getToolCredentials: getCredentials } = await import('./lib/chat-tool-config')
        const credentials = getCredentials('gpt-image')
        if (!credentials.apiKey) {
          return { success: false, message: '请先填写 API Key' }
        }
        try {
          const baseUrl = credentials.baseUrl?.trim() || 'https://api.openai.com/v1'
          const response = await fetch(`${baseUrl.replace(/\/+$/, '')}/models`, {
            headers: { Authorization: `Bearer ${credentials.apiKey}` },
          })
          if (response.status === 404) {
            // 部分 OpenAI 兼容端点不提供 /models 列表，但服务可达
            return { success: true, message: '端点可达（不支持 /models 列表，无法进一步验证模型）' }
          }
          if (!response.ok) {
            const errorText = await response.text()
            return { success: false, message: `API 请求失败 (${response.status}): ${errorText.slice(0, 200)}` }
          }
          return { success: true, message: '连接成功，API Key 可用' }
        } catch (error) {
          const msg = error instanceof Error ? error.message : String(error)
          return { success: false, message: `连接失败: ${msg}` }
        }
  ...
  // 列出会话的生成图片
  ipcMain.handle(
    GALLERY_IPC_CHANNELS.LIST_GENERATED_IMAGES,
    async (_, request: GeneratedImagesRequest): Promise<GeneratedImageItem[]> => {
      const { listGeneratedImages } = await import('./lib/gallery-service')
      return listGeneratedImages(request)
    }
  )

  // 读取当前会话画廊中的图片；服务层会重新枚举并校验路径归属
  ipcMain.handle(
    GALLERY_IPC_CHANNELS.READ_GENERATED_IMAGE,
    async (_, request: GeneratedImagesRequest, localPath: string): Promise<string> => {
      const { readGeneratedImageAsBase64 } = await import('./lib/gallery-service')
      return readGeneratedImageAsBase64(request, localPath)
    }
  )

  // 另存当前会话画廊中的图片；读取前同样校验会话归属
  ipcMain.handle(
    GALLERY_IPC_CHANNELS.SAVE_GENERATED_IMAGE_AS,
    async (event, request: GeneratedImagesRequest, localPath: string, defaultFilename: string): Promise<boolean> => {
      const { readGeneratedImageAsBase64 } = await import('./lib/gallery-service')
      const { basename: pathBasename, extname: pathExtname } = await import('node:path')
      // 先校验会话归属，再触发原生对话框；默认文件名仅取 basename。
      const base64 = readGeneratedImageAsBase64(request, localPath)
      const safeFilename = pathBasename(defaultFilename) || 'generated-image.png'
      const win = BrowserWindow.fromWebContents(event.sender)
      ...
      const result = await dialog.showSaveDialog(win ?? BrowserWindow.getFocusedWindow()!, {
        ...
        filters: [
          { name: `${filterMap[ext] ?? 'Image'} 图片`, extensions: [ext || 'png'] },
          { name: '所有文件', extensions: ['*'] },
        ],
      })
      if (result.canceled || !result.filePath) return false
  ...
  ipcMain.handle(
    ...
    async (_, response: AskUserResponse): Promise<void> => {
      ...
      // 粘贴图片先落盘，再把绝对路径追加进对应答案文本，Agent 通过 Read 查看
      const answers = await mergeAskUserAttachments(response)
      respondAgentAskUser(requestId, answers)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, response: ExitPlanModeResponse): Promise<void> => {
      // 反馈附图先落盘，把绝对路径追加进反馈文本后再交给审批服务
      const normalized = await mergeExitPlanFeedbackAttachments(response)
      respondAgentExitPlan(normalized)

      // 审批只负责解除对应 tool call。临时授权与明确的持久切换都由
      // orchestrator 在校验精确 run token 后生效；IPC 不得越过该校验写 session meta。
    }
  )
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@domi/shared').PendingRequestsSnapshot> => {
      return {
  ...
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_WORK_ACTIVITY,
    async (): Promise<import('@domi/shared').WorkActivityProjection> => {
      try {
        return await getWorkActivityProjection()
      } catch (error) {
        console.error('[Work Activity] 宿主投影失败:', error)
        throw new Error(describeWorkActivityHostFailure(error))
      }
    },
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.MARK_WORK_ACTIVITY_VIEWED,
    async (_, rootSessionId: string): Promise<void> => markWorkActivityViewed(rootSessionId),
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.ACKNOWLEDGE_WORK_ACTIVITY_OUTCOME,
    async (_, rootSessionId: string): Promise<void> => acknowledgeWorkActivityOutcome(rootSessionId),
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.REMOVE_WORK_ACTIVITY_COMPLETED,
    async (_, rootSessionId: string): Promise<void> => removeWorkActivityCompleted(rootSessionId),
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.REMOVE_ALL_WORK_ACTIVITY_COMPLETED,
    async (): Promise<number> => removeAllWorkActivityCompleted(),
  )
  ipcMain.handle(
    AGENT_IPC_CHANNELS.STOP_WORK_ACTIVITY_SESSION,
    async (_, rootSessionId: string): Promise<import('@domi/shared').WorkActivityStopResult> => stopWorkActivitySession(rootSessionId),
  )
  ...
  // 打开 Composer 文件或文件夹选择器
  ipcMain.handle(
    AGENT_IPC_CHANNELS.OPEN_FILE_OR_FOLDER_DIALOG,
    async (_, sessionId: string, kind: ComposerAttachmentKind): Promise<FileOrFolderDialogResult> => {
      if (!getAgentSessionMeta(sessionId)) throw new Error(`会话不存在: ${sessionId}`)
      const result = await openFileOrFolderDialog(kind)
      visionDirectoryGrants.register(sessionId, result.directories.map((directory) => directory.path))
      return result
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: AgentAttachDirectoryInput): Promise<string[]> => {
      ...
      const safePath = realpathSync(resolve(input.directoryPath))
      if (!statSync(safePath).isDirectory()) throw new Error('只能附加目录')
      const existing = meta.attachedDirectories ?? []
      const updated = existing.includes(safePath) ? existing : [...existing, safePath]
      const trustedVisionPath = visionDirectoryGrants.consume(input.sessionId, safePath)
      const visionDirectories = meta.visionRelayAttachedDirectories ?? []
      const updatedVisionDirectories = trustedVisionPath && !visionDirectories.includes(trustedVisionPath)
        ? [...visionDirectories, trustedVisionPath]
        : visionDirectories
      updateAgentSessionMeta(input.sessionId, {
        attachedDirectories: updated,
        visionRelayAttachedDirectories: updatedVisionDirectories,
      })
      // 启动附加目录文件监听
      watchAttachedDirectory(safePath)
      return updated
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: AgentAttachDirectoryInput): Promise<string[]> => {
      ...
      const safePath = (() => {
        try { return realpathSync(resolve(input.directoryPath)) } catch { return resolve(input.directoryPath) }
      })()
      const existing = meta.attachedDirectories ?? []
      const updated = existing.filter((d) => d !== safePath && d !== input.directoryPath)
      const updatedVisionDirectories = (meta.visionRelayAttachedDirectories ?? [])
        .filter((d) => d !== safePath && d !== input.directoryPath)
      updateAgentSessionMeta(input.sessionId, {
        attachedDirectories: updated,
        visionRelayAttachedDirectories: updatedVisionDirectories,
      })
      releaseDirectoryWatcherIfUnreferenced(safePath)
      return updated
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, repo: import('@domi/shared').WorkspaceWorktreeRepo) => {
  ...
  ipcMain.handle(
    ...
    async (_, dirPath: string, access?: FileAccessOptions): Promise<FileEntry[]> => {
      ...
      const usesSessionTargetPath = sessionTargetAccess.usesSessionTargetPathSpace(options)
      let targetRoot: string | null = null
      let safePath: string
      if (usesSessionTargetPath) {
        // 一次请求只解析一次 checkout lease；逐条重复解析会让大目录在 Windows 上等待数十秒。
        const resolvedDirectory = await sessionTargetAccess.resolveRelativeDirectory(options.sessionId, dirPath)
        if (!resolvedDirectory) throw new Error('Session Target 目录不存在或超出授权范围')
        targetRoot = resolvedDirectory.rootPath
        safePath = resolvedDirectory.directoryPath
      } else {
        safePath = resolve(dirPath)
      }
      // 通用绝对路径目录可能已被删除（如删除 Agent 会话后面板仍持有旧路径），优雅返回空列表。
      if (!existsSync(safePath)) return []
      if (!usesSessionTargetPath && !(await isPathAllowedForSession(safePath, options, sessionTargetAccess))) {
      ...
      for (const item of items) {
        ...
        const projectedPath = usesSessionTargetPath
          ? sessionTargetAccess.projectRelativePathFromRoot(targetRoot!, fullPath)
          : fullPath
  ...
  // 聚合当前会话关联的项目产物：当前目标改动、文件检查点与历次 Worktree 交付记录。
  ipcMain.handle(
    AGENT_IPC_CHANNELS.LIST_SESSION_PROJECT_ARTIFACTS,
    async (_, sessionId: string): Promise<SessionProjectArtifact[]> => {
      if (!sessionTargetAccess.isActivePiSession({ sessionId })) return []
      return sessionProjectArtifacts.list(sessionId)
    },
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions): Promise<void> => {
      ...
      if (options.pathSpace === 'session-local-project') {
        throw new Error('Session Target 绑定前，Local 项目仅支持浏览')
      }
      const usesSessionTargetPath = sessionTargetAccess.usesSessionTargetPathSpace(options)
      if (usesSessionTargetPath && (!filePath || filePath === '.')) {
        throw new Error('不能删除 Session Target 根目录')
      }
      const safePath = usesSessionTargetPath
        ? await sessionTargetAccess.resolveRelative(options.sessionId, filePath)
        : resolve(filePath)
      if (!safePath || !options || !(await isPathAllowedForSession(safePath, options, sessionTargetAccess))) {
        throw new Error('访问路径超出当前会话的授权范围')
      }
      if (options.pathSpace === 'session-workbench'
        && sessionTargetAccess.isSessionWorkbenchRoot(options.sessionId, safePath)) {
        throw new Error('不能删除会话 workbench 根目录')
      }
  ...
  ipcMain.handle(
    ...
    async (_, filename: string, content: string): Promise<string> => {
      ...
      const tmpDir = join(tmpdir(), PREVIEW_TEMP_DIR_NAME)
  ...
  ipcMain.handle(
    ...
    async (_, folderPath: string, access?: FileAccessOptions): Promise<void> => {
      ...
      const allowed = options
        ? await isPathAllowedForSession(folderPath, options, sessionTargetAccess)
        : isPathAllowed(folderPath)
      if (!allowed) throw new Error('访问路径超出当前会话的授权范围')
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<{ resolvedPath: string; content: string } | null> => {
      const { resolveAndReadFile } = await import('./lib/file-preview-service.ts')
      const options = normalizeFileAccessOptions(access)
      const activePi = sessionTargetAccess.usesSessionTargetPathSpace(options)
      const resolved = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!resolved) return null
      const result = resolveAndReadFile(resolved)
      return result && activePi ? { ...result, resolvedPath: filePath } : result
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, content: string, access?: FileAccessOptions | string[]): Promise<boolean> => {
      ...
      const { writeFileSync } = await import('node:fs')
      const { resolveTargetPath } = await import('./lib/file-preview-service.ts')
      const options = normalizeFileAccessOptions(access)
      if (options?.pathSpace === 'session-local-project') return false
      const resolved = sessionTargetAccess.usesSessionTargetPathSpace(options)
        ? await sessionTargetAccess.resolveRelative(options.sessionId, filePath, true)
        : resolveTargetPath(filePath, getPreviewCandidateBasePaths(options))
      if (!resolved || !(await isPathAllowedForSession(resolved, options, sessionTargetAccess, true))) {
        console.warn('[IPC] file:write-text 拒绝越界路径:', resolved)
        return false
      }
  ...
  // 解析文件或目录路径。文件返回不暴露真实路径的预览 URL；目录由 renderer 请求系统打开。
  ipcMain.handle(
    'file:resolve-path',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<ResolvedPreviewPath | null> => {
      const options = normalizeFileAccessOptions(access)
      const result = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!result) return null
      return createResolvedPreviewPath(result, registerDomiFilePath)
    }
  )

  // 解析 HTML 预览路径并授权加载同目录相对资源；Renderer 只接收 opaque token URL。
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<ResolvedPreviewPath | null> => {
      ...
      const result = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!result) return null
      return createResolvedHtmlPreviewPath(result, registerDomiDirectoryPath)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<{ tmpHtmlUrl: string } | null> => {
      const { preparePdfPreview } = await import('./lib/file-preview-service.ts')
      const options = normalizeFileAccessOptions(access)
      const resolved = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!resolved) return null
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<{ resolvedPath: string; html: string } | null> => {
      const { convertDocxToHtml } = await import('./lib/file-preview-service.ts')
      const options = normalizeFileAccessOptions(access)
      const activePi = sessionTargetAccess.usesSessionTargetPathSpace(options)
      const resolved = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!resolved) return null
  ...
  ipcMain.handle(
    'file:office-to-html',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@domi/shared').OfficePreviewResult | null> => {
      const { convertOfficeToHtml } = await import('./lib/file-preview-service.ts')
      const options = normalizeFileAccessOptions(access)
      const activePi = sessionTargetAccess.usesSessionTargetPathSpace(options)
      const resolved = await resolvePreviewReadPath(filePath, options, sessionTargetAccess)
      if (!resolved) return null
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, newName: string, access?: FileAccessOptions): Promise<void> => {
      ...
      if (!newName.trim() || newName === '.' || newName.includes('/') || newName.includes('\\') || newName.includes('..') || newName.includes(sep)) {
        throw new Error('文件名不能为空、"."，也不能包含路径分隔符或 ".."')
      }
      ...
      if (options.pathSpace === 'session-workbench'
        && sessionTargetAccess.isSessionWorkbenchRoot(options.sessionId, safePath)) {
        throw new Error('不能重命名会话 workbench 根目录')
      }
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, targetDir: string, access?: FileAccessOptions): Promise<void> => {
      ...
      if (!options
        || !(await isPathAllowedForSession(safePath, options, sessionTargetAccess))
        || !(await isPathAllowedForSession(safeTarget, options, sessionTargetAccess))) {
        throw new Error('访问路径超出当前会话的授权范围')
      }
      if (options.pathSpace === 'session-workbench'
        && sessionTargetAccess.isSessionWorkbenchRoot(options.sessionId, safePath)) {
        throw new Error('不能移动会话 workbench 根目录')
      }
  ...
  ipcMain.handle(
    ...
    async (_, dirPath: string, access?: FileAccessOptions | string[]): Promise<FileEntry[]> => {
      ...
      const options = normalizeSessionFileAccessOptions(access)
      if (!options || !(await isPathAllowedForSession(safePath, options, sessionTargetAccess))) {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, sessionId?: string, workspaceSlug?: string): Promise<string> => {
      if (!filePath || typeof filePath !== 'string' || typeof sessionId !== 'string' || !sessionId) {
        throw new Error('附加文件 IPC 缺少 Session 上下文或有效文件路径')
      }
      ...
      const access = normalizeFileAccessOptions({ sessionId, workspaceSlug })
      if (sessionTargetAccess.isActivePiSession(access)) {
        if (!(await sessionTargetAccess.authorizeSessionFileRequest(safePath, access))) {
          throw new Error('访问路径不在当前 Session Target 授权范围内')
        }
      } else {
        // 通用附加文件功能保留既有授权根；Session Target 不接受 renderer 替换 workspaceSlug。
        ...
        if (!isAllowed) throw new Error('访问路径不在允许范围内')
      }
  ...
  ipcMain.handle(
    ...
    async (_, rootPath: unknown, query: string, limit = 20, additionalPaths?: string[], sessionPaths?: string[], sessionId?: string, pathSpace?: FileAccessOptions['pathSpace']): Promise<FileSearchResult> => {
      if (typeof sessionId !== 'string' || !sessionId) {
        console.warn('[IPC] Agent 文件搜索缺少 Session 上下文')
        return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
      }
      const { readdirSync, statSync } = await import('node:fs')
      const { resolve, relative, basename } = await import('node:path')

      let safeRoot: string
      const access = normalizeSessionFileAccessOptions({ sessionId, pathSpace })
      if (!access) return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
      if (sessionTargetAccess.usesSessionTargetPathSpace(access)) {
        const target = await sessionTargetAccess.resolveActiveSearchTarget(access.sessionId, rootPath)
        if (!target) {
          console.warn('[IPC] 拒绝 renderer 为 active 搜索提交本地 root')
          return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
        }
        safeRoot = target.root
        additionalPaths = target.attachedRoots
        sessionPaths = undefined
      } else if (access.pathSpace === 'session-workbench') {
        const workbenchRoot = sessionTargetAccess.resolveSessionWorkbenchRoot(access.sessionId, rootPath)
        if (!workbenchRoot) {
          console.warn('[IPC] 拒绝 renderer 搜索非当前会话 workbench 路径')
          return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
        }
        safeRoot = workbenchRoot
        additionalPaths = undefined
      } else if (access.pathSpace === 'session-local-project') {
        const localProjectRoot = await sessionTargetAccess.resolveSessionLocalProjectRoot(access.sessionId, rootPath)
        if (!localProjectRoot) {
          console.warn('[IPC] 拒绝 renderer 搜索非未绑定会话的 Local 项目根')
          return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
        }
        safeRoot = localProjectRoot
        additionalPaths = undefined
        sessionPaths = undefined
      } else if (sessionTargetAccess.isActivePiSession(access)) {
        console.warn('[IPC] Active Pi 搜索缺少有效路径空间')
        return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
      } else {
        if (typeof rootPath !== 'string' || !rootPath) {
          return { entries: [], total: 0, sessionEntries: [], workspaceEntries: [] }
        }
        safeRoot = resolve(rootPath)
      }
  ...
  // 启用或停用单条 Work 附加提示词
  ipcMain.handle(
    SYSTEM_PROMPT_IPC_CHANNELS.UPDATE_WORK_ACTIVATION,
    async (_, id: string, enabled: boolean): Promise<void> => {
      return updateWorkPromptActivation(id, enabled)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, id: string | null, scope?: SystemPromptScope): Promise<void> => {
      return setDefaultPrompt(id, scope)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@domi/shared').FeishuBotConfigInput) => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@domi/shared').DingTalkBotConfigInput) => {
  ...
  ipcMain.handle(USAGE_IPC_CHANNELS.QUERY, async (_, options?: UsageQueryOptions) => {
    return queryUsage(options)
  })
  ...
  ipcMain.handle('migration:cancelImport', async (_, tempDir: string) => {
    if (tempDir && existsSync(tempDir) && tempDir.includes('domi-import-')) {
  ...
  ipcMain.handle(
    ...
    async (event): Promise<void> => {
      ...
      const sourceWindow = BrowserWindow.fromWebContents(event.sender)
      toggleVoiceDictationWindow({ targetIsDomi: !!sourceWindow })
    }
  )
  ...
  ipcMain.handle('migration:export', async (_, options: unknown) => {
    assertMigrationExportRequest(options)
    const { exportData } = await import('./lib/migration-service')
    return exportData(options as unknown as Parameters<typeof exportData>[0])
  })

  ipcMain.handle('migration:exportV2', async (_, options: unknown) => {
    assertMigrationExportRequest(options)
    const { exportDataV2 } = await import('./lib/migration-service')
    return exportDataV2(options as unknown as Parameters<typeof exportDataV2>[0])
  })

  ipcMain.handle('migration:parseImportFile', async (_, filePath: string) => {
    if (!isSupportedMigrationImportFile(filePath)) {
      throw new Error('仅支持 Domi 迁移文件或兼容的旧版迁移文件')
    }
  ...
  ipcMain.handle('migration:openFileDialog', async () => {
    ...
    const result = await dialog.showOpenDialog({
      title: '选择迁移文件',
      filters: [
        { name: 'Domi 迁移文件', extensions: [...MIGRATION_IMPORT_EXTENSIONS] },
      ],
      properties: ['openFile'],
    })
  ...
  ipcMain.handle('migration:saveFileDialog', async (_, mode: MigrationFileMode) => {
    assertMigrationFileMode(mode)

    const { dialog } = await import('electron')
    const ext = getMigrationExportExtension(mode)
    const defaultName = `domi-migration-${new Date().toISOString().slice(0, 10)}.${ext}`
    const result = await dialog.showSaveDialog({
      title: '保存迁移文件',
      defaultPath: defaultName,
      filters: [
        { name: mode === 'personal' ? 'Domi 个人备份' : 'Domi 分享包', extensions: [ext] },
      ],
    })
    return result.canceled ? null : result.filePath
  })
  ...
  // 否则 NaN / -Infinity / 越界值会污染 ~/.domi/automations.json，无法回滚。
