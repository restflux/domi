 ...
 * Bun 运行时路径检测模块
 *
 * Bun 是 Domi 的可选组件；打包后的 Pi Agent Runtime 不依赖用户全局安装 Bun。
 * 仅用于：
...
import type { BunRuntimeStatus, PlatformArch } from '@domi/shared'
 ...
 * 检测并返回 Bun 运行时状态
 *
 * Bun 是可选组件，Domi 核心 Agent 运行不依赖系统 Bun。这里的检测结果只用于：
 * - 系统运行时状态卡片展示
...
export async function detectBunRuntime(): Promise<BunRuntimeStatus> {
  ...
  console.log('[Bun 检测] 未找到 Bun（可选，不影响 Domi 核心功能）')
  return {
    available: false,
    path: null,
    version: null,
