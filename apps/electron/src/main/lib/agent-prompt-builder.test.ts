...
describe('Session Target 提示词', () => {
  ...
  test('Given direct owner Isolated Target When 构建提示词 Then 仅在存在可交付变更时指引 Agent 准备验收卡', () => {
    ...
    expect(prompt).toContain('只生成带有“预览修改”入口的验收卡，不会修改 Local')
    expect(prompt).toContain('检查通过后再点“确认保存”')
    expect(prompt).toContain('可通过“撤回预览”撤回，尚未最终保存')
    expect(prompt).toContain('若宿主已显示正在创建预览，不要要求用户重复点击')
    expect(prompt).not.toContain('同步到 Local 验收')
  ...
  test('Given delivered follow-up When 新请求需要修改 Then 使用结构化工具并在确认后自动续跑', () => {
    ...
    expect(prompt).toContain('宿主会把 `details` 确定性渲染为会话正文')
    expect(prompt).toContain('确认卡只显示简短摘要')
    expect(prompt).toContain('当前项目受交付保护')
    expect(prompt).toContain('`RequestDirectWorkflow` 申请本次执行')
    expect(prompt).not.toContain('强制 Read Only follow-up')
    expect(prompt).toContain('不得写绝对本地路径')
