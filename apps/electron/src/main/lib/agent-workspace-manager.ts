 ...
 * - 工作区索引：~/.domi/agent-workspaces.json（轻量元数据）
 * - 工作区目录：~/.domi/agent-workspaces/{slug}/（Agent 的 cwd）
 */

import { readFileSync, writeFileSync, existsSync, readdirSync, cpSync, copyFileSync, mkdirSync, statSync, lstatSync, openSync, readSync, closeSync, realpathSync, accessSync, constants } from 'node:fs'
...
import {
  ...
  defaultSkillCopyFilter,
  isUnmodifiedDefaultSkill,
  readDefaultSkillsManifest,
  readWorkspaceDefaultSkillsState,
  replaceManagedSkillDirectory,
  writeWorkspaceDefaultSkillsState,
...
import { inferMcpTransportType, normalizeMcpTransportType } from '@domi/shared'
import type { AgentWorkspace, CreateAgentWorkspaceInput, LocalProjectRootStatus, WorkspaceMcpConfig, SkillMeta, SkillImportSource, OtherWorkspaceSkillsGroup, WorkspaceCapabilities, SkillFileNode, SkillFileContent, WorkspaceMemorySummary, WorkspaceInstructionMigrationStatus } from '@domi/shared'
 ...
 * 使用 Domi 托管的 workspace-files/，以保持历史项目完全兼容。
 */
...
/**
 * 按工作区 ID 解析可由系统文件管理器打开的项目文件夹。
 * Renderer 只提交不透明 ID，物理路径解析与存在性检查留在主进程。
 */
export function resolveAgentWorkspaceProjectFolder(id: string): string {
  const workspace = getAgentWorkspace(id)
  if (!workspace) throw new Error('项目不存在或已被删除')

  const projectPath = getProjectFilesPath(workspace.slug)
  if (!existsSync(projectPath)) throw new Error('项目文件夹不存在，请重新选择或恢复项目文件夹')
  if (!statSync(projectPath).isDirectory()) throw new Error('项目路径不是文件夹，请重新选择项目文件夹')
  return projectPath
}

/** 将 ~/.domi/default-skills/ 的内容逐个复制到工作区，并记录 Domi 来源与 seen 状态。 */
function copyDefaultSkills(workspaceSlug: string, options: { throwOnError?: boolean } = {}): void {
  const defaultDir = getDefaultSkillsDir()
  const managedBaselineDir = getDefaultSkillsBaselineDir(defaultDir)
  const sourceDir = existsSync(managedBaselineDir) ? managedBaselineDir : defaultDir
  const targetDir = getWorkspaceSkillsDir(workspaceSlug)
  const workspaceDir = getAgentWorkspacePath(workspaceSlug)
  const manifest = readDefaultSkillsManifest(defaultDir)
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const info = manifest.skills[entry.name]
      const version = info?.version ?? parseSkillVersion(source)
      const baselineHash = info?.currentHash ?? computeSkillDirectoryHash(source)
    ...
    writeWorkspaceDefaultSkillsState(workspaceDir, seenSlugs)
...
export function deleteAgentWorkspace(id: string): void {
  ...
  workspaceInstructionMigrationReports.delete(removed.slug)
  writeIndex(index)
...
function removeRetiredDefaultSkillsFromWorkspace(
  ...
  for (const slug of RETIRED_DEFAULT_SKILL_SLUGS) {
    ...
    for (const skillDir of skillDirs) {
      ...
      if (!isUnmodifiedDefaultSkill(targetPath, knownHashes)) {
        console.warn(`[Agent 工作区] 退役 Skill 已定制或来源不明，保留: ${workspace.slug}/${slug} (${skillDir.state})`)
        continue
      }
      try {
...
/**
 * 只升级可证明未修改的 Domi 副本；active/inactive 原地保持。
 * 首次建立 seen-state 时保守记录当前 bundle 全量 slug，不补回缺失项；此后只注入真正新出现的默认 Skill。
 */
export function upgradeDefaultSkillsInWorkspaces(): void {
  ...
  const sourceDir = existsSync(managedBaselineDir) ? managedBaselineDir : defaultDir
  const manifest = readDefaultSkillsManifest(defaultDir)
  const defaultSkills = new Map<string, { version: string, baselineHash: string, knownHashes: string[], sourcePath: string }>()
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const info = manifest.skills[entry.name]
      const baselineHash = info?.currentHash ?? computeSkillDirectoryHash(sourcePath)
      if (!baselineHash) continue
      defaultSkills.set(entry.name, {
        version: info?.version ?? parseSkillVersion(sourcePath),
        baselineHash,
        knownHashes: info?.knownBaselineHashes ?? [baselineHash],
        sourcePath,
      })
    }
  } catch {
  ...
  for (const workspace of index.workspaces) {
    const workspaceDir = getAgentWorkspacePath(workspace.slug)
    const existingState = readWorkspaceDefaultSkillsState(workspaceDir)
    const seenSlugs = new Set(existingState?.seenSlugs ?? defaultSkills.keys())
    removeRetiredDefaultSkillsFromWorkspace(workspace, manifest, seenSlugs)
    ...
    for (const [slug, info] of defaultSkills) {
      ...
      const targetPath = existsSync(activePath) ? activePath : existsSync(inactivePath) ? inactivePath : undefined
      const state = targetPath === inactivePath ? 'inactive' : 'active'

      if (targetPath) {
        seenSlugs.add(slug)
        const currentVer = parseSkillVersion(targetPath)
        if (
          compareSemver(info.version, currentVer) > 0
          && isUnmodifiedDefaultSkill(targetPath, info.knownHashes)
        ) {
          const replaced = replaceManagedSkillDirectory(info.sourcePath, targetPath, {
            slug,
            version: info.version,
            baselineHash: info.baselineHash,
          })
          if (replaced) {
            console.log(`[Agent 工作区] 已升级默认 Skill: ${workspace.slug}/${slug} (${state}, ${currentVer} → ${info.version})`)
          }
        } else if (compareSemver(info.version, currentVer) > 0) {
          console.warn(`[Agent 工作区] 默认 Skill 已定制或来源不明，跳过升级: ${workspace.slug}/${slug} (${state})`)
        }
        continue
      }
      ...
      try {
        copyManagedSkillDirectory(info.sourcePath, activePath, {
          slug,
          version: info.version,
          baselineHash: info.baselineHash,
        })
...
export function ensurePluginManifest(workspaceSlug: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `domi-workspace-${workspaceSlug}`,
    version: '1.0.0',
  }
...
/** Pi 运行时使用的 effective MCP：外部 Pi 全局配置在前，项目配置同名覆盖。 */
export function getEffectiveWorkspaceMcpConfig(workspaceSlug: string): WorkspaceMcpConfig {
  const workspaceConfig = getWorkspaceMcpConfig(workspaceSlug)
  if (!(getSettings().piGlobalMcpEnabled ?? false)) return workspaceConfig
  return mergeEffectiveMcpConfig(readPiGlobalMcpConfig().config, workspaceConfig, true)
}

/** Pi 运行时使用的 effective Skills；inactive 项目 Skill 也会压住同名全局回退。 */
export function getEffectiveWorkspaceSkills(workspaceSlug: string): SkillMeta[] {
  const workspaceSkills = getAllWorkspaceSkills(workspaceSlug)
  if (!(getSettings().externalGlobalSkillsEnabled ?? false)) {
    return workspaceSkills.filter((skill) => skill.enabled)
  }
  return mergeEffectiveSkills(workspaceSkills, scanGlobalSkills().skills, true)
    .filter((skill) => skill.enabled)
}

/** Pi ResourceLoader 的路径顺序必须保持项目优先，后续全局项使用单文件路径。 */
export function getEffectivePiSkillPaths(workspaceSlug: string): string[] {
  const globalSkillFiles = getEffectiveWorkspaceSkills(workspaceSlug)
    .filter((skill) => skill.origin && skill.sourcePath)
    .map((skill) => join(skill.sourcePath!, 'SKILL.md'))
  return [getWorkspaceSkillsDir(workspaceSlug), ...globalSkillFiles]
}

export function getWorkspaceCapabilities(workspaceSlug: string): WorkspaceCapabilities {
  const workspaceMcpConfig = getWorkspaceMcpConfig(workspaceSlug)
  const effectiveMcpConfig = getEffectiveWorkspaceMcpConfig(workspaceSlug)
  const skills = getEffectiveWorkspaceSkills(workspaceSlug)
  const builtinMcpServers = listBuiltinMcpServers({ workspaceSlug })
  const memory = getWorkspaceMemorySummary(workspaceSlug)

  const mcpServers = Object.entries(effectiveMcpConfig.servers ?? {})
    .filter(([name]) => name !== 'memos-cloud')
    .map(([name, entry]) => ({
      name,
      enabled: entry.enabled,
      type: entry.type,
      origin: Object.hasOwn(workspaceMcpConfig.servers, name) ? 'workspace' as const : 'pi-global' as const,
    }))
...
export function deleteWorkspaceSkill(workspaceSlug: string, skillSlug: string): void {
  ...
  const defaultManifest = readDefaultSkillsManifest(getDefaultSkillsDir())
  if (defaultManifest.skills[skillSlug]) {
    const workspaceDir = getAgentWorkspacePath(workspaceSlug)
    const state = readWorkspaceDefaultSkillsState(workspaceDir)
    writeWorkspaceDefaultSkillsState(workspaceDir, [...(state?.seenSlugs ?? []), skillSlug])
  }
...
export interface WorkspaceInstructionMigrationReport {
  workspaceSlug: string
  status: WorkspaceInstructionMigrationStatus
  message?: string
}

const workspaceInstructionMigrationReports = new Map<string, WorkspaceInstructionMigrationReport>()

function instructionFileSummary(absPath: string): WorkspaceMemorySummary['agentsMd'] {
  const st = lstatIfPresent(absPath)
  if (!st) return { exists: false, path: absPath, size: 0 }
  const isSafeFile = st.isFile() && !st.isSymbolicLink()
  return {
    exists: isSafeFile,
    path: absPath,
    size: isSafeFile ? st.size : 0,
...
/** @deprecated 兼容旧调用；Domi 的主指令文件已迁移为 AGENTS.md。 */
export function getWorkspaceClaudeMdPath(workspaceSlug: string): string {
  return getWorkspaceAgentsMdPath(workspaceSlug)
}

export function getLegacyWorkspaceClaudeMdPath(workspaceSlug: string): string {
  return join(getAgentWorkspacePath(workspaceSlug), LEGACY_WORKSPACE_CLAUDE_MD)
}
...
function inspectWorkspaceInstructionMigration(workspaceSlug: string): WorkspaceInstructionMigrationReport {
  const agentsPath = getWorkspaceAgentsMdPath(workspaceSlug)
  const legacyPath = getLegacyWorkspaceClaudeMdPath(workspaceSlug)

  try {
    const agentsStat = lstatIfPresent(agentsPath)
    const legacyStat = lstatIfPresent(legacyPath)
    const agentsExists = Boolean(agentsStat)
    const legacyExists = Boolean(legacyStat)
    if ((agentsStat && (!agentsStat.isFile() || agentsStat.isSymbolicLink()))
      || (legacyStat && (!legacyStat.isFile() || legacyStat.isSymbolicLink()))) {
      return {
        workspaceSlug,
        status: 'blocked',
        message: 'AGENTS.md 或 legacy CLAUDE.md 不是可安全迁移的普通文件，已保留原路径。',
      }
    }
    if ((agentsStat?.size ?? 0) > SKILL_FILE_SIZE_LIMIT || (legacyStat?.size ?? 0) > SKILL_FILE_SIZE_LIMIT) {
      return {
        workspaceSlug,
        status: 'blocked',
        message: 'AGENTS.md 或 legacy CLAUDE.md 超过 10 MB 安全迁移上限，已保留原文件。',
      }
    }
    if (agentsExists && legacyExists) {
      return readFileSync(agentsPath).equals(readFileSync(legacyPath))
        ? { workspaceSlug, status: 'duplicate_removed', message: 'AGENTS.md 与 legacy CLAUDE.md 内容相同。' }
        : { workspaceSlug, status: 'conflict', message: 'AGENTS.md 与 legacy CLAUDE.md 内容不同，已完整保留两份等待人工整理。' }
    }
    if (agentsExists) return { workspaceSlug, status: 'ready' }
    if (legacyExists) return { workspaceSlug, status: 'migrated', message: 'legacy CLAUDE.md 可安全迁移到 AGENTS.md。' }
    return { workspaceSlug, status: 'ready' }
  } catch (error) {
    return {
      workspaceSlug,
      status: 'failed',
      message: `检查工作区指令失败: ${error instanceof Error ? error.message : '未知错误'}`,
    }
  }
}

export function migrateWorkspaceInstructionFile(
  workspaceSlug: string,
  dependencies: { removeFile?: (path: string) => void } = {},
): WorkspaceInstructionMigrationReport {
  const removeFile = dependencies.removeFile ?? ((path: string) => rmSyncWithRetry(path, { force: true }))
  const report = inspectWorkspaceInstructionMigration(workspaceSlug)
  const agentsPath = getWorkspaceAgentsMdPath(workspaceSlug)
  const legacyPath = getLegacyWorkspaceClaudeMdPath(workspaceSlug)

  try {
    if (report.status === 'migrated') {
      // 先以排他模式创建目标，避免检查与写入之间的竞态覆盖；源文件在目标完整落盘前始终保留。
      copyFileSync(legacyPath, agentsPath, constants.COPYFILE_EXCL)
      if (!readFileSync(agentsPath).equals(readFileSync(legacyPath))) {
        rmSyncWithRetry(agentsPath, { force: true })
        throw new Error('迁移副本校验失败')
      }
      try {
        removeFile(legacyPath)
      } catch (error) {
        // 删除源失败时回滚目标副本，继续保留唯一 legacy 真源。
        rmSyncWithRetry(agentsPath, { force: true })
        throw error
      }
      return { workspaceSlug, status: 'migrated', message: '已将 Domi 工作区 CLAUDE.md 安全迁移为 AGENTS.md。' }
    }
    if (report.status === 'duplicate_removed') {
      if (!readFileSync(agentsPath).equals(readFileSync(legacyPath))) {
        return { workspaceSlug, status: 'conflict', message: '迁移期间两份指令内容发生变化，已完整保留等待人工整理。' }
      }
      removeFile(legacyPath)
      return { workspaceSlug, status: 'duplicate_removed', message: '已清理与 AGENTS.md 完全相同的 legacy CLAUDE.md。' }
    }
    return report
  } catch (error) {
    return {
      workspaceSlug,
      status: 'failed',
      message: `迁移工作区指令失败，原文件已保留或回滚: ${error instanceof Error ? error.message : '未知错误'}`,
    }
  }
}

/** 对所有 Domi 受管工作区执行幂等 AGENTS.md 迁移；绝不触碰用户项目根。 */
export function migrateWorkspaceInstructionFiles(): WorkspaceInstructionMigrationReport[] {
  return readIndex().workspaces.map((workspace) => {
    const report = migrateWorkspaceInstructionFile(workspace.slug)
    workspaceInstructionMigrationReports.set(workspace.slug, report)
    if (report.status === 'conflict' || report.status === 'blocked' || report.status === 'failed') {
      console.warn(`[Agent 工作区] ${report.message} (${workspace.slug})`)
    } else if (report.status !== 'ready') {
      console.log(`[Agent 工作区] ${report.message} (${workspace.slug})`)
    }
    return report
  })
}
...
export function getWorkspaceMemorySummary(workspaceSlug: string): WorkspaceMemorySummary {
  ...
  const agentsMd = instructionFileSummary(getWorkspaceAgentsMdPath(workspaceSlug))
  const legacyClaudeMd = instructionFileSummary(getLegacyWorkspaceClaudeMdPath(workspaceSlug))
  const inspectedMigration = inspectWorkspaceInstructionMigration(workspaceSlug)
  const previousMigration = workspaceInstructionMigrationReports.get(workspaceSlug)
  const migration = previousMigration?.status === 'failed'
    && (inspectedMigration.status === 'migrated' || inspectedMigration.status === 'duplicate_removed')
    ? previousMigration
    : inspectedMigration
  return {
    agentsMd,
    ...(legacyClaudeMd.exists ? { legacyClaudeMd } : {}),
    instructionMigration: { status: migration.status, ...(migration.message ? { message: migration.message } : {}) },
    // 历史 renderer 继续得到有效主文件路径，但不能借此写回 legacy 文件。
    claudeMd: agentsMd,
...
export function readWorkspaceAgentsMd(workspaceSlug: string): SkillFileContent {
  ...
  if (!st.isFile() || st.isSymbolicLink()) throw new Error(`${WORKSPACE_AGENTS_MD} 不是普通文件`)
...
export function writeWorkspaceAgentsMd(workspaceSlug: string, content: string): void {
  ...
  const target = getWorkspaceAgentsMdPath(workspaceSlug)
  if (existsSync(target)) {
    const st = lstatSync(target)
    if (!st.isFile() || st.isSymbolicLink()) throw new Error(`${WORKSPACE_AGENTS_MD} 不是普通文件`)
  }
  writeFileSync(target, content, 'utf-8')
  workspaceInstructionMigrationReports.delete(workspaceSlug)
  console.log(`[Agent 工作区] 已更新工作区 AGENTS.md: ${workspaceSlug}`)
}

/** @deprecated 兼容旧 IPC/preload；代理到 AGENTS.md 主文件。 */
...
interface WorkspaceConfig {
  ...
  worktreeRepos?: import('@domi/shared').WorkspaceWorktreeRepo[]
}
 ...
 */
export async function getWorktreeRepos(workspaceSlug: string): Promise<import('@domi/shared').WorkspaceWorktreeRepo[]> {
  const config = readWorkspaceConfig(workspaceSlug)

  // repoPath 归一化后去重
  const byPath = new Map<string, import('@domi/shared').WorkspaceWorktreeRepo>()
...
export function addWorktreeRepo(workspaceSlug: string, repo: import('@domi/shared').WorkspaceWorktreeRepo): import('@domi/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
...
export function removeWorktreeRepo(workspaceSlug: string, repoPath: string): import('@domi/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
  const existing = config.worktreeRepos ?? []
