...
export interface ClosableProxyFetch {
  fetchFn: typeof globalThis.fetch
  close: () => Promise<void>
}

/** 为单次敏感请求创建可显式释放的代理连接池。 */
export function createClosableProxyFetch(proxyUrl?: string): ClosableProxyFetch {
  const normalized = proxyUrl?.trim()
  if (!normalized) return { fetchFn: fetch, close: async () => {} }
  const dispatcher = new ProxyAgent(normalized)
  const fetchFn = ((input: RequestInfo | URL, init?: RequestInit) => undiciFetch(input as RequestInfo, {
    ...init,
    dispatcher,
  })) as unknown as typeof globalThis.fetch
  return {
