 ...
 * - 查找渠道、解密 API Key
 * - 管理 AbortController
 * - 调用 @domi/core 的 Provider 适配器系统
 * - 桥接 StreamEvent → webContents.send()
 * - 持久化消息到 JSONL + 更新索引
 * - 模块化工具的 function calling 循环（通过 ChatToolRegistry + ChatToolExecutor）
 *
 * 纯逻辑（消息转换、SSE 解析、请求构建）已抽象到 @domi/core/providers。
 */

import { randomUUID } from 'node:crypto'
import type { WebContents } from 'electron'
import { CHAT_IPC_CHANNELS } from '@domi/shared'
import type { ChatSendInput, ChatMessage, GenerateTitleInput, FileAttachment, ChatToolActivity } from '@domi/shared'
import {
  getAdapter,
  streamSSE,
  fetchTitle,
} from '@domi/core'
import type { ImageAttachmentData, ContinuationMessage } from '@domi/core'
...
export async function sendMessage(
  ...
  if (channel.provider === 'openai-codex') {
    webContents.send(CHAT_IPC_CHANNELS.STREAM_ERROR, {
      conversationId,
      error: 'Chat 模式暂不支持 ChatGPT 订阅（Codex OAuth），请切换到 Work 模式使用。',
    })
    return
  }
  ...
  // token 与真实 provider 请求数累积（跨工具轮次，最终写入 usage-entries.jsonl）
  let accumulatedUsage = createChatUsageAccumulator()

  /** 将本轮真实 provider 请求数与可用 usage 写入 usage-entries.jsonl，写后复位。 */
  const flushUsageRecord = (): void => {
    if (accumulatedUsage.providerRequestCount <= 0) return
    try {
      appendUsageEntry({
        timestamp: Date.now(),
        mode: 'chat',
        ...
        cacheReadTokens: accumulatedUsage.cacheReadTokens > 0 ? accumulatedUsage.cacheReadTokens : undefined,
        cacheCreationTokens: accumulatedUsage.cacheCreationTokens > 0 ? accumulatedUsage.cacheCreationTokens : undefined,
        providerRequestCount: accumulatedUsage.providerRequestCount,
        providerRequestCountAccuracy: 'exact',
      })
    } catch (error) {
      // 用量记录失败不影响主流程
      console.warn('[聊天服务] 记录 token 用量失败:', error)
    }
    accumulatedUsage = createChatUsageAccumulator()
  }
  ...
  try {
    ...
    while (round < MAX_TOOL_ROUNDS) {
      ...
      accumulatedUsage = addChatProviderRequest(accumulatedUsage)
      ...
      accumulatedUsage = mergeChatProviderUsage(accumulatedUsage, usage)
    ...
    if (pendingToolResults && continuationMessages.length > 0) {
      ...
      accumulatedUsage = mergeChatProviderUsage(accumulatedUsage, finalResult.usage)
    }
    ...
    // 成功路径：落盘本轮用量（在 try 末尾、finally 之前）
    flushUsageRecord()
  } catch (error) {
    ...
    if (controller.signal.aborted) {
      ...
      // 中止路径：即使 provider 没有返回 usage，也要保留已经发生的请求数。
      flushUsageRecord()
      return
    }
