 ...
 * Domi 每次发布即覆盖升级，无需运行时同步到用户目录。
 */

import type { BuiltinMcpCategory, McpToolSummary } from '@domi/shared'
import manifest from './default-mcp.json' with { type: 'json' }

export interface BuiltinMcpDefinition {
  /** 设置 / 凭据键，历史值，向后兼容（如 'domi-cloud'、'nano-banana'） */
  id: string
  ...
  /** 是否提供开关（基础设施型如 domi-cloud 置 false） */
  toggleable: boolean
