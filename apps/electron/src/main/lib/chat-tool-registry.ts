...
import type { ToolDefinition, ToolParameterProperty } from '@domi/core'
import type { ChatToolInfo, ChatToolMeta } from '@domi/shared'
import { getChatToolsConfig } from './chat-tool-config'
import {
  hasRequiredHttpToolCredentials,
  listHttpCredentialKeys,
} from './chat-tools/http-tool-credentials.ts'
import {
...
export function getEnabledTools(enabledToolIds?: string[]): EnabledToolsResult {
  ...
  for (const customMeta of config.customTools) {
    ...
    if (
      !customMeta.httpConfig
      || !hasRequiredHttpToolCredentials(customMeta.httpConfig, config.toolCredentials[toolId] ?? {})
    ) continue
...
export function getAllToolInfos(): ChatToolInfo[] {
  ...
  for (const customMeta of config.customTools) {
    ...
    const credentials = config.toolCredentials[customMeta.id] ?? {}
    const credentialKeys = customMeta.httpConfig
      ? listHttpCredentialKeys(customMeta.httpConfig)
      : []
    ...
    infos.push({
      ...
      available: Boolean(
        customMeta.httpConfig
        && hasRequiredHttpToolCredentials(customMeta.httpConfig, credentials)
      ),
      credentialKeys,
      configuredCredentialKeys: credentialKeys.filter((key) => Boolean(credentials[key])),
    })
  }
