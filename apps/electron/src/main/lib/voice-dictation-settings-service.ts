...
export function getVoiceDictationSettings(): VoiceDictationSettings {
  ...
  const legacyOutputMode = (raw as { outputMode?: string }).outputMode
  return {
    ...DEFAULT_VOICE_DICTATION_SETTINGS,
    ...raw,
    outputMode: legacyOutputMode === 'proma-input'
      ? 'domi-input'
      : raw.outputMode ?? DEFAULT_VOICE_DICTATION_SETTINGS.outputMode,
