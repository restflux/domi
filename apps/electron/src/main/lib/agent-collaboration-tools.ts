 ...
 * Agent 内置协作会话工具
 *
 * 通过 Pi custom tools 暴露 Domi Agent 子会话委派能力。
...
import { bindProductionAgentSessionTargetForLaunch } from './agent-session-target.ts'
...
interface DelegationRecord {
  ...
  cancelRuntimeTimer?: () => void
  completion: Promise<void>
  resolveCompletion: () => void
}

const MAX_WAIT_SECONDS = 2 * 60 * 60
const DEFAULT_WAIT_SECONDS = 10 * 60
const MAX_CONTINUATION_WAIT_SECONDS = 10 * 60
...
export function registerCollaborationEventBus(eventBus: import('./agent-event-bus').AgentEventBus): void {
  ...
  eventBus.on((sessionId: string, payload: AgentStreamPayload) => {
    ...
    if (payload.kind !== 'domi_event') return
    ...
    if (event.type === 'ask_user_request') {
      ...
      eventBus.emit(record.parentSessionId, {
        ...
        } as import('@domi/shared').DomiEvent,
      })
    }
...
interface StartDelegationResult {
  ...
  effectivePermissionMode?: DomiPermissionMode
...
function summarizeChildResult(childSessionId: string): string {
  ...
  for (let index = sdkMessages.length - 1; index >= 0; index -= 1) {
    ...
    if (!message) continue
    const text = extractTextFromSdkMessage(message).join('\n\n').trim()
...
function clearDelegationRuntimeTimer(record: DelegationRecord): void {
  record.cancelRuntimeTimer?.()
  record.cancelRuntimeTimer = undefined
  record.runtimeDeadlineAt = undefined
}
...
function markDelegationFinished(
  ...
  clearDelegationRuntimeTimer(record)
...
function armDelegationRuntimeTimer(record: DelegationRecord, maxRuntimeSeconds: number | undefined): void {
  clearDelegationRuntimeTimer(record)
  record.maxRuntimeSeconds = maxRuntimeSeconds
  if (maxRuntimeSeconds === undefined) return

  const timer = createDelegationRuntimeTimer(
    maxRuntimeSeconds * 1_000,
    () => {
      if (record.status !== 'running') return
      let error = `超过最大运行时长 ${maxRuntimeSeconds} 秒，已自动停止`
      try {
        stopRegisteredAgent(record.childSessionId, 'collaboration-timeout')
      } catch (stopError) {
        const stopMessage = stopError instanceof Error ? stopError.message : String(stopError)
        error = `${error}；停止调用失败：${stopMessage}`
      }
      markDelegationFinished(record, 'cancelled', { error })
    },
    { unref: true },
  )
  record.runtimeDeadlineAt = timer.deadlineAt
  record.cancelRuntimeTimer = timer.cancel
}

function getDelegationLaunchSummary(record: DelegationRecord): Record<string, unknown> {
  return {
    delegationId: record.delegationId,
...
function getDelegationWaitSummary(item: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    runtimeDeadlineAt: item.runtimeDeadlineAt,
    pendingBlockedEvents: item.pendingBlockedEvents,
  }
}

function listKnownDelegations(parentSessionId: string): DelegationListSource[] {
  const live = Array.from(delegations.values())
    .filter((item) => item.parentSessionId === parentSessionId)
    .map((item): DelegationListSource => ({
      delegationId: item.delegationId,
      childSessionId: item.childSessionId,
      modelId: item.modelId,
      title: item.title,
      ...
      startedAt: item.startedAt,
      completedAt: item.completedAt,
      pendingBlockedEvents: getPendingBlockedEvents(item.delegationId),
    }))
  ...
  const persisted = listAgentSessions()
    ...
    .map((session): DelegationListSource => {
      const persistedStatus = session.delegationStatus === 'running'
        ? 'interrupted'
...
function getDelegationListResponse(
  parentSessionId: string,
  args: { includeCompleted?: boolean; limit?: number },
): Record<string, unknown> {
  return {
    maxRunningDelegations: MAX_RUNNING_DELEGATIONS_PER_PARENT,
    ...buildDelegationListPayload(listKnownDelegations(parentSessionId), args),
  }
}
...
function recoverDelegationRecordFromSession(
  ...
  fallbackPermissionMode: DomiPermissionMode | undefined,
...
function getCurrentParentPermissionMode(
  ...
  fallback: DomiPermissionMode | undefined,
): DomiPermissionMode | undefined {
...
function getDelegationRunControlOverrides(record: DelegationRecord): ReturnType<typeof buildDelegationRunControlOverrides> {
  const child = getAgentSessionMeta(record.childSessionId)
  return buildDelegationRunControlOverrides({
    executionPolicy: child?.executionPolicy ?? record.executionPolicy,
    workflow: child?.workflow ?? record.workflow,
    permissionMode: child?.permissionMode ?? record.permissionMode,
  })
}
...
function stopDelegation(parentSessionId: string, delegationId: string): Record<string, unknown> {
  ...
  stopRegisteredAgent(record.childSessionId, 'collaboration-user')
...
/**
 * 宿主级停止入口：供 Work Activity 等已完成二次确认的宿主 UI 复用协作终止语义。
 * 必须经过 parentSessionId + delegationId 归属校验，不能仅凭 childSessionId 绕过 owner 边界。
 */
export function stopDelegationForHost(parentSessionId: string, delegationId: string): boolean {
  const result = stopDelegation(parentSessionId, delegationId)
...
async function startDelegation(
  ...
  const maxRuntimeSeconds = resolveDelegationMaxRuntimeSeconds(role, args.maxRuntimeSeconds)
  const title = normalizeTitle(args.title, `协作：${task}`)
  const goal = truncateText(task, DELEGATION_GOAL_CHAR_LIMIT)
  const latestParent = parent ? getAgentSessionMeta(parent.id) ?? parent : undefined
  const controls = resolveDelegationExecutionControls({
    parentExecutionPolicy: latestParent?.executionPolicy ?? ctx.executionPolicy,
    parentPermissionMode: getCurrentParentPermissionMode(latestParent, ctx.permissionMode),
    requestedPermissionMode: args.permissionMode,
  })
  ...
  try {
    await bindProductionAgentSessionTargetForLaunch({
      sessionId: child.id,
      choice: { kind: 'inherit', parentSessionId: ctx.sessionId },
    })
  } catch (error) {
    await deleteAgentSession(child.id)
    throw error
  }
  ...
  armDelegationRuntimeTimer(record, maxRuntimeSeconds)
  ...
  runRegisteredHeadlessAgent(
    {
      ...
      ...buildDelegationRunControlOverrides(controls),
  ...
  return { record, effectivePermissionMode: controls.permissionMode, effectiveModelId }
}

interface ContinueDelegationArgs {
  delegationId: string
  message: string
  maxRuntimeSeconds?: number
  waitSeconds?: number
...
async function continueDelegation(
  ctx: CollaborationToolContext,
  args: ContinueDelegationArgs,
  ...
  const maxRuntimeSeconds = resolveDelegationMaxRuntimeSeconds(
    record.role,
    args.maxRuntimeSeconds ?? record.maxRuntimeSeconds,
  )
  ...
  runRegisteredHeadlessAgent(
    {
      ...
      modelId: record.modelId,
      workspaceId: ctx.workspaceId,
      ...getDelegationRunControlOverrides(record),
      triggeredBy: 'delegation',
      startedAt,
    },
    {
  ...
  const waitSeconds = Math.min(
    MAX_CONTINUATION_WAIT_SECONDS,
    Math.max(0, Math.floor(args.waitSeconds ?? 0)),
  )
  if (waitSeconds > 0) {
    await waitForLiveRecords([record], waitSeconds, 1)
  }

  return {
    delegation: getDelegationSummary(record),
    note: record.status === 'running'
      ? '子会话已启动并仍在运行中；需要结果时请按 delegationId 调用 wait_for_delegations。'
      : undefined,
  }
}
...
export function buildPiCollaborationTools(
  ...
  const delegateItemType = Type.Object({
    ...
    maxRuntimeSeconds: Type.Optional(Type.Number({ minimum: 60, maximum: MAX_WAIT_SECONDS, description: '子会话最大运行秒数；review 默认 900 秒' })),
  })
  ...
  return [
    ...
    sdk.defineTool({
      ...
      label: '委派子 Agent',
      description: '创建一个真实可见的 Domi 协作子 Agent 会话来并行处理独立子任务。只用于长耗时、可并行、需要追踪的任务。',
      parameters: Type.Object({
        title: Type.Optional(Type.String({ description: '子会话标题' })),
        role: roleType,
      ...
      async execute(toolCallId: string, params: unknown) {
        ...
        return piJsonResult({
          delegation: getDelegationWaitSummary(getDelegationResult(ctx.sessionId, result.delegationId)),
          effectivePermissionMode: result.effectivePermissionMode,
          effectiveModelId: result.effectiveModelId,
          note: '子会话已启动。请保留 delegationId；需要结果时按 ID 调用 wait_for_delegations。',
        })
      },
    }),
    sdk.defineTool({
      ...
      label: '批量委派子 Agent',
      description: '批量创建多个真实可见的 Domi 协作子 Agent 会话。适合把同一大任务拆成多片并行处理。',
      parameters: Type.Object({
      ...
      async execute(toolCallId: string, params: unknown) {
        ...
        return piJsonResult({
          delegations: batch.created.map((item) => getDelegationWaitSummary(
            getDelegationResult(ctx.sessionId, item.delegationId),
          )),
    ...
    sdk.defineTool({
      ...
      label: '等待子会话完成',
      description: '等待一个或多个 Domi 协作子会话完成，并返回结构化结果摘要。',
      parameters: Type.Object({
        ...
        timeoutSeconds: Type.Optional(Type.Number({ minimum: 1, maximum: MAX_WAIT_SECONDS, description: '最长等待秒数，默认 600，最大 7200' })),
      }),
      async execute(_toolCallId: string, params: unknown) {
        ...
        return piJsonResult({
          ...
          delegations: allDelegations.map(getDelegationWaitSummary),
        })
      },
    }),
    sdk.defineTool({
      ...
      label: '列出协作子会话',
      description: '列出当前父会话的紧凑委派状态。默认仅返回运行中委派；恢复历史时显式传 includeCompleted=true。',
      parameters: Type.Object({
        includeCompleted: Type.Optional(Type.Boolean({ description: '是否包含已完成委派，默认 false' })),
        limit: Type.Optional(Type.Number({ minimum: 1, maximum: MAX_RUNNING_DELEGATIONS_PER_PARENT, description: '最多返回数量，默认 20，最大 50' })),
      }),
      async execute(_toolCallId: string, params: unknown) {
        const args = params as { includeCompleted?: boolean; limit?: number }
        return piJsonResult(getDelegationListResponse(ctx.sessionId, args))
      },
    }),
    sdk.defineTool({
      name: 'mcp__collaboration__get_delegation_results',
      label: '读取子会话结果',
      description: '按已知委派 ID 读取一个或多个 Domi 协作子会话的结果摘要；创建后应直接保留 ID。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '停止子会话',
      description: '停止一个正在运行的 Domi 协作子会话。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '批量停止子会话',
      description: '批量停止多个正在运行的 Domi 协作子会话。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      async execute(_toolCallId: string, params: unknown) {
        ...
        if (blocked.type === 'permission' && blocked.permissionRequestId) {
          ...
          const response = await permissionService.respondToPermission(blocked.permissionRequestId, behavior, false)
          blocked.resolved = response.ok
          if (blocked.resolved && _eventBusRef) {
            _eventBusRef.emit(blocked.childSessionId, {
              kind: 'domi_event',
    ...
    sdk.defineTool({
      ...
      label: '追加后续指令',
      description: '向已结束的协作子会话追加后续指令。默认立即返回；需要结果时按 delegationId 调用 wait_for_delegations。',
      parameters: Type.Object({
        delegationId: Type.String({ description: '要继续操作的委派 ID' }),
        message: Type.String({ description: '追加给子 Agent 的后续指令' }),
        maxRuntimeSeconds: Type.Optional(Type.Number({ minimum: 60, maximum: MAX_WAIT_SECONDS, description: '本次继续运行的最大秒数' })),
        waitSeconds: Type.Optional(Type.Number({ minimum: 0, maximum: MAX_CONTINUATION_WAIT_SECONDS, description: '工具内部等待秒数，默认 0，最大 600' })),
      }),
      async execute(_toolCallId: string, params: unknown) {
        return piJsonResult(await continueDelegation(ctx, params as ContinueDelegationArgs))
      },
    }),
  ]
}
