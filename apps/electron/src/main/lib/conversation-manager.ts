 ...
 *
 * 负责对话的 CRUD 操作和消息持久化。
 * - 对话索引：~/.domi/conversations.json（轻量元数据）
 * - 消息存储：~/.domi/conversations/{id}.jsonl（JSONL 格式，逐行追加）
 */
...
import type { ConversationMeta, ChatMessage, RecentMessagesResult, MessageSearchResult } from '@domi/shared'
