 ...
 * 工作区文件监听器
 *
 * 使用 fs.watch 递归监听 ~/.domi/agent-workspaces/ 目录，
 * 根据变化的文件路径区分事件类型：
...
import { isHighNoisePath, normalizeWatchFilename, shouldNotifyForWatchFilename } from './workspace-watcher-utils.ts'
...
/** 外部用户级 Skills / Pi MCP 监听器。 */
const globalCapabilityWatchers: FSWatcher[] = []
  ...
  if (!entry) {
    try {
      ...
      const watcher = watch(parentPath, { recursive: false }, (_eventType, filename) => {
        ...
        const isTargetRecovery = [...targetPaths].some((targetPath) => resolve(targetPath) === changedPath)
      ...
      watcher.on('error', (err) => {
        ...
        for (const targetPath of pathsToRestore) unavailableDirectoryParents.delete(targetPath)
...
export function startWorkspaceWatcher(win: BrowserWindow): void {
  ...
  mainWin = win
  startGlobalCapabilitiesWatchers(win)
  ...
  try {
    watcher = watch(watchDir, { recursive: true }, (_eventType, filename) => {
      ...
      // 跳过 .git、node_modules、构建产物等高频路径，避免 Git 状态读取/刷新形成 watcher 回环。
 ...
 * 停止工作区文件监听
 */
function notifyGlobalCapabilitiesChanged(win: BrowserWindow): void {
  if (globalCapabilitiesTimer) clearTimeout(globalCapabilitiesTimer)
  globalCapabilitiesTimer = setTimeout(() => {
    if (!win.isDestroyed()) win.webContents.send(AGENT_IPC_CHANNELS.CAPABILITIES_CHANGED)
    globalCapabilitiesTimer = null
  }, DEBOUNCE_MS)
}

function startGlobalCapabilitiesWatchers(win: BrowserWindow): void {
  for (const existing of globalCapabilityWatchers.splice(0)) {
    try { existing.close() } catch { /* watcher may already be closed */ }
  }
  ...
  for (const root of skillRoots) {
    ...
    try {
      const externalWatcher = watch(root, { recursive: true }, () => notifyGlobalCapabilitiesChanged(win))
      externalWatcher.on('error', (error) => {
        console.warn('[全局能力监听] Skills 监听失效:', root, error)
      })
      globalCapabilityWatchers.push(externalWatcher)
    } catch (error) {
      console.warn('[全局能力监听] 无法监听 Skills:', root, error)
    }
  }

  const mcpPath = resolvePiGlobalMcpPath()
  const mcpDir = dirname(mcpPath)
  if (isExistingDirectory(mcpDir)) {
    try {
      const externalWatcher = watch(mcpDir, { recursive: false }, (_eventType, filename) => {
        if (!filename || basename(filename.toString()) !== basename(mcpPath)) return
        notifyGlobalCapabilitiesChanged(win)
      })
      externalWatcher.on('error', (error) => {
        console.warn('[全局能力监听] Pi MCP 监听失效:', mcpPath, error)
      })
      globalCapabilityWatchers.push(externalWatcher)
    } catch (error) {
      console.warn('[全局能力监听] 无法监听 Pi MCP:', mcpPath, error)
    }
  }
}
...
export function stopWorkspaceWatcher(): void {
  ...
  for (const externalWatcher of globalCapabilityWatchers.splice(0)) {
    try { externalWatcher.close() } catch { /* watcher may already be closed */ }
  }
  if (globalCapabilitiesTimer) {
    clearTimeout(globalCapabilitiesTimer)
    globalCapabilitiesTimer = null
  }
