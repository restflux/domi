 ...
 * 文本输出服务
 *
 * 语音输入完成后优先写入 Domi 输入框，否则尝试写入当前光标位置。
 */
...
let targetWasDomiInput = false
let activePreviewSessionId: string | null = null
let closedPreviewSessionId: string | null = null

/** 在显示语音浮窗前记录目标是否为 Domi 主窗口。 */
export function captureVoiceDictationTarget(forceDomiInput?: boolean): boolean {
  const mainWindow = getMainWindow()
  targetWasDomiInput = forceDomiInput ?? BrowserWindow.getFocusedWindow() === mainWindow
  return targetWasDomiInput
}

function shouldWriteToDomiInput(settings: VoiceDictationSettings): boolean {
  return settings.outputMode === 'domi-input' ||
    (settings.outputMode === 'auto' && targetWasDomiInput)
}
...
export function previewVoiceDictationText(
  ...
  if (!text || !shouldWriteToDomiInput(settings)) return
...
export async function commitVoiceDictationText(
  ...
  if ((hasActivePreview || shouldWriteToDomiInput(settings)) && sendTextEvent(VOICE_DICTATION_IPC_CHANNELS.INSERT_TEXT, {
    sessionId: input.sessionId,
    text: trimmed,
  })) {
    activePreviewSessionId = null
    closedPreviewSessionId = input.sessionId
    return { mode: 'domi-input', success: true, message: '已写入 Domi 输入框' }
  }
