...
export function createPiCompactionBoundaryMessage(
  sessionId: string,
  result: { summary: string; estimatedTokensAfter?: number },
): SDKMessage {
  return {
    type: 'system',
    ...
    ...(result.estimatedTokensAfter != null && result.estimatedTokensAfter > 0 && {
...
export function convertPiMessage(
  ...
  if (message.role === 'assistant') {
    ...
    // Pi 的 message_update 是累计内容预览帧；其中 usage 尚未终结，常见值是全 0。
    // 只在 message_end 的 final 帧透传 usage，避免下一次模型生成时把上一轮真实占用覆盖为 0。
    const usage = final ? usageFromAssistant(assistant) : undefined
...
export function convertResultMessage(
  ...
  const assistants = messages.filter((m): m is AssistantMessage => {
    if (!m || typeof m !== 'object' || !('role' in m) || m.role !== 'assistant') return false
    const metadata = m as AssistantMessage & { _partial?: boolean; isReplay?: boolean; parent_tool_use_id?: string | null }
    return metadata._partial !== true && metadata.isReplay !== true && !metadata.parent_tool_use_id
  })
  ...
  const providerRequestCount = assistants.filter((message) => message.usage != null).length
  ...
  return {
    ...
    ...(providerRequestCount > 0 && {
      _providerRequestCount: providerRequestCount,
