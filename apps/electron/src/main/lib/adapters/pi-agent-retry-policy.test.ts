...
function createTestSession(settings: NativeRetrySettings): {
  ...
  session._retryNowRequested = false
...
describe('patched Pi AgentSession retry policy', () => {
  ...
  test('lets the user skip native retry backoff without cancelling the same transcript', async () => {
    ...
    const retry = session._prepareRetry(failed)
    expect(session.retryNow()).toBe(true)

    await expect(retry).resolves.toBe(true)
    expect(events.map((event) => event.type)).toEqual(['auto_retry_start'])
    const pendingRetry = session._pendingRetryAttemptStart
    if (
      !pendingRetry
      || typeof pendingRetry.attempt !== 'number'
      || typeof pendingRetry.totalAttempt !== 'number'
      || typeof pendingRetry.delayMs !== 'number'
      || typeof pendingRetry.totalDelayMs !== 'number'
    ) {
      throw new Error('Expected a numeric pending retry attempt payload')
    }
    expect(pendingRetry.attempt).toBe(1)
    expect(pendingRetry.totalAttempt).toBe(1)
    expect(pendingRetry.delayMs).toBeLessThan(1_000)
    expect(session._retryRunDelayMs).toBeLessThan(1_000)
  })
