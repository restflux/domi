 ...
 *
 * The renderer never receives raw domi-file:// absolute paths. Main process
...
function registerEntry(path: string, isDirectory: boolean): string {
  ...
  return `domi-file://${token}`
}

export function registerDomiFilePath(path: string): string {
  return registerEntry(path, false)
}

export function registerDomiDirectoryPath(path: string): string {
  return registerEntry(path, true)
}

export function handleDomiFileRequest(request: Request): Promise<Response> | Response {
  let url: URL
  try {
