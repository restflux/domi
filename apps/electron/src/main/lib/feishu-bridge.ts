...
import { FEISHU_IPC_CHANNELS, AGENT_IPC_CHANNELS } from '@domi/shared'
...
import { buildFeishuGeneratedImagePayload } from './bridge-generated-image-payloads'
import {
...
import { formatBridgeRuntimeStatusLines } from './bridge-run-metadata'
...
class FeishuBridge {
  ...
  /** 正在等待 IM/桌面回答的会话；用于把流式卡标题切换为等待确认。 */
  private interactionWaitingSessions = new Set<string>()
  /** 用过流式卡的 sessionId 集合（complete 时判定是否需要降级回复卡） */
  private streamingCardsUsedSessions = new Set<string>()
  /** 交互请求 ID → 飞书确认卡位置，用于桌面抢答同步与会话级清理。 */
  private interactionCardMessageIds = new Map<string, { messageId: string; sessionId: string }>()
  /** sessionId → 本轮发起用户；群聊确认只允许该用户回答或点击。 */
  private interactionOperators = new Map<string, string>()
  ...
  constructor(botConfig: FeishuBotConfig) {
    ...
    this.interactions = new BridgeInteractionCoordinator({
      ...
      onTimeout: (sessionId, chatId) => {
        void this.sendMessage(chatId, '等待确认已超时，本轮任务已停止。')
        this.endInteractionRun(sessionId)
        void stopAgent(sessionId, 'feishu-command')
      },
    })
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // LarkChannel 统一接收消息与 card.action.trigger；按钮值仍需与本地待请求逐项校验。
      this.channel.on({
        message: (msg) => {
        ...
        cardAction: (event) => {
          this.handleInteractionCardAction(
            event.chatId,
            event.messageId,
            event.operator.openId,
            event.action.value,
          ).catch((error) => {
            console.error('[飞书 Bridge] 处理确认卡片操作异常:', redactSensitiveLogValue(error))
          })
        },
      })
  ...
  stop(): void {
    ...
    this.interactionCardMessageIds.clear()
    this.interactionOperators.clear()
    ...
    this.interactionWaitingSessions.clear()
    ...
    // 重启后仍需用来给桌面 Session 镜像建群。完整重置请删除 ~/.domi/feishu-metadata-{botId}.json。
    this.botOpenId = null
  ...
  updateBinding(input: FeishuUpdateBindingInput): FeishuChatBinding | null {
    ...
    if (input.archived !== undefined) {
      ...
      if (input.archived) {
        this.endInteractionRun(binding.sessionId)
        this.sessionToChat.delete(binding.sessionId)
      } else {
   ...
   * source=session-mirror 的绑定回到同一个 Domi session。
   */
  async ensureSessionMirror(session: AgentSessionMeta): Promise<void> {
    ...
    const appSettings = getSettings()
    const workspaceId = session.workspaceId ?? this.resolveWorkspaceIdForNewSession()
  ...
  async startSessionMirrorRun(session: AgentSessionMeta): Promise<void> {
    ...
    const mirrorSettings = getSettings()
    this.interactionOperators.set(session.id, binding.userId)
    this.interactions.beginRun(session.id, binding.chatId)
    const runMetadata = await resolveBridgeRunMetadata({
      channelId: session.channelId ?? mirrorSettings.agentChannelId,
      modelId: session.modelId ?? mirrorSettings.agentModelId,
      sessionMeta: session,
      settings: mirrorSettings,
    })
    const initialState = createInitialState({
      model: runMetadata.modelName,
      thinkingLevel: runMetadata.thinkingLevel,
    })
  ...
  private endInteractionRun(sessionId: string): void {
    this.interactions.endRun(sessionId)
    this.interactionOperators.delete(sessionId)
    this.interactionWaitingSessions.delete(sessionId)
    for (const [requestId, card] of this.interactionCardMessageIds) {
      if (card.sessionId !== sessionId) continue
      this.interactionCardMessageIds.delete(requestId)
      void this.updateCardMessage(card.messageId, {
        config: { wide_screen_mode: true },
        header: { title: { tag: 'plain_text', content: '确认已关闭' }, template: 'grey' },
        elements: [{ tag: 'markdown', content: '本轮任务已结束或停止，该确认不再有效。' }],
      })
    }
  }
  ...
  private async handleFeishuMessage(data: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      if (!isControlCommand) {
        const pendingView = this.interactions.getPendingView(chatId)
        const expectedOperator = pendingView ? this.interactionOperators.get(pendingView.sessionId) : undefined
        if (pendingView && expectedOperator && expectedOperator !== 'unknown' && expectedOperator !== userId) {
          await this.sendMessage(chatId, '只有发起本轮任务的用户可以处理该确认。')
          return
        }
        ...
        if (interactionResult.handled) {
          ...
          await this.sendFeishuInteractionResult(chatId, interactionResult)
          return
        }
      }
  ...
  private async handleInteractionCardAction(
    chatId: string,
    messageId: string,
    operatorOpenId: string,
    value: unknown,
  ): Promise<void> {
    const action = parseFeishuInteractionActionValue(value)
    if (!action) return
    const pendingView = this.interactions.getPendingView(chatId)
    const expectedOperator = pendingView ? this.interactionOperators.get(pendingView.sessionId) : undefined
    if (expectedOperator && expectedOperator !== 'unknown' && expectedOperator !== operatorOpenId) {
      await this.sendMessage(chatId, '只有发起本轮任务的用户可以处理该确认。')
      return
    }
    const result = this.interactions.handleAction(chatId, action.requestId, action.actionId)
    if (result.message) await this.sendMessage(chatId, result.message)
    if (result.view && result.status !== 'accepted' && result.status !== 'expired') {
      await this.updateCardMessage(messageId, buildFeishuInteractionCard(result.view))
      return
    }
    if (result.status === 'accepted' || result.status === 'expired') {
      this.interactionCardMessageIds.delete(action.requestId)
      await this.updateCardMessage(messageId, {
        config: { wide_screen_mode: true },
        header: {
          title: { tag: 'plain_text', content: result.status === 'accepted' ? '确认已提交' : '确认已失效' },
          template: result.status === 'accepted' ? 'green' : 'grey',
        },
        elements: [{ tag: 'markdown', content: result.message ?? '该确认已处理。' }],
      })
    }
  }

  private async sendFeishuInteractionView(chatId: string, view: BridgeInteractionView): Promise<void> {
    const messageId = await this.sendCardMessage(chatId, buildFeishuInteractionCard(view))
    if (!messageId) return
    const current = this.interactions.getPendingView(chatId)
    if (current?.requestId === view.requestId && current.generation === view.generation) {
      this.interactionCardMessageIds.set(view.requestId, { messageId, sessionId: view.sessionId })
      return
    }
    // 卡片发送期间桌面可能已经抢答；不能把迟到卡片留成仍可点击的假待办。
    await this.updateCardMessage(messageId, {
      config: { wide_screen_mode: true },
      header: { title: { tag: 'plain_text', content: '确认已完成' }, template: 'green' },
      elements: [{ tag: 'markdown', content: '该确认已在其他入口处理。' }],
    })
  }

  private async sendFeishuInteractionResult(
    chatId: string,
    result: BridgeInteractionResult,
  ): Promise<void> {
    if (result.message) await this.sendMessage(chatId, result.message)
    if (result.view && result.status !== 'accepted' && result.status !== 'expired') {
      await this.sendFeishuInteractionView(chatId, result.view)
    }
  }
  ...
  private async handleCommand(msgCtx: FeishuMessageContext, text: string): Promise<void> {
    ...
    switch (command?.toLowerCase()) {
      ...
      case '/answer':
        await this.sendMessage(chatId, '当前没有等待回答的确认。')
        break
  ...
  /**
   * 解析飞书下一条新会话使用的项目：显式指定 > Bot 默认 > 机器人共享默认
   * > 默认/首个有效项目。桌面端最近会话只更新 agentWorkspaceId，不参与新字段。
   */
  private resolveWorkspaceIdForNewSession(overrideWorkspaceId?: string): string | undefined {
    const appSettings = getSettings()
    const candidateWorkspaceIds = [
      overrideWorkspaceId,
      this.botConfig.defaultWorkspaceId,
      resolveAgentRemoteDefaultWorkspaceId(appSettings),
    ]
    for (const workspaceId of candidateWorkspaceIds) {
      if (workspaceId && getAgentWorkspace(workspaceId)) return workspaceId
    }

    const byTime = listAgentWorkspacesByUpdatedAt()
    return byTime.find((workspace) => workspace.slug === 'default')?.id ?? byTime[0]?.id
  }
  ...
  private async createNewSession(
    ...
    const { chatId } = msgCtx
    const appSettings = getSettings()
    const workspaceId = this.resolveWorkspaceIdForNewSession(overrideWorkspaceId)

    if (!workspaceId) {
      await this.sendMessage(chatId, '请先在 Domi 设置中创建项目。')
      return
    }
    ...
    if (!channelId) {
      await this.sendMessage(chatId, '请先在 Domi Agent 设置中选择渠道。')
      return
    }
    ...
    if (oldBinding) {
      this.endInteractionRun(oldBinding.sessionId)
  ...
  private async handleStopCommand(msgCtx: FeishuMessageContext): Promise<void> {
    ...
    stopAgent(binding.sessionId, 'feishu-command')
    this.endInteractionRun(binding.sessionId)
  ...
  private async handleSwitchCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    const binding: FeishuChatBinding = {
      ...
      userId: msgCtx.senderOpenId,
      sessionId: match.id,
      workspaceId: match.workspaceId ?? this.resolveWorkspaceIdForNewSession() ?? '',
  ...
  private async handleWorkspaceCommand(msgCtx: FeishuMessageContext, arg?: string): Promise<void> {
    ...
    const workspaces = listAgentWorkspacesByUpdatedAt()
    const binding = this.chatBindings.get(chatId)
    const currentWorkspaceId = binding?.workspaceId ?? this.resolveWorkspaceIdForNewSession()
  ...
  private async handleNowCommand(msgCtx: FeishuMessageContext): Promise<void> {
    ...
    const runMetadata = await resolveBridgeRunMetadata({
      channelId: binding?.channelId || this.botConfig.defaultChannelId || nowSettings.agentChannelId,
      modelId: binding?.modelId || this.botConfig.defaultModelId || nowSettings.agentModelId,
      sessionMeta: session,
      settings: nowSettings,
    })
    lines.push(...formatBridgeRuntimeStatusLines(runMetadata, true))

    // 项目信息：未绑定会话时也展示下一条消息将使用的默认项目。
    const workspaceId = binding?.workspaceId ?? this.resolveWorkspaceIdForNewSession()
  ...
  private async handleModelCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    if (!targetBinding) {
      ...
      if (!targetBinding) {
        await this.sendMessage(chatId, '请先发送一条消息创建会话，或在 Domi 设置中选择 Agent 渠道。')
        return
      }
    }
  ...
  private async handleUserMessage(
    ...
    const runMetadata = await resolveBridgeRunMetadata({
      channelId,
      modelId,
      sessionMeta: launchSession,
      settings: latestSettings,
    })

    // 初始化本轮交互与输出缓冲。
    this.interactionOperators.set(binding.sessionId, msgCtx.senderOpenId)
    ...
    try {
      const cardStream = await CardStream.open(
        ...
        renderRunCard(initialState, {
          header: headerTitle,
          // 停止仍保留确定性的文本命令；交互按钮仅用于本轮确认卡，不混用终止语义。
    ...
    // 群聊时注入 Pi custom tool，允许 Agent 主动拉取更多群聊历史。
    const customTools = msgCtx.chatType === 'group'
      ? await this.createFeishuChatTools(chatId)
      : []
  ...
  private handleAgentPayload(sessionId: string, payload: AgentStreamPayload): void {
    // 对于飞书发起的会话，缓冲由 handleUserMessage 初始化；桌面镜像只有卡片状态。
    const buffer = this.sessionBuffers.get(sessionId)
    const chatId = this.sessionToChat.get(sessionId)

    if (chatId && payload.kind === 'domi_event') {
      if (payload.event.type === 'ask_user_request') {
        const view = this.interactions.registerAskUser(payload.event.request)
        if (view) {
          this.interactionWaitingSessions.add(sessionId)
          this.refreshStreamingInteractionStatus(sessionId)
          void this.sendFeishuInteractionView(chatId, view)
        }
        return
      }
      ...
      if (payload.event.type === 'ask_user_resolved' || payload.event.type === 'exit_plan_mode_resolved') {
        ...
        this.interactionWaitingSessions.delete(sessionId)
        this.refreshStreamingInteractionStatus(sessionId)
        const interactionCard = this.interactionCardMessageIds.get(payload.event.requestId)
        this.interactionCardMessageIds.delete(payload.event.requestId)
        if (interactionCard && (!resolved || !resolved.submittedByThisCoordinator)) {
          void this.updateCardMessage(interactionCard.messageId, {
            config: { wide_screen_mode: true },
            header: { title: { tag: 'plain_text', content: '确认已完成' }, template: 'green' },
            elements: [{ tag: 'markdown', content: '回答已提交，Agent 将继续处理。' }],
          })
        }
        if (resolved && !resolved.submittedByThisCoordinator) {
          void this.sendMessage(chatId, '该确认已在 Domi 或其他入口完成，Agent 将继续处理。')
        }
        return
      }
      if (payload.event.type === 'permission_resolved') {
        ...
        if (interactionCard) {
          void this.updateCardMessage(interactionCard.messageId, {
            config: { wide_screen_mode: true },
            header: { title: { tag: 'plain_text', content: '桌面确认已完成' }, template: 'green' },
            elements: [{ tag: 'markdown', content: '权限请求已在 Domi 桌面处理。' }],
          })
        }
        if (resolved) void this.sendMessage(chatId, '桌面权限确认已完成，Agent 将继续处理。')
        return
      }
    }
    ...
    if (runState && cardStream) {
      ...
      if (nextState !== runState) {
        ...
        const headerTitle = (header ? `${header.trim()} · ` : '') +
          (nextState.terminal === 'running'
            ? (this.interactionWaitingSessions.has(sessionId) ? '等待你的确认' : 'Agent 处理中')
            : 'Agent 已完成')
    ...
    if (buffer && payload.kind === 'sdk_message') {
      ...
      if (msg.type === 'assistant' && !isPartialSDKMessage(msg)) {
        ...
        for (const toolUseId of collectGeneratedImageToolUseIds(msg)) {
    ...
    if (payload.kind === 'domi_event' && payload.event.type === 'title_updated') {
      this.updateSessionMirrorGroupName(sessionId, payload.event.title)
    }
  }

  private refreshStreamingInteractionStatus(sessionId: string): void {
    const runState = this.streamingRunStates.get(sessionId)
    const cardStream = this.streamingCards.get(sessionId)
    if (!runState || !cardStream || runState.terminal !== 'running') return
    const chatId = this.sessionToChat.get(sessionId) ?? ''
    const prefix = this.resolveContextPrefix(chatId)
    const title = this.interactionWaitingSessions.has(sessionId) ? '等待你的确认' : 'Agent 处理中'
    cardStream.update(renderRunCard(runState, {
      header: prefix ? `${prefix.trim()} · ${title}` : title,
      stopHint: '发送 `/stop` 可终止当前任务',
    }))
  }
  ...
  private handleFeishuSessionComplete(sessionId: string): void {
    ...
    const chatId = this.sessionToChat.get(sessionId)
    if (chatId) {
      void this.deliverFeishuSessionReply(chatId, result, buffer.images, usedStreamingCard)
    }
  }

  private async deliverFeishuSessionReply(
    chatId: string,
    result: FormattedAgentResult,
    images: BridgeGeneratedImage[],
    usedStreamingCard: boolean,
  ): Promise<void> {
    // 用过流式卡时跳过 sendAgentReply：流式卡已经把完整文本呈现给用户。
    if (!usedStreamingCard) {
      try {
        await this.sendAgentReply(chatId, result)
      } catch (sendError) {
        console.error('[飞书 Bridge] 发送 Agent 回复失败:', redactSensitiveLogValue(sendError))
      }
    }
    for (const image of images) {
      try {
        await this.sendGeneratedImage(chatId, image)
      } catch (sendError) {
        console.error('[飞书 Bridge] 发送生成图片失败:', redactSensitiveLogValue(sendError))
      }
    }
  }
  ...
  /** 创建飞书群聊 Pi custom tool，仅注入当前群聊运行。 */
  private async createFeishuChatTools(chatId: string): Promise<ToolDefinition[]> {
    try {
      const sdk = await import('@earendil-works/pi-coding-agent')
      const tool = buildFeishuChatHistoryTool(sdk, {
        fetchHistory: (options) => this.fetchChatHistory(chatId, options),
        formatHistory: (messages) => this.formatChatHistoryContext(messages),
      })
      console.log('[飞书 Bridge] 已创建群聊 Pi 工具')
      return [tool]
    } catch (error) {
      console.warn('[飞书 Bridge] 创建群聊 Pi 工具失败:', redactSensitiveLogValue(error))
      return []
    }
  }
  ...
  private async sendGeneratedImage(chatId: string, image: BridgeGeneratedImage): Promise<void> {
    if (!this.client) return
    const data = image.data
      ? Buffer.from(image.data, 'base64')
      : Buffer.from(readAttachmentAsBase64(image.localPath!), 'base64')
    if (data.length === 0) throw new Error('生成图片内容为空')
    ...
    // 飞书 SDK 对 image.create 的返回类型声明与实际响应（data 包裹）不一致，按运行时形状读取
    const uploadResponse = upload as unknown as { data?: { image_key?: string } } | undefined
    const imageKey = uploadResponse?.data?.image_key
    if (typeof imageKey !== 'string' || !imageKey) throw new Error('飞书图片上传未返回 image_key')

    const binding = this.chatBindings.get(chatId)
    const replyToId = binding?.chatType === 'group'
      ? this.lastUserMessageId.get(chatId)
      : undefined
    const payload = buildFeishuGeneratedImagePayload(imageKey)
    const response = replyToId
      ? await this.client.im.message.reply({ path: { message_id: replyToId }, data: payload })
      : await this.client.im.message.create({
          params: { receive_id_type: 'chat_id' },
          data: { receive_id: chatId, ...payload },
        })
    const sentId = (response?.data as Record<string, unknown> | undefined)?.message_id
    if (typeof sentId === 'string' && sentId) this.addToDedup(this.recentMessageIds, sentId)
  }
  ...
  private async sendCard(chatId: string, card: Record<string, unknown>): Promise<string | undefined> {
  ...
  private async updateCardMessage(messageId: string, card: Record<string, unknown>): Promise<void> {
    if (!this.client) return
    try {
      ...
      console.error('[飞书 Bridge] 更新确认卡片失败:', redactSensitiveLogValue(error))
    }
  }
  ...
  /** 回复指定消息（卡片，群聊线程回复） */
  private async replyCard(messageId: string, card: Record<string, unknown>): Promise<string | undefined> {
    if (!this.client) return undefined
   ...
   * 发送卡片消息到聊天（自动选择回复或新建）
   */
  private async sendCardMessage(chatId: string, card: Record<string, unknown>): Promise<string | undefined> {
    const binding = this.chatBindings.get(chatId)
    const replyToId = binding?.chatType === 'group'
      ? this.lastUserMessageId.get(chatId)
      : undefined
    ...
    if (replyToId) {
      return this.replyCard(replyToId, card)
    }
