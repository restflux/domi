...
import { createFetchedOpenAIChannelModel, type OpenAIModelItem } from './channel-model-capabilities'
...
function cloneModels(models: readonly ChannelModel[]): ChannelModel[] {
  return models.map((model) => ({
    ...
    ...(model.providerMetadata ? {
      providerMetadata: {
        ...model.providerMetadata,
        ...(model.providerMetadata.input ? { input: [...model.providerMetadata.input] } : {}),
        ...(model.providerMetadata.reasoningLevels ? { reasoningLevels: [...model.providerMetadata.reasoningLevels] } : {}),
        ...(model.providerMetadata.thinkingLevelMap ? { thinkingLevelMap: { ...model.providerMetadata.thinkingLevelMap } } : {}),
      },
    } : {}),
    ...(model.temporaryAdaptation ? {
      temporaryAdaptation: {
        ...model.temporaryAdaptation,
        ...(model.temporaryAdaptation.input ? { input: [...model.temporaryAdaptation.input] } : {}),
        ...(model.temporaryAdaptation.reasoningLevels ? { reasoningLevels: [...model.temporaryAdaptation.reasoningLevels] } : {}),
        ...(model.temporaryAdaptation.thinkingLevelMap ? { thinkingLevelMap: { ...model.temporaryAdaptation.thinkingLevelMap } } : {}),
      },
    } : {}),
  }))
}

function createPresetModelsResult(providerName: string, models: readonly ChannelModel[]): FetchModelsResult {
  return {
    success: true,
...
function resolveQwenTokenPlanIndividualTestModelId(modelId?: string, models?: ChannelModel[]): string {
  const explicitModelId = modelId?.trim()
  if (explicitModelId) return explicitModelId
  return resolveFirstTestModelId(models) ?? QWEN_TOKEN_PLAN_INDIVIDUAL_PRESET_MODELS[0]!.id
}
...
function migrateConfig(config: ChannelsConfig): { config: ChannelsConfig; changed: boolean } {
  ...
  if (version >= CONFIG_VERSION && config.channels.every((channel) => !!channel.credentialVersion)) {
  ...
  const channels = config.channels.map((channel) => {
    ...
    if (!migrated.credentialVersion) {
      migrated = { ...migrated, credentialVersion: randomUUID() }
    }
    return migrated
  })
...
export function updateChannel(
  ...
  options: { preserveCredentialVersion?: boolean } = {},
): Channel {
  const config = readConfig()
  ...
  const updated: Channel = {
    ...
    finishReasonMode: input.finishReasonMode ?? existing.finishReasonMode,
    updatedAt: Date.now(),
    credentialVersion: input.apiKey && !options.preserveCredentialVersion
      ? randomUUID()
...
/** 保存 Pi 或 Domi 刷新后的完整 Codex OAuth 凭据。 */
export function persistCodexOAuthCredentials(channelId: string, credentials: CodexOAuthCredentials): void {
  ...
  updateChannel(
    channelId,
    { apiKey: serializeCodexCredentials(merged) },
    { preserveCredentialVersion: !existing?.accountId || !merged.accountId || existing.accountId === merged.accountId },
  )
}
...
export async function testChannel(channelId: string): Promise<ChannelTestResult> {
  ...
  try {
    switch (provider) {
      ...
      case 'custom':
        if (provider === 'qwen-token-plan-individual') {
          return await testQwenTokenPlanIndividual(
            channel.baseUrl,
            apiKey,
            resolveQwenTokenPlanIndividualTestModelId(undefined, channel.models),
            proxyUrl,
          )
        }
      ...
      default:
        return { success: false, message: `不支持的供应商: ${provider}。该供应商不在 Domi 支持的渠道列表中，请改用可配置的第三方渠道。` }
    }
  } catch (error) {
...
async function testAnthropicCompatible(
  ...
  if (provider === 'kimi-coding' || provider === 'zhipu-coding' || provider === 'zhipu-coding-team') {
    ...
    headers['User-Agent'] = getKimiCodingPlanUserAgent(pkg.version)
...
async function testQwenTokenPlanMessages(
  ...
  const response = await fetchFn(url, withTimeout({
    ...
    headers: {
      ...
      'content-type': 'application/json',
      Authorization: `Bearer ${apiKey}`,
      'User-Agent': getKimiCodingPlanUserAgent(pkg.version),
    },
    body: JSON.stringify({
      model: modelId,
      max_tokens: 8,
...
/**
 * 国际 Individual 订阅同样不依赖 /models，直接通过 OpenAI Chat Completions 验证凭据。
 */
async function testQwenTokenPlanIndividual(
  baseUrl: string,
  apiKey: string,
  modelId: string,
  proxyUrl?: string,
): Promise<ChannelTestResult> {
  const url = resolveOpenAIChatCompletionsUrl(baseUrl, 'qwen-token-plan-individual')
  const fetchFn = getFetchFn(proxyUrl)
...
async function fetchZhipuTeamQuota(
  credentials: import('@domi/shared').ZhipuTeamCredentials,
  proxyUrl?: string,
): Promise<ZhipuQuotaResponse | { error: string }> {
  const fetchFn = getFetchFn(proxyUrl)
  const headers: Record<string, string> = {
      ...
      case 'custom':
        if (provider === 'qwen-token-plan-individual') {
          return await testQwenTokenPlanIndividual(
            input.baseUrl,
            input.apiKey,
            resolveQwenTokenPlanIndividualTestModelId(input.modelId),
            proxyUrl,
          )
        }
...
async function fetchDeepSeekModels(
  ...
  const models: ChannelModel[] = items.map(createFetchedOpenAIChannelModel)
