...
export function createInitialState(meta: RunState['meta'] = {}): RunState {
  return {
    blocks: [],
...
export function reduce(state: RunState, payload: AgentStreamPayload): RunState {
  if (payload.kind === 'sdk_delta') {
    ...
    const existingSnapshot = state.partialAssistantSnapshots[uuid] ?? { blocks: {} }
    const blocks = { ...existingSnapshot.blocks }
    let next = state

    for (const delta of deltas) {
      switch (delta.type) {
        case 'start':
          for (const key of Object.keys(blocks)) delete blocks[Number(key)]
          next = {
            ...next,
            blocks: next.blocks.filter((block) => !(block.kind === 'text' && block.streaming)),
            reasoning: { content: '', active: false },
          }
          break
        case 'text_start':
          blocks[delta.contentIndex] = { type: 'text', content: '' }
          break
        case 'text_delta': {
          const previous = blocks[delta.contentIndex]
          const content = previous?.type === 'text' ? previous.content : ''
          blocks[delta.contentIndex] = { type: 'text', content: content + delta.delta }
          if (delta.delta) next = appendText(next, delta.delta)
          break
        }
        case 'text_end': {
          const previous = blocks[delta.contentIndex]
          const previousContent = previous?.type === 'text' ? previous.content : ''
          const remainder = cumulativeDelta(delta.content, previousContent)
          blocks[delta.contentIndex] = { type: 'text', content: delta.content }
          if (remainder) next = appendText(next, remainder)
          next = { ...next, blocks: closeStreamingText(next.blocks) }
          break
        }
        case 'thinking_start':
          blocks[delta.contentIndex] = { type: 'thinking', content: '' }
          break
        case 'thinking_delta': {
          const previous = blocks[delta.contentIndex]
          const content = previous?.type === 'thinking' ? previous.content : ''
          blocks[delta.contentIndex] = { type: 'thinking', content: content + delta.delta }
          if (delta.delta) next = appendThinking(next, delta.delta)
          break
        }
        case 'thinking_end': {
          const previous = blocks[delta.contentIndex]
          const previousContent = previous?.type === 'thinking' ? previous.content : ''
          const remainder = cumulativeDelta(delta.content, previousContent)
          blocks[delta.contentIndex] = { type: 'thinking', content: delta.content }
          if (remainder) next = appendThinking(next, remainder)
          next = { ...next, reasoning: { ...next.reasoning, active: false } }
          break
        }
        case 'toolcall_start':
        case 'toolcall_end': {
          const toolCall = delta.toolCall
          if (toolCall) next = startTool(next, toolCall.id, toolCall.name, toolCall.arguments ?? {})
          break
        }
        case 'toolcall_delta':
          break
