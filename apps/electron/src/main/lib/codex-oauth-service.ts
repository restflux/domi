...
/**
 * Pi 的 ModelRuntime 只要求 CredentialStore 的结构契约。OAuth 凭据仍由 Domi
...
/**
 * Pi 的公开 OAuth API 不接收 fetch 注入；Domi 通过 AsyncLocalStorage request scope
 * 为 ModelRuntime 内部 fetch 绑定应用代理，同时让本地 loopback 回调保持直连。
 */
  ...
  try {
    return await runWithOAuthProxyScope(async () => {
      ...
      const credentials = await runtime.login('openai-codex', 'oauth', {
        signal: abort.signal,
        prompt: async (prompt) => {
          // 当前 Domi 产品仍固定浏览器授权；这里只补齐 OAuth 内部请求的代理 scope。
