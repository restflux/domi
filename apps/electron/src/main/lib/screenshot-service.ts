...
import { SCREENSHOT_LIMITS } from '@domi/shared'
...
<meta charset="utf-8">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src 'self' data: blob: domi-file: https: http:; media-src 'self' data: blob: domi-file: https: http:; font-src 'self' data: https: http:; style-src 'unsafe-inline'; script-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'">
<style>${safeCss}</style>
<style>
...
.domi-screenshot-wrapper{padding:${SCREENSHOT_PADDING_TOP}px ${SCREENSHOT_PADDING_X}px;background:${bg};width:max-content;max-width:100%;margin:0 auto}
.domi-screenshot-sheet{width:max-content;max-width:100%;margin:0 auto;background:${bg}}
.domi-screenshot-sheet [contenteditable],
.domi-screenshot-sheet [contenteditable="false"]{outline:none}
.domi-screenshot-sheet .ProseMirror-selectednode,
.domi-screenshot-sheet .selectedCell::after{display:none!important}
</style></head><body class="${safeThemeClass}">
<div class="domi-screenshot-wrapper">
<main class="domi-screenshot-sheet">${safeHtml}</main>
</div>
</body></html>`
}
...
async function screenshotCapture(htmlContent: string, width: number): Promise<Buffer> {
  const tmpPath = join(tmpdir(), `domi-ss-${Date.now()}.html`)
  ...
  return withLock(async () => {
    try {
      ...
      const { canceled, filePath } = await dialog.showSaveDialog({
        title: '保存截图',
        defaultPath: join(homedir(), 'Desktop', `domi-${ts}.png`),
