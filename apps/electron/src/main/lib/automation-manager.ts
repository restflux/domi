 ...
 * - Pi 是唯一 runtime；仅显式 Pi 的旧记录可迁移，Claude/缺失 runtime 必须拒绝。
 * - 同时把 index.version bump 到当前值，避免下次启动反复迁移。
 * 返回是否发生改动，由调用方决定是否写回磁盘。
 */
export function assertPiOnlyAutomationRuntime(indexVersion: number, automation: { agentRuntime?: unknown }): void {
  const runtime = automation.agentRuntime
  if (indexVersion < INDEX_VERSION && runtime !== 'pi') {
    const label = runtime === undefined ? '缺失（旧版默认 Claude）' : String(runtime)
    throw new Error(`不支持的旧 Automation runtime: ${label}。请使用仍支持该 runtime 的旧版 Domi 处理该任务。`)
  }
  if (runtime !== undefined && runtime !== 'pi') {
    throw new Error(`不支持的旧 Automation runtime: ${String(runtime)}。请使用仍支持该 runtime 的旧版 Domi 处理该任务。`)
  }
}
...
function migrateLegacyFields(data: AutomationsIndex): boolean {
  ...
  for (const a of data.automations) {
    const legacyAutomation = a as Automation & { agentRuntime?: unknown }
    assertPiOnlyAutomationRuntime(data.version, legacyAutomation)
    if (legacyAutomation.agentRuntime === 'pi') {
      delete legacyAutomation.agentRuntime
      changed = true
    }
