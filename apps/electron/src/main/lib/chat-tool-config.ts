...
import { assertSafeHttpToolConfig } from './chat-tools/http-tool-credentials.ts'
import type { ChatToolsFileConfig, ChatToolState, ChatToolMeta } from '@domi/shared'
...
export function updateToolCredentials(toolId: string, credentials: Record<string, string>): void {
  const config = getChatToolsConfig()
  const isCustomTool = config.customTools.some((tool) => tool.id === toolId)
  config.toolCredentials[toolId] = isCustomTool
...
export function addCustomTool(meta: ChatToolMeta): void {
  if (meta.executorType === 'http') {
    if (!meta.httpConfig) throw new Error('自定义 HTTP 工具缺少 httpConfig')
    assertSafeHttpToolConfig(meta.httpConfig)
  }
