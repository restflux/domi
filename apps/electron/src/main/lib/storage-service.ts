...
import { INSTALLER_TEMP_DIR_NAME, PREVIEW_TEMP_DIR_NAME } from '@domi/shared'
import {
  getConfigDir,
  getAgentSessionsDir,
  getSdkConfigDir,
...
async function calcTempFilesCategory(): Promise<StorageCategory> {
  const previewDir = join(tmpdir(), PREVIEW_TEMP_DIR_NAME)
  const installerDir = join(app.getPath('temp'), INSTALLER_TEMP_DIR_NAME)
