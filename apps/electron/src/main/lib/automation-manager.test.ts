...
import { applyMaxRunsUpdate, assertPiOnlyAutomationRuntime, computeNextRunAt } from './automation-manager'

describe('Pi-only Automation runtime migration', () => {
  test('Given a pre-cut explicit Pi task When validating migration Then it is accepted', () => {
    expect(() => assertPiOnlyAutomationRuntime(2, { agentRuntime: 'pi' })).not.toThrow()
  })

  test('Given a pre-cut missing or Claude runtime task When validating migration Then both are rejected', () => {
    expect(() => assertPiOnlyAutomationRuntime(2, {}))
      .toThrow('不支持的旧 Automation runtime: 缺失（旧版默认 Claude）')
    expect(() => assertPiOnlyAutomationRuntime(2, { agentRuntime: 'claude' }))
      .toThrow('不支持的旧 Automation runtime: claude')
  })

  test('Given a Pi-only v3 task without runtime When validating migration Then it is accepted', () => {
    expect(() => assertPiOnlyAutomationRuntime(3, {})).not.toThrow()
  })
})
