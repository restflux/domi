...
- ${workflowDescription}`)
  ...
  if (ctx.sessionTarget) {
    ...
    if (ctx.sessionTarget.followupOnly) {
      sections.push([
        '## 交付后的执行与项目保护',
        '- 研究模式下，需要执行查询或诊断时先调用 `RequestDirectWorkflow` 申请本次执行，不要为了非项目修改操作申请下一轮 Worktree。执行模式或本次执行获批后，仍须遵守独立的项目保护。',
        '- 优先使用明确只读的工具或可证明无副作用的命令。未知脚本、重定向及可能产生项目文件的诊断命令仍可能被拒绝；执行授权不等于项目修改授权，也不自动授权远程数据库写入。会话工作台可以在执行授权下写入，但不代表可以修改项目。',
      ].join('\n\n'))
    }
...
用户要求根据意见修改时，把报告视为待判断的模型建议，核对当前原稿和版本后只采纳合理部分；需要取舍时询问用户，仍遵守研究/执行和宿主审批。不自动发起循环审改。`)
  }
