...
export function buildAgentReplyCard(result: FormattedAgentResult, subtitle?: string): Record<string, unknown> {
  ...
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: 'Domi Agent' },
...
export function buildErrorCard(errorMessage: string): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'Domi 错误' },
      template: 'red',
    },
    elements: [
...
export function buildWorkspaceListCard(
  ...
  return {
    ...
    elements: [
      ...
      {
        tag: 'note',
        elements: [{ tag: 'plain_text', content: '对话前可使用 /workspace <序号或名称> 选择项目' }],
      },
    ],
  }
}
...
export function buildHelpCard(): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'Domi Bot 命令' },
      template: 'blue',
    },
    elements: [
      {
        ...
        content: [
          ...
          '`/workspace` (`/ws`) `[序号或名称]` — 对话前查看或选择项目',
          '`/model` (`/m`) `[渠道 [模型]]` — 查看或切换渠道/模型',
          '',
          '首次对话前可先用 /workspace 选择项目；直接发送文本会自动创建或发送到当前会话。',
        ].join('\n'),
      },
    ],
  }
