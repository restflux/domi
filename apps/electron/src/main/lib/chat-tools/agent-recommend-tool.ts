/**
 * Work 模式推荐工具模块（Chat 模式）
 *
 * 智能识别适合 Work 模式的任务，推荐用户切换。
...
export const AGENT_RECOMMEND_TOOL_META: ChatToolMeta = {
  ...
  name: 'Work 模式推荐',
  description: '智能识别适合 Work 模式的任务，推荐用户切换',
  params: [
...
你有能力推荐用户切换到 Work 模式。**请积极主动地识别适合 Work 模式的任务**，在回复用户之前先调用 suggest_agent_mode 工具。

## Work 模式的能力

Work 模式拥有以下 Chat 模式不具备的能力：
...
- **reason 必须具体说明 Work 模式如何帮助用户更好地实现他们的目标**。结合用户的具体任务描述 Agent 会怎样做。例如："你想做桌面端 AI 产品调研，Work 模式可以通过 MCP 和 Skills 系统化地搜集各产品信息、自动整理对比分析，并输出结构化的调研报告文档。"
...
export const AGENT_RECOMMEND_TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    description: 'Proactively recommend switching to Work mode when the task involves research, coding, file operations, command execution, content creation, data analysis, or any multi-step work. Call this before responding whenever the task would meaningfully benefit from Agent capabilities.',
    parameters: {
      type: 'object',
      properties: {
        reason: {
          type: 'string',
          description: 'Specific explanation of how Work mode can better help the user achieve their goal',
        },
        suggestedPrompt: {
          type: 'string',
