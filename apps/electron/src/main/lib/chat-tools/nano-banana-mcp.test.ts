...
let buildPiNanoBananaTools: typeof import('./nano-banana-mcp').buildPiNanoBananaTools

beforeAll(async () => {
  ;({ buildPiNanoBananaTools } = await import('./nano-banana-mcp'))
})
...
describe('Nano Banana Pi custom tool', () => {
  test('Given a runtime-neutral generator When tool executes Then params and cwd are bridged into a Pi AgentToolResult', async () => {
    ...
    const tool = buildPiNanoBananaTool(
      sdk,
      'session-1',
      'D:/workspace/project',
    ...
    expect(tool.name).toBe('mcp__nano_banana__generate_image')
    expect(tool.description).toContain('regenerate')
    expect(tool.promptSnippet).toContain('call mcp__nano_banana__generate_image directly')
  ...
  test('Given output mode is omitted When tool executes Then generation defaults to session attachments', async () => {
    const calls: unknown[] = []
    const sdk = { defineTool: (definition: unknown) => definition } as unknown as typeof import('@earendil-works/pi-coding-agent')
    const tool = buildPiNanoBananaTool(sdk, 'session-default', 'D:/workspace/project', async (_prompt, _sessionId, options) => {
      calls.push(options)
  ...
  test('Given generator returns no image When tool executes Then textual output cannot be mistaken for completion', async () => {
    const sdk = { defineTool: (definition: unknown) => definition } as unknown as typeof import('@earendil-works/pi-coding-agent')
    const tool = buildPiNanoBananaTool(sdk, 'session-empty', undefined, async () => ({
      content: [{ type: 'text', text: '未生成图片内容' }],
    }))

    expect(tool.execute('tool-call-empty', { prompt: 'draw' }, undefined, undefined, {} as never))
      .rejects.toThrow('图片生成失败: 未生成图片内容')
  })

  test('Given generator failure When tool executes Then the result is explicitly marked as an error', async () => {
    const sdk = { defineTool: (definition: unknown) => definition } as unknown as typeof import('@earendil-works/pi-coding-agent')
    const tool = buildPiNanoBananaTool(sdk, 'session-2', undefined, async () => {
...
describe('buildPiNanoBananaTools（Agent 注入条件：仅凭能力中心开关+凭据）', () => {
  const sdk = { defineTool: (definition: unknown) => definition } as unknown as typeof import('@earendil-works/pi-coding-agent')
  ...
  test('Given 凭据已配置且工具页开关关闭 When 构建注入 Then 返回 mcp__nano_banana__generate_image（不依赖工具开关）', () => {
    mockedCredentials = { apiKey: 'test-key' }

    const tools = buildPiNanoBananaTools(sdk, 'session-nano-banana')

    expect(tools).toHaveLength(1)
    expect(tools[0]!.name).toBe('mcp__nano_banana__generate_image')
  })

  test('Given 未配置凭据 When 构建注入 Then 返回空数组', () => {
    mockedCredentials = {}

    expect(buildPiNanoBananaTools(sdk, 'session-nano-banana')).toEqual([])
  })
})
