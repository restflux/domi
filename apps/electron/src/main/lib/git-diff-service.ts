...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@domi/shared'
import { normalizePathForCompare } from '@domi/shared'
import type { ChangeSource, ChangedFileStatus } from '@domi/shared'
...
function toChangeCandidate(input: string): ChangeCandidate | null {
  ...
  try {
    const resolved = realpathSync.native(lexicalPath)
    ...
    if (stats.isFile()) {
      return {
        ...
        fileOnly: true,
        displayRoot: dirname(lexicalPath),
      }
    }
    if (stats.isDirectory()) {
      return {
        ...
        matchPath: normalizeComparablePath(resolved),
        fileOnly: false,
        displayRoot: lexicalPath,
      }
    }
  } catch {
    // 附加文件被删除后仍可能需要展示 git 删除记录；此时用 canonical 父目录找仓库、按文件精确匹配。
    const lexicalParent = dirname(lexicalPath)
    let parent = lexicalParent
    try { parent = realpathSync.native(lexicalParent) } catch { /* 保留 lexical fallback */ }
    const missingPath = join(parent, basename(lexicalPath))
    return {
      searchPath: parent,
      matchPath: normalizeComparablePath(missingPath),
      fileOnly: true,
      displayRoot: lexicalParent,
    }
  }
  return null
}
...
export async function getUnstagedChanges(
  ...
  // 候选路径用于过滤和投影：目录匹配子树，附加文件只匹配自身。
  const matchingCandidate = (absPath: string): ChangeCandidate | undefined => {
  ...
  const projectRelativePath = (candidate: ChangeCandidate, absPath: string): string => (
    // Projection math must use the canonical search root; displayRoot intentionally preserves
    // the caller's path spelling (for example a Windows 8.3 TEMP alias).
    relative(candidate.searchPath, absPath).replace(/\\/g, '/')
  )
...
export async function getDiffContents(dirPath: string, filePath: string, gitRoot?: string, baseRef?: string): Promise<{ oldContent: string; newContent: string } | null> {
  ...
  let oldContent = ''
  try {
    // `./` 让 revision:path 相对于当前项目 cwd 解析；root 可以是仓库子目录。
...
/**
 * 还原文件的 index + working tree。Active Session Target 显式传入 Session Base；Legacy 默认 HEAD。
 */
export async function revertFile(
  ...
  const sourceArgs = sourceRef ? [`--source=${sourceRef}`] : []
  const args = ['restore', ...sourceArgs, '--staged', '--worktree', '--', safePath]
  const result = await runGitCommand(args, root)
  if (result === null) {
    throw new Error(`还原失败: git ${args.join(' ')}`)
  }
}
 ...
 *
 * 用于安全校验：worktree 常被放在主仓库之外（如 ~/domi-dev/worktrees/xxx），
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@domi/shared').WorktreeInfo[]> {
  ...
  const worktrees: import('@domi/shared').WorktreeInfo[] = []
...
export async function getWorktreeChanges(
...
): Promise<import('@domi/shared').UnstagedChangesResult> {
  if (!existsSync(worktreePath)) {
    return { isGitRepo: false, files: [], untrackedFiles: [], gitRootNames: [] }
  }

  // Diff 是纯本地读取；调用方提供已确定的 Session Base OID 或显式 base ref，禁止隐式联网。
  // 确认是 git 仓库
  ...
  const gitRoot = normalizeGitRoot(realpathSync.native(toplevel))
  const projectRoot = normalizeGitRoot(realpathSync.native(worktreePath))
  const displayProjectRoot = normalizeGitRoot(resolve(worktreePath))
  const projectPrefix = relative(gitRoot, projectRoot).replace(/\\/g, '/').replace(/\/$/, '')
  if (projectPrefix === '..' || projectPrefix.startsWith('../') || isAbsolute(projectPrefix)) {
    return { isGitRepo: false, files: [], untrackedFiles: [], gitRootNames: [] }
  }
  const projectPathspec = projectPrefix || '.'
  const toProjectPath = (repoPath: string): string | null => {
    if (!projectPrefix) return repoPath
    const prefix = `${projectPrefix}/`
    return repoPath.startsWith(prefix) ? repoPath.slice(prefix.length) : null
  }
  const allFiles: import('@domi/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@domi/shared').ChangedFileEntry>()

  // Session Base 到最终 working tree 的单一净 Diff。一个 commit 改动若随后在 working tree
  // 被改回 B，必须完全抵消，不能把 B→HEAD 与 HEAD→I 的统计相加。
  const committedStatus = await runGitCommand(
    ['diff', baseBranch, '--name-status', '--', projectPathspec],
    gitRoot,
  )
  const committedNumstat = await runGitCommand(
    ['diff', baseBranch, '--numstat', '--', projectPathspec],
    gitRoot,
  )
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@domi/shared').ChangedFileStatus
      let filePath: string
      ...
      const projectedPath = toProjectPath(filePath)
      if (projectedPath === null) continue
      const entry: import('@domi/shared').ChangedFileEntry = {
        filePath: projectedPath,
        status,
        additions: stats.additions,
        deletions: stats.deletions,
        source: 'none',
        gitRoot: displayProjectRoot,
      }
      fileMap.set(projectedPath, entry)
    }
  }
  ...
  // 新文件（未追踪）
  const untrackedFiles: import('@domi/shared').UntrackedFileEntry[] = []
  const untrackedOutput = await runGitCommand(
    ['ls-files', '--others', '--exclude-standard', '--', projectPathspec],
    gitRoot,
  )
  if (untrackedOutput) {
    for (const rel of untrackedOutput.split('\n').filter(Boolean)) {
      const projectedPath = toProjectPath(rel)
      if (projectedPath !== null && !fileMap.has(projectedPath)) {
        untrackedFiles.push({ filePath: projectedPath, gitRoot: displayProjectRoot })
      }
    }
  }
