...
function checkoutRefRoot(checkoutId: string): string {
  ...
  return `refs/domi/session-checkouts/${key}`
}
...
function isApplyBaseStrategy(value: unknown): boolean {
  return value === 'recorded_base'
    || value === 'isolated_contains_local_head'
    || value === 'local_contains_isolated_head'
    || value === 'shared_merge_base'
}
...
function isPreviewReceipt(value: unknown): boolean {
  return isRecord(value)
    ...
    && isApplyBaseStrategy(value.baseStrategy)
...
function isJournal(value: unknown): boolean {
  ...
  return validOperation
    ...
    && (value.baseStrategy === undefined || isApplyBaseStrategy(value.baseStrategy))
...
function isManagedCheckout(value: unknown): boolean {
  return isRecord(value)
    ...
    && (value.checkpoints === undefined || (Array.isArray(value.checkpoints) && value.checkpoints.every(isCheckpoint)))
    && typeof value.sourceRef === 'string'
