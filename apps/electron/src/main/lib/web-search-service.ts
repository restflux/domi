 ...
 * Domi keeps these as app-hosted tools so Agent runtimes can use a stable,
 * provider-agnostic WebSearch/WebFetch surface even when the selected model does
 * not support native hosted web-search tools.
 */

import { getToolCredentials, getToolState } from './chat-tool-config.ts'
...
export async function searchWeb(options: WebSearchOptions): Promise<TavilySearchResponse> {
  ...
  return getManagedWebAccess().run(TAVILY_SEARCH_URL, 'web_search', async () => (
    postTavily<TavilySearchResponse>(TAVILY_SEARCH_URL, apiKey, {
      query,
...
export async function fetchWebPage(options: WebFetchOptions): Promise<TavilyExtractResponse> {
  ...
  return getManagedWebAccess().run(options.url, 'web_fetch', async (url) => (
    postTavily<TavilyExtractResponse>(TAVILY_EXTRACT_URL, apiKey, {
      urls: url,
      ...(prompt ? { query: prompt } : {}),
