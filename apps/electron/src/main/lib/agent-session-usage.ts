 ...
 *
 * 数据来源：~/.domi/agent-sessions/{id}.jsonl 里最后一条带 usage 的消息。
 * 优先级：
...
import { calculateContextUsageRatio, inferAgentSdkContextWindow, inferContextWindow, resolveSDKResultContextWindow } from '@domi/shared'
import type { SDKAssistantMessage, SDKResultMessage } from '@domi/shared'
...
export function getSessionContextUsageRatio(sessionId: string): number | undefined {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (msg.type === 'result') {
      ...
      const contextWindow = resolveSDKResultContextWindow(result)?.contextWindow
