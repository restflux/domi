...
import type { GitBashStatus, RuntimeStatus, ShellEnvironmentStatus, WslStatus } from '@domi/shared'
...
describe('Agent Windows Shell 运行环境', () => {
  ...
  test('Given Git Bash 与 WSL 均可用 When 用户显式选择 WSL Then 使用 WSL', () => {
    ...
    expect(result).toMatchObject({
      ...
      env: {
        DOMI_WINDOWS_SHELL: 'wsl',
        DOMI_WSL_DISTRO: 'Ubuntu-24.04',
