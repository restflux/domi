 ...
 *
 * 负责文件附件的本地存储、读取和删除。
 * 存储路径：~/.domi/attachments/{conversationId}/{uuid}.ext
 *
 * - 保存：base64 解码 → 写入文件
...
import { FILE_FILTERS, getComposerAttachmentDialogOptions } from './attachment-dialog-options'
 ...
 * ~/.domi/attachments/{conversationId}/{uuid}.ext
 *
 * @param input 保存附件参数
 ...
 *
 * 支持两种路径格式：
 * 1. 相对路径 {conversationId}/{uuid}.ext → 解析到 ~/.domi/attachments/
 * 2. 绝对路径（Agent 工作区附件）→ 需在 ~/.domi/ 目录下，直接读取
 *
 ...
 * 删除对话的全部附件
 *
 * 删除整个 ~/.domi/attachments/{conversationId}/ 目录。
 *
...
/**
 * 打开 Agent Composer 的文件或文件夹选择器。
 * 文件按附件语义读取，文件夹只返回路径，交由会话目录授权流程处理。
 */
export async function openFileOrFolderDialog(kind: ComposerAttachmentKind): Promise<FileOrFolderDialogResult> {
  const parentWindow = BrowserWindow.getFocusedWindow()
  const dialogOptions = getComposerAttachmentDialogOptions(kind)
  ...
  console.log(`[附件服务] Composer 选择器选择了 ${fileResult.files.length} 个内存附件，${fileResult.largeFiles?.length ?? 0} 个大文件引用，${directories.length} 个目录，${fileResult.skippedFiles?.length ?? 0} 个跳过`)
