...
function handleAgentEvent(sessionId: string, payload: AgentStreamPayload): void {
  if (payload.kind === 'domi_event') {
    handleDomiEvent(sessionId, payload.event)
...
function handleDomiEvent(sessionId: string, event: import('@domi/shared').DomiEvent): void {
  switch (event.type) {
...
function handleSdkMessage(sessionId: string, message: import('@domi/shared').SDKMessage): void {
  switch (message.type) {
    case 'assistant': {
      const aMsg = message as import('@domi/shared').SDKAssistantMessage
    ...
    case 'user': {
      const uMsg = message as import('@domi/shared').SDKUserMessage
      const session = sessions.get(sessionId)
      if (!session) break
    ...
    case 'result': {
      const rMsg = message as import('@domi/shared').SDKResultMessage
      const session = ensureSession(sessionId)
    ...
    case 'system': {
      const sMsg = message as import('@domi/shared').SDKSystemMessage
      const session = ensureSession(sessionId)
      switch (sMsg.subtype) {
        case 'task_started': {
          session.phase = 'running'
...
function requiresImmediateAgentIslandPush(payload: AgentStreamPayload): boolean {
  ...
  return message.type === 'result' || (message.type === 'assistant' && Boolean((message as import('@domi/shared').SDKAssistantMessage).error))
}
...
/**
 * 完成态的未读由主进程统一管理；错误和需要交互的 attention 不能被普通查看吞掉。
 */
function markAgentIslandSessionViewed(sessionId: string): void {
  if (markAgentIslandViewed(sessions.get(sessionId))) schedulePush()
}
