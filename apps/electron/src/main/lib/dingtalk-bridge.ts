 ...
 * - 消息路由到 Domi Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@domi/shared'
...
import { buildDingTalkGeneratedImagePayload } from './bridge-generated-image-payloads'
...
class DingTalkBridge {
    ...
    this.commandHandler = new BridgeCommandHandler({
      ...
      adapter: {
        sendText: async (chatId: string, text: string, meta, delivery) => {
          ...
          try {
            if (!webhook) throw new Error('没有可用的 sessionWebhook')
            ...
            if (!resp.ok) throw new Error(`钉钉文本回复失败: HTTP ${resp.status}`)
          } catch (error) {
            // 最终交付必须抛出给共用重试队列；普通状态提示保持原有 best-effort 行为。
            if (delivery) throw error
            console.error(`[钉钉 Bridge/${this.botConfig.name}] 发送消息异常:`, redactSensitiveLogValue(error))
          }
        },
        sendImage: async (chatId, image, meta) => {
          const ctx = meta as { sessionWebhook?: string } | undefined
          const webhook = ctx?.sessionWebhook ?? this.webhookCache.get(chatId)
          if (!webhook) throw new Error('没有可用的 sessionWebhook')
          ...
          const mediaId = await this.uploadDingTalkImage(data, image.filename, image.mediaType)
          const resp = await fetch(webhook, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(buildDingTalkGeneratedImagePayload(mediaId, image.filename)),
          })
          if (!resp.ok) throw new Error(`钉钉图片回复失败: HTTP ${resp.status}`)
        },
      },
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    // 先验证项目是否有效；Bot 独立默认失效时继续回退机器人共享默认，
    // 与 BridgeCommandHandler 的新会话解析顺序保持一致。
    const remoteDefaultWorkspaceId = resolveAgentRemoteDefaultWorkspaceId(getSettings())
    const preCheckWorkspaceId = this.botConfig.defaultWorkspaceId && getAgentWorkspace(this.botConfig.defaultWorkspaceId)
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, '请先在 Domi 设置中选择 Agent 渠道。')
      return
    }
  ...
  /** 上传图片并取得可用于机器人 markdown 的 media_id。 */
  private async uploadDingTalkImage(data: Buffer, filename: string, mediaType: string): Promise<string> {
    if (data.length === 0) throw new Error('生成图片内容为空')
    if (data.length > MAX_IMAGE_SIZE) throw new Error(`生成图片超过 ${MAX_IMAGE_SIZE} 字节限制`)
    const token = await this.getAccessToken()
    const form = new FormData()
    form.append('media', new Blob([new Uint8Array(data)], { type: mediaType }), filename || 'generated-image.png')
    const resp = await fetch(`https://oapi.dingtalk.com/media/upload?access_token=${encodeURIComponent(token)}&type=image`, {
      method: 'POST',
      body: form,
    })
    const payload = await resp.json().catch(() => ({})) as { errcode?: number; errmsg?: string; media_id?: string }
    if (!resp.ok || (payload.errcode != null && payload.errcode !== 0) || !payload.media_id) {
      throw new Error(`钉钉图片上传失败: HTTP ${resp.status} ${payload.errcode ?? ''} ${payload.errmsg ?? ''}`.trim())
    }
    return payload.media_id.startsWith('@') ? payload.media_id.slice(1) : payload.media_id
  }
