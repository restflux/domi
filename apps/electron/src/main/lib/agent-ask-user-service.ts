...
interface PendingAskUser {
  ...
  onAnswered?: (request: AskUserRequest, answers: Record<string, string>) => void
}
...
export class AgentAskUserService {
  ...
  handleAskUserQuestion(
    ...
    return new Promise<PermissionResult>((resolve) => {
      this.pendingRequests.set(request.requestId, { resolve, request, onAnswered })
  ...
  respondToAskUser(requestId: string, answers: Record<string, string>): string | null {
    ...
    // 回调必须在 Promise resolve 前同步执行，确保用户交互消息排在 Agent 续写之前。
    try {
      pending.onAnswered?.(pending.request, answers)
    } catch (error) {
      console.error('[AgentAskUserService] 处理 AskUser 回答回调失败:', error)
    }
