 ...
 * - 发送消息/输入状态
 *
 * 消息路由到 Domi Agent，回复通过 iLink API 发送。
 */
...
import { WECHAT_IPC_CHANNELS, WECHAT_ITEM_TYPE, WECHAT_MESSAGE_TYPE, WECHAT_MESSAGE_STATE } from '@domi/shared'
...
import { buildWeChatGeneratedImageItem } from './bridge-generated-image-payloads'
import {
...
function encryptAesEcbWithKey(plaintext: Buffer, key: Buffer): Buffer {
...
class ILinkClient {
  ...
  async sendMessage(
    ...
    contextToken: string,
    clientId = `domi_${crypto.randomUUID()}`,
  ): Promise<void> {
    const response = await this.post<WeChatSendMessageResponse>('/ilink/bot/sendmessage', {
      msg: {
    ...
    assertWeChatSendMessageSucceeded(response)
  }
  ...
  /** 上传图片到微信 CDN，并返回 IMAGE 消息所需的加密引用。 */
  async uploadImage(toUserId: string, data: Buffer): Promise<UploadedWeChatImage> {
    if (data.length === 0) throw new WeChatDeliveryError('生成图片内容为空', false)
    if (data.length > MAX_FILE_SIZE) {
      throw new WeChatDeliveryError(`生成图片超过 ${MAX_FILE_SIZE} 字节限制`, false)
    }

    const aesKey = crypto.randomBytes(16)
    const fileKey = crypto.randomBytes(16).toString('hex')
    const ciphertext = encryptAesEcbWithKey(data, aesKey)
    const upload = await this.post<GetUploadUrlResponse>('/ilink/bot/getuploadurl', {
      filekey: fileKey,
      media_type: 1,
      to_user_id: toUserId,
      rawsize: data.length,
    ...
    assertWeChatSendMessageSucceeded(upload, '图片上传地址获取')
    if (!upload.upload_param && !upload.upload_full_url) throw new Error('微信未返回图片上传参数')
    ...
    if (!isAllowedCdnUrl(uploadUrl)) throw new Error(`图片上传 URL 域名不在白名单: ${uploadUrl}`)

    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), DOWNLOAD_MEDIA_TIMEOUT_MS)
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`微信 CDN 图片上传失败: HTTP ${response.status} ${body.slice(0, 200)}`)
      }
      const encryptQueryParam = response.headers.get('x-encrypted-param')
      if (!encryptQueryParam) throw new Error('微信 CDN 未返回 x-encrypted-param')
      return {
        encryptQueryParam,
  ...
  /** 发送图片消息（便捷方法） */
  async sendImage(toUserId: string, data: Buffer, contextToken: string, clientId?: string): Promise<void> {
    const uploaded = await this.uploadImage(toUserId, data)
    return this.sendMessage(toUserId, [
      buildWeChatGeneratedImageItem(
        uploaded.encryptQueryParam,
        uploaded.aesKey,
        uploaded.encryptedFileSize,
      ),
    ], contextToken, clientId)
  }
...
class WeChatBridge {
  ...
  private commandHandler = new BridgeCommandHandler({
    platformName: '微信',
    adapter: {
      sendText: async (chatId: string, text: string, meta, delivery) => {
        if (!this.client) throw new Error('微信 Bridge 当前未连接')
        const ctx = meta as { contextToken?: string } | undefined
        const contextToken = ctx?.contextToken ?? ''
        if (!contextToken) throw new WeChatDeliveryError('微信消息缺少 context_token', false)
        // 微信单条消息有长度限制，超长分段；每个分片在跨重试时保持自己的稳定 client_id。
        const MAX_LEN = 4000
        const chunks = text.length <= MAX_LEN
          ? [text]
          : text.match(new RegExp(`.{1,${MAX_LEN}}`, 'gs')) ?? [text]
        for (let index = 0; index < chunks.length; index++) {
          const clientId = delivery ? `${delivery.deliveryId}:chunk:${index}` : undefined
          await this.client.sendText(chatId, chunks[index]!, contextToken, clientId)
        }
      },
      sendImage: async (chatId, image, meta, delivery) => {
        if (!this.client) throw new Error('微信 Bridge 当前未连接')
        const ctx = meta as { contextToken?: string } | undefined
        const contextToken = ctx?.contextToken ?? ''
        if (!contextToken) throw new WeChatDeliveryError('微信图片消息缺少 context_token', false)
        let data: Buffer
        try {
          data = image.data
          ...
          throw new WeChatDeliveryError(`读取生成图片失败: ${detail}`, false)
        }
        await this.client.sendImage(chatId, data, contextToken, delivery?.deliveryId)
      },
      isRetryableDeliveryError: isRetryableWeChatDeliveryError,
    },
  ...
  logout(): void {
    this.stop()
    this.commandHandler.discardTransientState()
  ...
  private async handleMessage(msg: WeChatIncomingMessage): Promise<void> {
    ...
    if (!binding) {
      await this.client.sendText(chatId, '请先在 Domi 设置中选择 Agent 渠道。', contextToken)
      return
    }
