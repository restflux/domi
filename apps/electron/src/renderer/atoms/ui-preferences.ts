...
export async function initializeUiPreferences(
  ...
  setWorkSidebarPreferences?: (preferences: WorkSidebarPreferences) => void,
): Promise<void> {
  try {
    ...
    setWorkSidebarPreferences?.({
      ...
      ...settings.workSidebarPreferences,
    })
  } catch (error) {
...
/** 更新 Work 项目会话列表分组与排序偏好并持久化。 */
export async function updateWorkSidebarPreferences(preferences: WorkSidebarPreferences): Promise<void> {
  await window.electronAPI.updateSettings({ workSidebarPreferences: preferences })
}
