...
import type { SystemPromptConfig, SystemPrompt } from '@domi/shared'
...
export const promptConfigAtom = atom<SystemPromptConfig>({
  prompts: [BUILTIN_DEFAULT_PROMPT, BUILTIN_WORK_PRODUCT_DELIVERY_PROMPT],
...
export const selectedPromptIdAtom = atomWithStorage<string>(
  'domi-selected-system-prompt-id',
  BUILTIN_DEFAULT_ID
)

/** 提示词管理页当前选中的 Work 提示词 ID。 */
export const selectedWorkPromptIdAtom = atomWithStorage<string>(
  'domi-selected-work-system-prompt-id',
  BUILTIN_WORK_PRODUCT_DELIVERY_ID,
)

/** Chat 提示词列表（派生只读） */
export const promptListAtom = atom<SystemPrompt[]>(
  (get) => get(promptConfigAtom).prompts.filter((prompt) => prompt.scope === 'chat')
)
...
export const selectedPromptAtom = atom<SystemPrompt | undefined>((get) => {
  ...
  return config.prompts.find((p) => p.id === selectedId && p.scope === 'chat')
})
...
export function resolveSystemMessage(
  ...
  const prompt = config.prompts.find((p) => p.id === promptId && p.scope === 'chat')
