...
export type SettingsTab = 'general' | 'channels' | 'vision-relay' | 'proxy' | 'appearance' | 'about' | 'prompts' | 'tools' | 'bots' | 'tutorial' | 'shortcuts' | 'voice-input' | 'migration' | 'storage' | 'usage'
export type ToolSettingsFocus = 'memory' | 'web-search' | 'nano-banana' | 'gpt-image' | 'custom-tools'

/** 当前设置标签页（不持久化，每次打开设置默认显示渠道） */
export const settingsTabAtom = atom<SettingsTab>('channels')

/** AI 工具设置页的目标配置区，用于从内置 MCP 详情直达对应配置 */
