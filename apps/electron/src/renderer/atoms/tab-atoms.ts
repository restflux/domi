 ...
 *
 * 顶部只保留当前会话；文件预览与草稿均由 Right Workspace 承载。
...
export type ScratchPadSaveState = 'loading' | 'saving' | 'saved' | 'error'
/** 草稿持久化状态，供 Right Workspace 上下文栏反馈。 */
export const scratchPadSaveStateAtom = atom<ScratchPadSaveState>('loading')
...
export function getPersistableTabState(
  ...
  const persistentActiveTabId = activeTab && persistentTabs.some((tab) => tab.id === activeTab.id)
    ? activeTab.id
...
export function openTab(
  ...
  // 兼容旧调用：草稿已迁入 Right Workspace，不再创建顶部入口。
  if (item.type === 'scratch') {
    const fallback = tabs.filter((tab) => tab.id !== SCRATCH_PAD_ID && !isPreviewTab(tab)).at(-1)
    return {
      tabs: fallback ? [fallback] : [],
...
export function closeTab(
  ...
  if (activeTabId === tabId || (boundPreviewId !== null && activeTabId === boundPreviewId)) {
    const ownerSessionTab = isPreviewTab(closingTab)
      ? newTabs.find((tab) => isSessionTab(tab) && tab.sessionId === closingTab.sessionId)
      : undefined
    if (ownerSessionTab) {
      newActiveTabId = ownerSessionTab.id
