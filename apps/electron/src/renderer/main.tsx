...
import { useGlobalBrowserListeners } from './hooks/useGlobalBrowserListeners'
import { useGlobalTerminalListeners } from './hooks/useGlobalTerminalListeners.ts'
import { useGlobalChatListeners } from './hooks/useGlobalChatListeners'
import { useGlobalWorkActivity } from './hooks/useGlobalWorkActivity'
import { tabsAtom, activeTabIdAtom, getPersistableTabState, scratchPadContentAtom, scratchPadLoadedAtom, scratchPadSaveStateAtom, SCRATCH_PAD_ID } from './atoms/tab-atoms'
...
import type { FeishuBotBridgeState, FeishuBridgeState, DingTalkBotBridgeState, DingTalkBridgeState } from '@domi/shared'
import { Toaster } from './components/ui/sonner'
import { EditableContextMenu } from './components/ui/editable-context-menu'
import { toast } from 'sonner'
import { diffCapabilities } from '@domi/shared'
import type { WorkspaceCapabilities } from '@domi/shared'
import { showCapabilityChangeToasts } from './lib/capabilities-toast'
import { GlobalShortcuts } from './components/shortcuts/GlobalShortcuts'
import { WorkActivityNotificationInitializer } from './hooks/useWorkActivityNotifications'
...
if (isMainWindow || isPlanningWindow) {
  document.documentElement.classList.add('domi-main-window')
}
...
function AgentSettingsInitializer(): null {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Pi 支持所有协议；默认渠道只需处于启用状态。
      if (settings.agentChannelId && selectedChannel?.enabled) {
        ...
        console.warn('[AgentSettings] agentChannelId 指向已禁用或不存在的渠道，清除')
...
function NotificationsInitializer(): null {
  ...
  const setAttentionEnabled = useSetAtom(workActivityAttentionNotificationsEnabledAtom)
  const setCompletionEnabled = useSetAtom(workActivityCompletionNotificationsEnabledAtom)
  ...
  }, [setAttentionEnabled, setCompletionEnabled, setEnabled, setSoundEnabled, setSounds])
...
function DockBadgeInitializer(): null {
  ...
  useEffect(() => {
    const clearActiveSessionBadge = (): void => {
      ...
      // Scratch Pad 会保留 currentAgentSessionId；必须以实际激活的 Agent/预览 Tab 为准。
...
function BrowserListenersInitializer(): null {
  useGlobalBrowserListeners()
  return null
}

function TerminalListenersInitializer(): null {
 ...
 * 工作动态初始化组件
 *
 * 主窗口只挂载一次，中央页面和左侧概览共享同一份宿主投影与失效刷新。
 */
function WorkActivityInitializer(): null {
  useGlobalWorkActivity()
  return null
}
    ...
    Promise.all([
      ...
      if (validTabs.length === 0) {
        if (settings.scratchPadActive) {
          window.electronAPI.updateSettings({ scratchPadActive: false }).catch(() => {})
        }
        restoredRef.current = true
        return
      }
      ...
      if (activeTab) {
        if (activeTab.type === 'chat') {
          ...
          // 旧版关闭时停留在 Scratch Tab：升级后迁移为右侧工作区草稿工具。
          if (settings.scratchPadActive) {
            store.set(rightWorkspaceSessionStateMapAtom, (current) => (
              activateSessionRightWorkspaceTab(current, activeTab.sessionId, 'scratch')
            ))
 ...
 *
 * 启动时从磁盘加载 scratch-pad.md 内容并自动保存。
 * 同时清理旧版本残留的 Scratch Tab 与 scratchPadActive 设置。
 */
  ...
  useEffect(() => {
    const init = async (): Promise<void> => {
      try {
        ...
        store.set(scratchPadSaveStateAtom, 'saved')

        const currentTabs = store.get(tabsAtom)
        const migratedTabs = currentTabs.filter((tab) => tab.id !== SCRATCH_PAD_ID)
        if (migratedTabs.length !== currentTabs.length) {
          store.set(tabsAtom, migratedTabs)
          if (store.get(activeTabIdAtom) === SCRATCH_PAD_ID) {
            store.set(activeTabIdAtom, migratedTabs.at(-1)?.id ?? null)
          }
        }
        console.log('[ScratchPad] 初始化完成，已加载内容:', !!loadedMd)
      } catch (err) {
        store.set(scratchPadSaveStateAtom, 'error')
  ...
  useEffect(() => {
    const save = (): void => {
      ...
      if (window.electronAPI.saveScratchPad) {
        ...
        store.set(scratchPadSaveStateAtom, 'saving')
        window.electronAPI.saveScratchPad(md).then((ok) => {
          store.set(scratchPadSaveStateAtom, ok ? 'saved' : 'error')
...
if (isQuickTaskWindow) {
  ...
  ReactDOM.createRoot(document.getElementById('root')!).render(
    <React.StrictMode>
      ...
      <BrowserListenersInitializer />
      <TerminalListenersInitializer />
      <WorkActivityInitializer />
      <WorkActivityNotificationInitializer />
      <ChatToolInitializer />
      <AutomationInitializer />
      <PlanningInitializer />
      <FeishuInitializer />
