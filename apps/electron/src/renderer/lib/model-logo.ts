...
// 兜底图标：使用 Domi 自有品牌 Logo。
// 当模型 ID 与供应商类型都无法匹配到品牌图标时，统一回退到 Domi Mark。
...
// OpenCode
import OpenCodeLogo from '@/assets/models/opencode.png'
...
const PROVIDER_LOGO_MAP: Record<ProviderType, string> = {
  ...
  'opencode-go-openai': OpenCodeLogo,
  zhipu: ZhipuLogo,
  ...
  'qwen-token-plan-individual': QwenLogo,
  xiaomi: XiaomiLogo,
 ...
 * 根据模型 ID 获取 Logo
 *
 * 仅按模型 ID 正则匹配品牌图标；匹配不到时直接回退到 Domi 默认图标，
 * 不再借用渠道供应商的品牌 Logo（避免第三方渠道上的自有模型被误标为
 * OpenAI / Claude 等品牌）。
 *
 * 渠道级品牌识别请使用 getChannelLogo / getProviderLogo。
 *
 * @param modelId 模型 ID
 * @param _provider 保留参数以兼容既有调用方；不再参与 Logo 解析
 */
