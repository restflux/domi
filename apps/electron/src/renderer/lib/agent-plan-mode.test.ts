import { describe, expect, test } from 'bun:test'
import {
  getDisplayedPermissionMode,
  getPlanModeChangeFromToolName,
  shouldClearPlanModeAfterStreamComplete,
  updatePlanModeSessionSet,
} from './agent-plan-mode'

describe('Agent 计划阶段状态', () => {
  test('Given EnterPlanMode 工具 When 解析计划状态 Then 标记进入计划阶段', () => {
    expect(getPlanModeChangeFromToolName('EnterPlanMode')).toEqual({
      active: true,
      source: 'tool',
    })
  })

  test('Given ExitPlanMode 工具 When 解析计划状态 Then 不直接标记离开计划阶段', () => {
    expect(getPlanModeChangeFromToolName('ExitPlanMode')).toBeNull()
  })

  test('Given 普通工具 When 解析计划状态 Then 不产生状态变化', () => {
    expect(getPlanModeChangeFromToolName('Read')).toBeNull()
  })

  test('Given 会话进入计划阶段 When 更新集合 Then 只新增目标会话', () => {
    const prev = new Set(['session-a'])
    const next = updatePlanModeSessionSet(prev, 'session-b', true)

    expect([...next].sort()).toEqual(['session-a', 'session-b'])
    expect(next).not.toBe(prev)
  })

  test('Given 会话离开计划阶段 When 更新集合 Then 只移除目标会话', () => {
    const prev = new Set(['session-a', 'session-b'])
    const next = updatePlanModeSessionSet(prev, 'session-b', false)

    expect([...next]).toEqual(['session-a'])
    expect(next).not.toBe(prev)
  })

  test('Given 状态没有变化 When 更新集合 Then 复用原集合', () => {
    const prev = new Set(['session-a'])

    expect(updatePlanModeSessionSet(prev, 'session-a', true)).toBe(prev)
    expect(updatePlanModeSessionSet(prev, 'session-b', false)).toBe(prev)
  })

  test.each([
    ['successful completion', 100, 100],
    ['execution failure', 100, 100],
    ['user stop', 100, 100],
  ] as const)('Given /plan run %s When the exact run ends Then restore the persistent mode', (_case, current, completed) => {
    expect(shouldClearPlanModeAfterStreamComplete(current, completed, false)).toBe(true)
  })

  test('Given /plan run enters background-task idle When the channel stays open Then keep the run-local Plan state', () => {
    expect(shouldClearPlanModeAfterStreamComplete(100, 100, true)).toBe(false)
  })

  test('Given a replacement run is already active When the old /plan completion arrives Then keep the new run state', () => {
    expect(shouldClearPlanModeAfterStreamComplete(200, 100, false)).toBe(false)
    expect(shouldClearPlanModeAfterStreamComplete(200, undefined, false)).toBe(false)
  })

  test('Given no newer run timestamp When an abnormal completion arrives Then fail closed by clearing Plan state', () => {
    expect(shouldClearPlanModeAfterStreamComplete(undefined, 100, false)).toBe(true)
  })

  test('Given Agent 自己进入计划阶段 When 权限按钮显示模式 Then 优先显示计划模式', () => {
    expect(getDisplayedPermissionMode('bypassPermissions', true)).toBe('plan')
  })

  test('Given Agent 不在计划阶段 When 权限按钮显示模式 Then 保留真实权限模式', () => {
    expect(getDisplayedPermissionMode('bypassPermissions', false)).toBe('bypassPermissions')
    expect(getDisplayedPermissionMode('plan', false)).toBe('plan')
  })
})
