...
/**
 * 侧边栏「新会话」项目选择顺序：当前项目优先，其余保持用户的项目排序。
 *
 * 这样主按钮可以直接在当前项目创建，下拉菜单也把最可能使用的项目放在首位，
 * 不要求用户再去项目行里寻找 hover 后才出现的小加号。
 */
export function prioritizeCurrentWorkspace(
  ...
  return [current, ...workspaces.filter((workspace) => workspace.id !== currentWorkspaceId)]
}
 ...
 * 找出指定项目中可复用的未发送草稿会话。
 *
 * 草稿输入按 sessionId 保存；如果“新建会话”总是生成新 ID，用户切走后再次点击
 * 新建就会落到空输入框。这里优先复用已有文字的草稿，其次复用最新空草稿，既恢复
 * 用户输入，也避免积累不可见的空会话。自动任务与委派子会话不参与手动新建复用。
 */
export function findReusableDraftAgentSession(
  sessions: readonly AgentSessionMeta[],
  draftSessionIds: ReadonlySet<string>,
  draftContents: ReadonlyMap<string, string>,
  ...
  return candidates.find((session) => (draftContents.get(session.id) ?? '').trim() !== '')
    ?? candidates[0]
}
...
/**
 * 用户发送消息时立即恢复会话活跃状态，并按本次发送时间刷新列表顺序。
 *
 * 主进程会在 Agent run 启动时持久化相同语义；这里负责发送瞬间的 UI 反馈，
 * 避免已归档会话在运行期间仍被归档过滤器隐藏，或继续停留在历史位置。
 */
export function reactivateAgentSessionForSend(
  sessions: readonly AgentSessionMeta[],
  sessionId: string,
  updatedAt = Date.now(),
): AgentSessionMeta[] {
  ...
  return replaceAgentSessionInFreshnessOrder(sessions, {
    ...current,
    archived: false,
    updatedAt,
  })
