...
import { buildExternalAgentRunActivation, shouldActivateExternalAgentRun, shouldForegroundExternalAgentRun } from './external-agent-run'
...
describe('外部 Agent 运行激活', () => {
  test('只有仍停留在 origin 且携带主进程 token 的 Worktree handoff 会自动切换', () => {
    ...
    expect(shouldForegroundExternalAgentRun({ source: 'worktree_handoff', originSessionId: 'parent', currentSessionId: 'parent' })).toBe(false)
    expect(shouldForegroundExternalAgentRun({ source: 'delegation', originSessionId: 'parent', activationToken: 'nonce', currentSessionId: 'parent' })).toBe(false)
  })
