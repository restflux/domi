...
describe('Agent mention serialization', () => {
  test('serializes a confirmed Slash command as the original command text', () => {
    const markdown = withHtmlDocument(() => htmlToMarkdown([
      '<p>',
      '<span data-type="mention" data-id="image" data-label="/image" data-mention-suggestion-char="/" data-command-menu-mention="true">/image</span> ',
      '生成设计稿',
      '</p>',
    ].join('')))

    expect(markdown).toBe('/image 生成设计稿')
  })
