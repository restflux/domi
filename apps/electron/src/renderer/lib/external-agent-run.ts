import type { AgentExternalRunSource, AgentSessionMeta } from '@domi/shared'
...
export function shouldForegroundExternalAgentRun(input: {
  source: AgentExternalRunSource
  originSessionId?: string
  activationToken?: string
  currentSessionId?: string | null
}): boolean {
  return input.source === 'worktree_handoff'
    && typeof input.activationToken === 'string'
    && input.activationToken.length > 0
    && input.originSessionId === input.currentSessionId
}
