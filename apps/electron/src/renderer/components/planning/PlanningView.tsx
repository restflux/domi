...
import { PLANNING_CONFLICT_ERROR } from '@domi/shared'
import type { PlanningGroup, Todo } from '@domi/shared'
...
import {
  ...
  type RecoverableTodoDetailDraft,
  type TodoNotesSaveRequestToken,
  type TodoNotesSaveStatus,
} from '@/lib/todo-notes-save-state'
...
function TodoWorkspace({ standalone = false }: { standalone?: boolean } = {}): React.ReactElement {
  ...
  const todoConflictRef = React.useRef(false)
  ...
  const notesSaveInFlightRef = React.useRef(new Map<string, Promise<Todo | undefined>>())
  const todoMutationQueueRef = React.useRef(createKeyedSerialTaskQueue())
  const todoMutationVersionRef = React.useRef(new Map<string, number>())
  const notesSaveGenerationRef = React.useRef(0)
  const latestNotesSaveGenerationRef = React.useRef(new Map<string, number>())
  const [notesSaveStatus, setNotesSaveStatus] = React.useState<TodoNotesSaveStatus | null>(null)
  const mountedRef = React.useRef(true)
  const detailTransitionRef = React.useRef(false)
  const approvedExternalSelectionRef = React.useRef<string | null | undefined>(undefined)
  const pendingExternalSelectionRef = React.useRef<string | null | undefined>(undefined)
  const externalSelectionTransitionRef = React.useRef(false)
  const flushDetailDraftRef = React.useRef<(options?: { showStatus?: boolean }) => Promise<boolean>>(async () => true)
  const saveDetailNotesRef = React.useRef<(options?: { showStatus?: boolean }) => Promise<boolean>>(async () => true)
  const persistDetachedDraftRef = React.useRef<(snapshot: RecoverableTodoDetailDraft) => Promise<boolean>>(async () => true)
  ...
  const applyTodoDetailDraft = React.useCallback((todo: Todo): void => {
    const savedTitle = todo.title
    const savedNotes = todo.notes ?? ''
    const recovered = loadRecoverableTodoDetailDraft(todo.id)
    const title = recovered?.title ?? savedTitle
    const notes = recovered?.notes ?? savedNotes
    const recoveredConflict = Boolean(recovered && recovered.expectedUpdatedAt !== todo.updatedAt)
    selectionEpochRef.current += 1
    selectedTodoIdRef.current = todo.id
    todoBaseUpdatedAtRef.current = todo.updatedAt
    todoMutationVersionRef.current.set(todo.id, Math.max(todoMutationVersionRef.current.get(todo.id) ?? 0, todo.updatedAt))
    detailTitleRef.current = title
    detailNotesRef.current = notes
    savedDetailTitleRef.current = savedTitle
    savedDetailNotesRef.current = savedNotes
    todoConflictRef.current = recoveredConflict
    setDetailTitle(title)
    setDetailNotes(notes)
    setTodoConflict(recoveredConflict)
    if (recovered && !recoveredConflict) {
      setTimeout(() => { void flushDetailDraftRef.current() }, 0)
    }
  }, [])

  const captureTodoDetailDraft = React.useCallback((): RecoverableTodoDetailDraft | null => {
    const todoId = selectedTodoIdRef.current
    const expectedUpdatedAt = todoBaseUpdatedAtRef.current
    if (!todoId || expectedUpdatedAt === null) return null
    return {
      todoId,
      expectedUpdatedAt,
      title: detailTitleRef.current,
      savedTitle: savedDetailTitleRef.current,
      notes: detailNotesRef.current,
      savedNotes: savedDetailNotesRef.current,
    }
  }, [])
  ...
  React.useEffect(() => {
    const requestedTodoId = selected?.id ?? null
    const currentTodoId = selectedTodoIdRef.current

    if (approvedExternalSelectionRef.current !== undefined && approvedExternalSelectionRef.current === requestedTodoId) {
      approvedExternalSelectionRef.current = undefined
      if (!selected) {
        selectionEpochRef.current += 1
    ...
    // 外部提醒等入口只能“请求”切换。先回滚 atom，等当前草稿完整 flush 后再提交目标选择。
    if (currentTodoId && currentTodoId !== requestedTodoId) {
      pendingExternalSelectionRef.current = requestedTodoId
      setSelectedId(currentTodoId)
      if (todoConflictRef.current) {
        pendingExternalSelectionRef.current = undefined
        toast.error('请先处理 Todo 冲突，再切换任务')
        return
      }
      if (externalSelectionTransitionRef.current) return

      externalSelectionTransitionRef.current = true
      detailTransitionRef.current = true
      void (async () => {
        const flushed = await flushDetailDraftRef.current()
        const targetTodoId = pendingExternalSelectionRef.current
        pendingExternalSelectionRef.current = undefined
        externalSelectionTransitionRef.current = false
        detailTransitionRef.current = false
        if (!flushed || targetTodoId === undefined) return
        approvedExternalSelectionRef.current = targetTodoId
        setSelectedId(targetTodoId)
      })()
      return
    }
    ...
    if (hasDirtyDraft) {
      todoConflictRef.current = true
      setTodoConflict(true)
      return
    }
    applyTodoDetailDraft(selected)
  }, [applyTodoDetailDraft, selected?.id, selected?.updatedAt, setSelectedId])

  type TodoUpdateOptions = {
    savedField?: 'title' | 'notes'
    silentNotesSync?: boolean
    notesSnapshot?: string
    notesRequest?: TodoNotesSaveRequestToken
  }

  const enqueueTodoMutation = React.useCallback(<T,>(
    id: string,
    operation: () => Promise<T>,
  ): Promise<T> => todoMutationQueueRef.current.enqueue(id, operation), [])

  const updateTodo = React.useCallback(async (
    id: string,
    input: Omit<Parameters<typeof window.electronAPI.updateTodo>[0], 'id'>,
    options: TodoUpdateOptions = {},
  ): Promise<Todo | undefined> => enqueueTodoMutation(id, async () => {
    const requestIsCurrent = (): boolean => !options.notesRequest || isCurrentTodoNotesSaveRequest(
      options.notesRequest,
      selectedTodoIdRef.current,
      selectionEpochRef.current,
      latestNotesSaveGenerationRef.current.get(options.notesRequest.todoId),
    )

    const latestVersion = todoMutationVersionRef.current.get(id)
    const requestInput = input.expectedUpdatedAt !== undefined && latestVersion !== undefined
      ? { ...input, expectedUpdatedAt: latestVersion }
      : input

    try {
      const updated = await window.electronAPI.updateTodo({ id, ...requestInput })
      if (updated) {
        todoMutationVersionRef.current.set(id, updated.updatedAt)
        setTodos((items) => items.map((item) => item.id === id ? updated : item))
        if (selectedTodoIdRef.current === id && requestIsCurrent()) {
          todoBaseUpdatedAtRef.current = updated.updatedAt
          if (options.savedField === 'title') {
            detailTitleRef.current = updated.title
            savedDetailTitleRef.current = updated.title
            if (mountedRef.current) setDetailTitle(updated.title)
          }
          if (options.savedField === 'notes') {
            const snapshot = options.notesSnapshot ?? (updated.notes ?? '')
            const hasNewerDraft = detailNotesRef.current !== snapshot
            savedDetailNotesRef.current = snapshot
            if (!hasNewerDraft) {
              detailNotesRef.current = snapshot
              if (!options.silentNotesSync && mountedRef.current) setDetailNotes(updated.notes ?? '')
            }
          }
          if (detailTitleRef.current === savedDetailTitleRef.current && detailNotesRef.current === savedDetailNotesRef.current) {
            clearRecoverableTodoDetailDraft(id)
          }
        }
      }
      return updated
    } catch (error) {
      const surfaceError = requestIsCurrent()
      if (error instanceof Error && error.message.includes(PLANNING_CONFLICT_ERROR)) {
        if (surfaceError && selectedTodoIdRef.current === id) {
          todoConflictRef.current = true
          if (mountedRef.current) setTodoConflict(true)
          toast.error('Todo 已在其他窗口更新，请重新加载后再试')
        }
      } else {
        console.error('[任务/日程] 更新 Todo 失败:', error)
        if (surfaceError) toast.error('更新 Todo 失败')
      }
      return undefined
    }
  }), [enqueueTodoMutation, setTodos])

  const persistDetachedDraft = React.useCallback(async (
    snapshot: RecoverableTodoDetailDraft,
  ): Promise<boolean> => {
    const persist = async (
      input: Omit<Parameters<typeof window.electronAPI.updateTodo>[0], 'id' | 'expectedUpdatedAt'>,
    ): Promise<Todo> => enqueueTodoMutation(snapshot.todoId, async () => {
      const expectedUpdatedAt = todoMutationVersionRef.current.get(snapshot.todoId) ?? snapshot.expectedUpdatedAt
      const updated = await window.electronAPI.updateTodo({
        id: snapshot.todoId,
        ...input,
        expectedUpdatedAt,
      })
      if (!updated) throw new Error('Todo 不存在')
      todoMutationVersionRef.current.set(snapshot.todoId, updated.updatedAt)
    ...
    try {
      ...
      if (title && title !== snapshot.savedTitle) await persist({ title })
      if (snapshot.notes !== snapshot.savedNotes) await persist({ notes: snapshot.notes })
      clearRecoverableTodoDetailDraft(snapshot.todoId)
      return true
    } catch (error) {
      console.error('[任务/日程] 切换 Todo 时保存草稿失败:', error)
      const message = error instanceof Error ? error.message : ''
      toast.error(message.includes(PLANNING_CONFLICT_ERROR) ? 'Todo 已在其他窗口更新，旧草稿未能保存' : '切换 Todo 时保存草稿失败')
      return false
    }
  }, [enqueueTodoMutation, setTodos])
  persistDetachedDraftRef.current = persistDetachedDraft

  const clearNotesSaveStateTimer = React.useCallback((): void => {
    if (!notesSaveStateTimerRef.current) return
    clearTimeout(notesSaveStateTimerRef.current)
    notesSaveStateTimerRef.current = null
  }, [])

  const saveDetailNotes = React.useCallback(async (
    ...
    const expectedUpdatedAt = todoBaseUpdatedAtRef.current
    if (!id || expectedUpdatedAt === null || todoConflictRef.current) return false

    const existing = notesSaveInFlightRef.current.get(id)
    if (existing) {
      await existing
      if (selectedTodoIdRef.current !== id || todoConflictRef.current) return false
      return saveDetailNotesRef.current(options)
    }

    const snapshot = detailNotesRef.current
    if (snapshot === savedDetailNotesRef.current) return true

    const request: TodoNotesSaveRequestToken = {
      todoId: id,
      selectionEpoch: selectionEpochRef.current,
      generation: ++notesSaveGenerationRef.current,
    }
    latestNotesSaveGenerationRef.current.set(id, request.generation)
    clearNotesSaveStateTimer()
    if (showStatus && mountedRef.current) {
      setNotesSaveStatus({ state: 'saving', todoId: id, generation: request.generation })
    }

    const requestPromise = updateTodo(id, {
      notes: snapshot,
      expectedUpdatedAt,
    }, {
      savedField: 'notes',
      silentNotesSync: true,
      notesSnapshot: snapshot,
      notesRequest: request,
    })
    notesSaveInFlightRef.current.set(id, requestPromise)
    const updated = await requestPromise
    if (notesSaveInFlightRef.current.get(id) === requestPromise) {
      notesSaveInFlightRef.current.delete(id)
    }

    const requestIsCurrent = isCurrentTodoNotesSaveRequest(
      request,
      selectedTodoIdRef.current,
      selectionEpochRef.current,
      latestNotesSaveGenerationRef.current.get(id),
    )
    if (!updated) {
      if (showStatus && requestIsCurrent && !todoConflictRef.current && mountedRef.current) {
        setNotesSaveStatus({ state: 'failed', todoId: id, generation: request.generation })
      }
      return false
    }
    ...
    if (showStatus && mountedRef.current) {
      setNotesSaveStatus({ state: 'saved', todoId: id, generation: request.generation })
      notesSaveStateTimerRef.current = setTimeout(() => {
        notesSaveStateTimerRef.current = null
        if (!mountedRef.current) return
        setNotesSaveStatus((current) => current?.todoId === id && current.generation === request.generation ? null : current)
      }, 2_000)
    }
    return true
  }, [clearNotesSaveStateTimer, updateTodo])
  ...
  const scheduleNotesAutoSave = React.useCallback((): void => {
    ...
    notesSaveTimerRef.current = setTimeout(() => {
      notesSaveTimerRef.current = null
      void saveDetailNotesRef.current()
    }, 800)
  }, [])

  const saveDetailTitle = React.useCallback(async (): Promise<boolean> => {
    const id = selectedTodoIdRef.current
    const expectedUpdatedAt = todoBaseUpdatedAtRef.current
    if (!id || expectedUpdatedAt === null || todoConflictRef.current) return false
    const title = detailTitleRef.current.trim()
    if (!title || title === savedDetailTitleRef.current) return true
    return Boolean(await updateTodo(id, { title, expectedUpdatedAt }, { savedField: 'title' }))
  }, [updateTodo])

  const flushDetailDraft = React.useCallback(async (
    options: { showStatus?: boolean } = {},
  ): Promise<boolean> => {
    if (notesSaveTimerRef.current) {
      clearTimeout(notesSaveTimerRef.current)
    ...
    if (!selectedTodoIdRef.current) return true
    if (todoConflictRef.current) {
      if (options.showStatus !== false) toast.error('请先处理 Todo 冲突，再关闭或切换任务')
      return false
    }
    if (!await saveDetailTitle()) return false
    return saveDetailNotesRef.current(options)
  }, [saveDetailTitle])
  flushDetailDraftRef.current = flushDetailDraft
  ...
  React.useEffect(() => {
    ...
    return () => {
      if (notesSaveTimerRef.current) clearTimeout(notesSaveTimerRef.current)
      clearNotesSaveStateTimer()
      const detachedDraft = captureTodoDetailDraft()
      if (detachedDraft) {
        saveRecoverableTodoDetailDraft(detachedDraft)
        void persistDetachedDraftRef.current(detachedDraft)
      }
      mountedRef.current = false
    }
  }, [captureTodoDetailDraft, clearNotesSaveStateTimer])

  const reloadTodoDetail = (): void => {
    if (!selected) return
    clearRecoverableTodoDetailDraft(selected.id)
    applyTodoDetailDraft(selected)
  }

  const selectTodo = React.useCallback(async (todo: Todo): Promise<void> => {
    if (todo.id === selectedTodoIdRef.current || detailTransitionRef.current) return
    detailTransitionRef.current = true
    try {
      if (!await flushDetailDraft()) return
      applyTodoDetailDraft(todo)
      setSelectedId(todo.id)
    } finally {
      detailTransitionRef.current = false
    }
  }, [applyTodoDetailDraft, flushDetailDraft, setSelectedId])

  const closeTodoDetail = React.useCallback(async (): Promise<boolean> => {
    if (detailTransitionRef.current) return false
    detailTransitionRef.current = true
    try {
      if (!await flushDetailDraft()) return false
      selectionEpochRef.current += 1
      selectedTodoIdRef.current = null
      todoBaseUpdatedAtRef.current = null
      setSelectedId(null)
  ...
  }, [flushDetailDraft, setSelectedId])

  const changeTodoView = React.useCallback(async (nextView: TodoListView): Promise<void> => {
    if (selectedTodoIdRef.current && !await closeTodoDetail()) return
    setView(nextView)
  }, [closeTodoDetail])

  const visibleNotesSaveState = getVisibleTodoNotesSaveState(notesSaveStatus, selected?.id)
  ...
  const createTodo = async (): Promise<void> => {
    ...
    try {
      ...
      // 不自动打开详情；关闭旧详情前先 flush，失败时保留旧草稿面板。
      if (await flushDetailDraft()) {
        selectionEpochRef.current += 1
        selectedTodoIdRef.current = null
        todoBaseUpdatedAtRef.current = null
        setSelectedId(null)
  ...
  const completeTodo = async (todo: Todo): Promise<void> => {
    const isSelected = selectedTodoIdRef.current === todo.id
    if (isSelected && !await flushDetailDraft()) return
    const expectedUpdatedAt = isSelected ? (todoBaseUpdatedAtRef.current ?? todo.updatedAt) : todo.updatedAt
    const updated = await updateTodo(todo.id, { status: todo.status === 'completed' ? 'open' : 'completed', expectedUpdatedAt })
    if (updated?.status === 'completed' && selectedId === todo.id && view !== 'completed') {
      selectionEpochRef.current += 1
      selectedTodoIdRef.current = null
      todoBaseUpdatedAtRef.current = null
      setSelectedId(null)
  ...
  const deleteTodo = async (): Promise<void> => {
    ...
    try {
      ...
      clearRecoverableTodoDetailDraft(todo.id)
  ...
  const startTodoInWorkspace = React.useCallback(async (todo: Todo, workspaceId: string): Promise<void> => {
    ...
    if (selectedTodoIdRef.current === todo.id && !await flushDetailDraft()) return

    setStartingTodoId(todo.id)
    try {
      const { todo: updatedTodo, session } = await enqueueTodoMutation(todo.id, async () => {
        const expectedUpdatedAt = todoMutationVersionRef.current.get(todo.id) ?? todo.updatedAt
        const result = await window.electronAPI.startTodoAgent({
          todoId: todo.id,
          workspaceId,
          expectedUpdatedAt,
        ...
        todoMutationVersionRef.current.set(todo.id, result.todo.updatedAt)
        return result
      })
      setTodos((items) => items.map((item) => item.id === updatedTodo.id ? updatedTodo : item))
      if (selectedTodoIdRef.current === updatedTodo.id) todoBaseUpdatedAtRef.current = updatedTodo.updatedAt
  ...
  }, [agentChannelId, agentModelId, agentWorkspaces, enqueueTodoMutation, flushDetailDraft, openSession, setAgentSessions, setCurrentWorkspaceId, setDraftSessionIds, setPendingPrompt, setTodos, standalone, startingTodoId])
