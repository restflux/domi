...
export function PreviewTabContent({ sessionId, mode = 'tab' }: PreviewTabContentProps): React.ReactElement {
  ...
  }, [fileName, mode, sessionId, setTabs])
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-content-area">
      <div className="min-h-0 flex-1 overflow-hidden">
        <DiffTabContent
          key={`${sessionId}:${currentFile.filePath}:${currentFile.gitWorkspace?.repositoryId ?? ''}:${currentFile.gitWorkspace?.layer ?? ''}`}
          filePath={currentFile.filePath}
          dirPath={dirPath}
          sessionId={sessionId}
          gitRoot={currentFile.gitRoot}
          sessionTarget={currentFile.sessionTarget}
          gitWorkspace={currentFile.gitWorkspace}
          pathSpace={currentFile.pathSpace}
          previewOnly={currentFile.previewOnly}
          readOnly={currentFile.readOnly}
          basePaths={currentFile.basePaths}
          snapshotContent={currentFile.snapshotContent}
