...
import { ChevronDown, Code2, Copy, Check, Eye, FileText, Files, List, Pencil, RefreshCw, Save, WrapText, X } from 'lucide-react'
import { useAtom, useAtomValue, useSetAtom } from 'jotai'
import DOMPurify from 'dompurify'
import { File as PierreFile } from '@pierre/diffs/react'
import type { FileAccessOptions, GitWorkspaceChangeLayer } from '@domi/shared'
...
import { getPreviewCandidateBasePaths, isAbsoluteFilePath, selectPreviewTextContent } from './preview-open-path'
import { calculateImageFitZoom } from './image-preview-zoom'
...
interface DiffTabContentProps {
  ...
  sessionId: string
  gitRoot?: string
  /** Active Pi 文件路径相对于 Session Target。 */
  sessionTarget?: boolean
  /** 轻量 Git 面板的仓库与 Diff 层次。 */
  ...
  /** 非 Session Target 文件使用的显式绝对路径空间。 */
...
export function DiffTabContent({ filePath, dirPath, sessionId, gitRoot, sessionTarget = false, gitWorkspace, gitCommit, pathSpace, previewOnly, readOnly, basePaths, snapshotContent, onEmptyDiff, toolbarActions, baseRef }: DiffTabContentProps): React.ReactElement {
  ...
  const [imageFitMode, setImageFitMode] = React.useState(true)
  ...
  const findContentKey = React.useMemo(() => JSON.stringify({
    ...
    markdownEditing,
    markdownSourceMode,
  }), [docxHtml.length, filePath, htmlSourceMode, loading, markdownEditing, markdownSourceMode, newContent.length, officeHtml.length, oldContent.length, previewOnly, viewMode])
  ...
  const fileAccess = React.useMemo(() => sessionTarget
    ? { sessionId, pathSpace: 'session-target' as const }
    : {
        sessionId,
        ...(pathSpace ? { pathSpace } : {}),
      ...
      }, [sessionId, sessionTarget, pathSpace, basePaths, dirPath, filePath])

  const contentCacheScope = React.useMemo(() => gitWorkspace
    ? `git-workspace:${gitWorkspace.repositoryId}:${gitWorkspace.layer}`
    : gitCommit
      ? `git-commit:${gitCommit.repositoryId}:${gitCommit.oid}`
      : sessionTarget
        ? 'session-target'
        : JSON.stringify({
            dirPath,
            ...
            pathSpace,
            basePaths: basePaths ?? [],
          }), [basePaths, dirPath, gitRoot, gitCommit, gitWorkspace, sessionTarget, pathSpace])
  ...
  const markdownFileAccess = React.useMemo(() => {
    if (sessionTarget) return { sessionId, pathSpace: 'session-target' as const }
  ...
  }, [basePaths, dirPath, filePath, sessionId, sessionTarget, pathSpace])
  ...
  }, [filePath, sessionId, previewOnly, isLegacyOffice, snapshotContent])

  const fitImageToContainer = React.useCallback((naturalSize = imageNaturalSize) => {
    const container = imageContainerRef.current
    if (!container || naturalSize.w <= 0 || naturalSize.h <= 0) return
    ...
    setImageZoom(calculateImageFitZoom({
  ...
  React.useEffect(() => {
    const container = imageContainerRef.current
    if (!container || !isImage || !imageDataUrl || !imageFitMode || imageNaturalSize.w <= 0) return

    const update = () => fitImageToContainer()
    update()
    const observer = new ResizeObserver(update)
    observer.observe(container)
    return () => observer.disconnect()
  }, [fitImageToContainer, imageDataUrl, imageFitMode, imageNaturalSize.w, isImage])
  ...
  React.useEffect(() => {
    ...
    const handler = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        setImageZoom((z) => Math.max(0.01, Math.min(5, z * (e.deltaY < 0 ? 1.04 : 1 / 1.04))))
      }
    }
  ...
  React.useEffect(() => {
    ...
    // 权威快照不复用普通文件缓存；进入快照模式时清除同 key 旧条目，
    // 避免之后回到普通预览又命中快照之前的陈旧内容。
    if (snapshotContent !== undefined) contentCache.delete(cacheKey)
    const cached = snapshotContent === undefined ? cacheGet(cacheKey) : undefined
      ...
      try {
        ...
        if (!cached) {
          if (previewOnly) {
            ...
            if (cancelled) return
            content = selectPreviewTextContent(result?.content, snapshotContent)
            if (isHtml) {
              let url = ''
              if (snapshotContent === undefined) {
                const preview = await window.electronAPI.resolveHtmlPreviewPath(filePath, fileAccess)
                if (cancelled) return
                url = preview?.kind === 'file' ? preview.url : ''
              } else {
                // 权威快照不能用当前磁盘页面冒充；快照场景退回只读源码。
              ...
              cacheSet(cacheKey, { oldContent: '', newContent: content, htmlPreviewUrl: url })
              return
            }
          } else {
            const request = sessionTarget ? createSessionTargetFileRequest(sessionId, filePath) : null
            const result = gitWorkspace
              ? await window.electronAPI.getGitWorkspaceDiff({
                  sessionId,
                  repositoryId: gitWorkspace.repositoryId,
                  relativePath: filePath,
                  layer: gitWorkspace.layer,
                })
              : gitCommit
                ? await window.electronAPI.getGitWorkspaceCommitDiff({
                    sessionId,
                ...
                : sessionTarget
                  ? request ? await window.electronAPI.getSessionTargetDiffContents(request) : null
                  : await window.electronAPI.getDiffContents({ dirPath, filePath, gitRoot, sessionId, baseRef })
            if (cancelled) return
          ...
          if (cacheKey && snapshotContent === undefined) {
            cacheSet(cacheKey, { oldContent: old, newContent: content })
          }
        }
  ...
  }, [filePath, dirPath, gitRoot, gitCommit, gitWorkspace, sessionTarget, previewOnly, previewContentVersion, fileAccess, isHtml, isPdf, isDocx, isOfficePreview, isLegacyOffice, isImage, sessionId, ext, getContentCacheKey, snapshotContent])
  ...
  React.useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        const result = gitWorkspace
          ...
          : sessionTarget
            ? request ? await window.electronAPI.getSessionTargetDiffContents(request) : null
            : await window.electronAPI.getDiffContents({ dirPath, filePath, gitRoot, sessionId })
    ...
    refresh()
    return () => { cancelled = true }
  }, [refreshVersion, previewOnly, filePath, dirPath, gitRoot, gitCommit, gitWorkspace, sessionId, sessionTarget, getContentCacheKey])
  ...
  React.useEffect(() => {
    ...
    } else if (isHtml && !htmlPreviewUrl && snapshotContent === undefined) {
      message = '无法加载 HTML 渲染预览'
    }
    if (message) {
      toastedPreviewFailRef.current = key
      toast.warning(message)
    }
  }, [previewOnly, loading, filePath, ext, isLegacyOffice, isPdf, pdfSrc, isDocx, docxHtml, isOfficePreview, officeHtml, isImage, imageDataUrl, isHtml, htmlPreviewUrl, snapshotContent])
  ...
  const copyableText = markdownEditing ? markdownDraft : (isOfficePreview ? officeText : newContent)
  const canCopyFileContent = copyableText.length > 0
  ...
  const handleCopyFileSystemItem = React.useCallback(async (): Promise<void> => {
    try {
      await window.electronAPI.copyFileSystemItem([filePath], fileAccess)
      toast.success('已复制文件')
    } catch (error) {
      toast.error('复制文件失败', {
  ...
  const handleCopyFileSystemPath = React.useCallback(async (): Promise<void> => {
    try {
      await window.electronAPI.copyFileSystemPath([filePath], fileAccess)
  ...
  const handleOpenSelectionChat = React.useCallback(async (): Promise<void> => {
    if (!previewSelection) return
    openSideChatPanel({ parentSessionId: sessionId, quotedText: previewSelection.text })
    window.getSelection()?.removeAllRanges()
    clearPreviewSelection()
  }, [previewSelection, openSideChatPanel, clearPreviewSelection, sessionId])
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-1.5 flex-shrink-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className={cn(
                'flex items-center rounded p-1 text-foreground/40 hover:bg-foreground/[0.06] hover:text-foreground/60 shrink-0',
                previewOnly && !isEditableText && !isHtml && 'ml-auto',
              )}
              title="复制文件、路径或内容"
              aria-label="复制文件、路径或内容"
            >
              {copied ? <Check className="size-3.5 text-green-500" /> : <Copy className="size-3.5" />}
              <ChevronDown className="size-2.5" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-40 z-[9999] min-w-0 p-0.5">
            <DropdownMenuItem
              className="text-xs py-1 [&>svg]:size-3.5"
              onSelect={() => { void handleCopyFileSystemItem() }}
            >
              <Files />
              复制文件
            </DropdownMenuItem>
            <DropdownMenuItem
              className="text-xs py-1 [&>svg]:size-3.5"
              onSelect={() => { void handleCopyFileSystemPath() }}
            >
              <Copy />
              复制文件路径
            </DropdownMenuItem>
        ...
        {isMarkdown && !markdownEditing && (
          <button
            ...
            className={cn(
              'p-1 rounded hover:bg-foreground/[0.06] shrink-0',
              tocPinned ? 'text-primary' : 'text-foreground/40 hover:text-foreground/60',
            )}
            title={tocPinned ? '取消固定目录' : '固定目录'}
            aria-label={tocPinned ? '取消固定目录' : '固定目录'}
          >
      ...
      <div className="relative flex-1 min-h-0 flex">
        ...
        <MarkdownToc
          ...
          enabled={Boolean(isMarkdown && !markdownEditing)}
          pinned={tocPinned}
              ...
              imageDataUrl ? (
                ...
                <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 px-2 py-1 rounded-lg bg-background/80 backdrop-blur-sm border border-border/30 shadow-sm">
                  <button
                    ...
                    onClick={() => {
                      setImageFitMode(false)
                      setImageZoom((z) => Math.max(0.01, z / 1.5))
                    }}
                  >−</button>
                  <button
                    type="button"
                    ...
                    aria-label="适应窗口"
                    className={cn(
                      'text-xs min-w-[48px] h-6 px-1 rounded text-center font-mono hover:bg-muted/50',
                      imageFitMode ? 'text-foreground' : 'text-muted-foreground',
                    )}
                    onClick={() => {
                      setImageFitMode(true)
                      fitImageToContainer()
                  ...
                  <button
                    ...
                    onClick={() => {
                      setImageFitMode(false)
                      setImageZoom((z) => Math.min(5, z * 1.5))
                    }}
                  >+</button>
                </div>
                <div
                  ref={imageContainerRef}
                  className="h-full overflow-auto scrollbar-thin p-4 pt-12"
                  style={{ cursor: imageFitMode ? 'default' : (imageDragging.current ? 'grabbing' : 'grab') }}
                  onMouseDown={(e) => {
                    if (imageFitMode || e.button !== 0) return
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: '100%', minHeight: '100%', width: imageNaturalSize.w > 0 ? imageNaturalSize.w * imageZoom : undefined, height: imageNaturalSize.h > 0 ? imageNaturalSize.h * imageZoom : undefined }}>
                    <img
                      ...
                      onLoad={(e) => {
                        ...
                        setImageFitMode(true)
                        fitImageToContainer(naturalSize)
                      }}
