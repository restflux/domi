...
import { highlightCode, highlightToTokens, getDisplayName } from '@domi/core'
import { MermaidBlock } from '@domi/ui'
import type { HighlightTokensResult } from '@domi/core'
...
function buildCodeBlockRenderModeDecorations(doc: ProseMirrorNode, editable: boolean): DecorationSet {
  ...
  doc.descendants((node, pos) => {
    ...
    decorations.push(Decoration.node(pos, pos + node.nodeSize, {
      'data-domi-render-mode': editable ? 'editing' : 'preview',
    }))
    return false
  })
  return DecorationSet.create(doc, decorations)
...
function isExternalUrl(src: string): boolean {
  return /^(?:https?:|data:|blob:|file:|domi-file:)/i.test(src)
}
...
function createMathView(initialNode: ProseMirrorNode, displayMode: boolean) {
  return createStaticHtmlView(initialNode, {
    ...
    className: displayMode
      ? 'not-prose my-4 overflow-x-auto scrollbar-thin text-center'
...
function createShikiCodeBlockView(initialNode: ProseMirrorNode, view: EditorView) {
  ...
  dom.dataset.domiCodeBlock = 'true'

  // 头部栏：语言标签 + 复制按钮
  const header = document.createElement('div')
  header.contentEditable = 'false'
  setClass(header, 'domi-code-header flex h-8 items-center justify-between border-b border-border/30 px-3 text-xs text-muted-foreground')
  ...
  setClass(body, 'domi-code-source-body markdown-code-block-body overflow-x-auto scrollbar-thin')

  const editPre = document.createElement('pre')
  setClass(editPre, 'markdown-code-edit-layer m-0 min-h-[3.2em] overflow-x-auto scrollbar-thin bg-transparent p-4 font-mono text-[13px] leading-[1.6]')
  ...
  setClass(mermaidHost, 'domi-mermaid-preview hidden')
  ...
  const render = (node: ProseMirrorNode) => {
    ...
    dom.classList.toggle('domi-code-block--mermaid', shouldRenderMermaid)
...
export const RawHtmlBlock = Node.create({
  ...
  addNodeView() {
    return ({ node }) => createStaticHtmlView(node, {
      className: 'not-prose my-3 overflow-auto scrollbar-thin',
