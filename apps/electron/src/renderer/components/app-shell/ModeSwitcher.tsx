/**
 * ModeSwitcher - Chat/Work 模式切换（带滑动指示器）
 *
...
const modes: { value: AppMode; icon: React.ReactNode }[] = [
  { value: 'agent', icon: <Workflow size={15} /> },
...
export function ModeSwitcher(): React.ReactElement {
  ...
  const handleModeSwitch = React.useCallback((targetMode: AppMode) => {
    ...
    // 全局 Work Activity 仅属于 Work 模式；切换模式时始终回到会话内容视图。
  ...
  return (
    <div className="titlebar-drag-region select-none">
      <div
        className="relative flex rounded-lg p-1 titlebar-drag-region mode-switcher-track sidebar-control-surface"
      >
        {/* 滑动背景指示器 */}
        <div
          className={cn(
            'mode-slider pointer-events-none absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-md bg-background shadow-xs transition-transform duration-200 ease-out',
