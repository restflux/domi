...
export function SearchDialog(): React.ReactElement {
  ...
  const handleAgentSearch = React.useCallback(async () => {
    ...
    const configDir = import.meta.env.DEV ? DEV_CONFIG_DIR_NAME : CONFIG_DIR_NAME
    const prompt = `请帮我在 Domi 的全部会话历史中搜索与以下描述相关的内容：
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border/50">
          ...
          <button
            ...
            title="适合在精准搜索找不到的情况下使用，Agent 会帮助你搜索整个 Domi 会话空间"
            className={cn(
              'flex items-center gap-1 px-2 py-1 rounded text-[12px] font-medium transition-colors',
        ...
        <div className="relative">
          ...
          {hasSearched && loading && allResults.length === 0 && (
            <div className="py-10 flex flex-col items-center justify-center gap-2 text-[13px] text-foreground/40">
              <AgentActivityOrb state="searching" size={20} aria-hidden="true" />
              <span>正在搜索...</span>
            </div>
          )}
