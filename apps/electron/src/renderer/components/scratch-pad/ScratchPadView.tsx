 ...
 *
 * 基于 TipTap 的轻量 Markdown 编辑器，内容持久化到 ~/.domi/scratch-pad.md。
...
import { FileDown, List, ListTodo } from 'lucide-react'
...
/** Right Workspace 内的草稿正文；标题、保存状态和展开操作由公共工具区承载。 */
export function ScratchPadWorkspace(): React.ReactElement {
  return <ScratchPadEditor />
}

function ScratchPadEditor(): React.ReactElement {
  ...
  const activeSessionTitle = React.useMemo(
    () => agentSessions.find((session) => session.id === activeSessionId)?.title ?? null,
  ...
  const scrollClassName = 'flex-1 overflow-auto scrollbar-thin px-4 pt-4'
  const contentClassName = 'h-full max-w-none'
  const speechWrapperClassName = 'absolute left-1/2 -translate-x-1/2 bottom-9 z-20'
  const speechButtonClassName = 'size-9 rounded-full bg-background/95 border border-border/60 shadow-md backdrop-blur hover:bg-accent text-foreground/80'
