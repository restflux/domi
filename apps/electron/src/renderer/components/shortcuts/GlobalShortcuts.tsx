...
import { normalizeAgentQueueDeliveryMode } from '@/lib/agent-queue-delivery-mode'
...
export function GlobalShortcuts(): null {
  ...
  const setAgentSteeringMode = useSetAtom(agentSteeringModeAtom)
  const setAgentFollowUpMode = useSetAtom(agentFollowUpModeAtom)
  const { createChat, createAgent } = useCreateSession()

  // Tab 管理（当前活动 Work 会话固定保留，不从主区快捷键关闭）
  ...
  useEffect(() => {
    ...
    window.electronAPI.getSettings().then((settings) => {
      ...
      setAgentSteeringMode(normalizeAgentQueueDeliveryMode(settings.agentSteeringMode))
      setAgentFollowUpMode(normalizeAgentQueueDeliveryMode(settings.agentFollowUpMode))
    }).catch(console.error)
  }, [setAgentFollowUpMode, setAgentSteeringMode, setShortcutOverrides, setSendWithCmdEnter])
  ...
  const handleCloseTab = useCallback(() => {
    ...
    if (!canCloseMainTab(activeTab, activeTabId)) return
    requestClose(activeTabId)
  }, [shortcutGuideOpen, setShortcutGuideOpen, commandPaletteOpen, setCommandPaletteOpen, settingsOpen, setSettingsOpen, channelFormDirty, setSettingsCloseRequested, searchOpen, setSearchOpen, tabs, activeTabId, requestClose])
  ...
  // Cmd+Shift+M → 切换模式；从草稿本触发时进入 Chat
  useShortcut(
    'toggle-mode',
    useCallback(
      () => setAppMode(resolveToggledConversationMode(appMode)),
      [appMode, setAppMode],
    ),
  )
    ...
    const cleanup = window.electronAPI.onQuickTaskOpenSession(async (data) => {
      try {
        ...
        if (data.mode === 'agent') {
          ...
          store.set(draftSessionIdsAtom, (prev) => new Set(prev).add(meta.id))
          ...
          const savedAttachments: import('@domi/shared').FileAttachment[] = []
