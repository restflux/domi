...
import type { FileIndexEntry } from '@domi/shared'
...
export function createFileMentionSuggestion(
  ...
  usesSessionTargetRef?: React.RefObject<boolean>,
  sessionTargetBoundRef?: React.RefObject<boolean>,
  /** 目标未绑定时的 Local 项目预览根（@ 引用回退搜索使用；已绑定时应传 null）。 */
  sessionTargetPreviewRootRef?: React.RefObject<string | null>,
): Omit<SuggestionOptions<FileIndexEntry>, 'editor'> {
  let missingWorkspaceToastShown = false
  // 无结果时的引导提示（仅当目标尚未绑定且没有 Local 预览根时展示）；随每次 items() 请求
  // 刷新，渲染时通过 requestGuard 保证读取到的是与当前 props.items 同一次请求的结果。
  let latestEmptyHint: string | null = null
  ...
  return {
    ...
    items: async ({ query }): Promise<FileIndexEntry[]> => {
      ...
      const usesSessionTarget = usesSessionTargetRef?.current === true
      const sessionTargetPending = usesSessionTarget && sessionTargetBoundRef?.current === false
      const previewRoot = sessionTargetPending ? (sessionTargetPreviewRootRef?.current ?? null) : null
      if (!usesSessionTarget && !wsPath) {
      ...
      try {
        ...
        if (!sessionId) return requestGuard.attachResult(requestId, [])
        const additionalPaths = attachedDirsRef?.current ?? []
        const sessionPaths = sessionAttachedDirsRef?.current ?? []

        if (sessionTargetPending && previewRoot) {
          // 目标未绑定：@ 引用回退到 Local 项目预览（主进程按只读 pathSpace 授权）。
          // 返回条目为项目相对路径，插入为 checkout 相对路径，首次发送绑定后仍可解析。
          ...
          latestEmptyHint = null
          return requestGuard.attachResult(
            requestId,
            result.entries.map((entry) => ({ ...entry, source: 'workspace' as const })),
          )
        }

        const result = usesSessionTarget
          ? await window.electronAPI.searchSessionTargetFiles(sessionId, query ?? '', 200)
          : await window.electronAPI.searchWorkspaceFiles(
              wsPath!,
              query ?? '',
              200,
    ...
    render: () => {
      ...
      function createRenderer(props: SuggestionProps<FileIndexEntry>) {
        ...
        const selectItem = (item: MentionSelection) => {
          const path = usesSessionTargetRef?.current
            ? createSessionTargetFileRequest(sessionIdRef?.current ?? '', item.path)?.relativePath
            : resolveFileMentionPath(item, workspacePathRef.current)
          if (path) props.command({ id: path, label: item.name })
        }
