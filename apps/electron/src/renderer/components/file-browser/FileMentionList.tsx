...
export interface FileMentionListProps {
  ...
  embedded?: boolean
  /** 无结果时的引导提示（如目标尚未创建、需要先发送消息），未提供时显示默认「无匹配文件」。 */
...
export const FileMentionList = React.forwardRef<FileMentionRef, FileMentionListProps>(
  function FileMentionList({ sessionEntries, workspaceEntries, onSelect, onBack, embedded = false, emptyHint = null }, ref) {
      ...
      return (
        <div className={cn(!embedded && 'rounded-lg border bg-popover shadow-lg', 'overflow-hidden min-w-[260px]')}>
          ...
          <div className="p-2 text-[11px] text-muted-foreground">
            {emptyHint ? (
              <div className="rounded-md bg-muted/50 px-2 py-1.5 leading-4">{emptyHint}</div>
            ) : (
              '无匹配文件'
            )}
          </div>
