...
export function FileDropZone({ workspaceSlug, sessionId, target = 'session', onFilesUploaded, onFilesAttached, onAttachFolder, onFoldersDropped }: FileDropZoneProps): React.ReactElement {
  ...
  const zoneClass = (side: 'left' | 'right'): string =>
    cn(
      'file-drop-action flex-1 flex min-h-11 items-center justify-center gap-2 rounded-md px-3 py-2 transition-colors duration-150 cursor-pointer',
      'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40',
      isDragOver === side
        ? 'bg-primary/20 ring-1 ring-primary/45'
        : 'bg-muted/25 hover:bg-muted/55',
        ...
        <>
          ...
          <Tooltip open={isDragOver === 'left' ? false : undefined}>
            <TooltipTrigger asChild>
              ...
              >
                ...
                <span className="text-[11px] text-muted-foreground/80">添加文件</span>
              </div>
            </TooltipTrigger>
          ...
          <Tooltip open={isDragOver === 'right' ? false : undefined}>
            <TooltipTrigger asChild>
              ...
              >
                <FolderPlus className="size-3.5 text-muted-foreground/60" />
                <span className="text-[11px] text-muted-foreground/80">附加文件夹</span>
              </div>
            </TooltipTrigger>
            <TooltipContent side="bottom">
