...
import { Bot, AlertTriangle, FileText, FileImage, Download, Split, Undo2, RotateCw, Plus, Minimize2, Wrench, Settings, Cpu, ExternalLink, Quote, Clock, FolderInput, FolderPlus, FolderGit2, ListTodo, Paperclip, ChevronDown, ChevronUp } from 'lucide-react'
...
import type { AgentPendingFile } from '@domi/shared'
import {
  getSDKCompactStatus,
...
export interface SDKMessageRendererProps {
  ...
  showHeader?: boolean
  /** 当前权威 Domi session ID；system message identity 不能作为操作授权。 */
  sessionId?: string
...
function WorktreeHandoffNotice({ message }: { message: SDKSystemMessage }): React.ReactElement | null {
  const openSession = useOpenSession()
  const childSessionId = typeof message.child_session_id === 'string' ? message.child_session_id : undefined
  const childSessionTitle = typeof message.child_session_title === 'string'
    ? message.child_session_title
    : 'managed Worktree 子会话'
  ...
  return (
    <div className="my-3 pl-[46px] pr-1">
      <button
        ...
        onClick={() => openSession('agent', childSessionId, childSessionTitle)}
      >
        <FolderGit2 className="mt-0.5 size-3.5 shrink-0 text-blue-500" />
        <div className="min-w-0 flex-1 space-y-1">
          <div className="font-medium text-foreground">已创建 managed Worktree 子会话</div>
          <div className="truncate text-muted-foreground">{childSessionTitle}</div>
        </div>
...
// extractMeta / MessageMeta 已迁移至 @domi/session-core
...
function extractTurnUsage(turnMessages: SDKMessage[]): { durationMs?: number; usage?: AgentEventUsage } {
  for (const msg of turnMessages) {
    ...
    const resolvedContextWindow = resolveSDKResultContextWindow(resultMsg)
    return {
      durationMs,
      usage: {
...
// extractUserText 已迁移至 @domi/session-core
...
// groupIntoTurns / mergeAdjacentSameModelTurns 已迁移至 @domi/session-core
...
export interface AssistantTurnRendererProps {
  ...
  /** 从 Local 会话分叉到新 managed Worktree。 */
  ...
  /** 当前权威 Domi session ID，用于消息内计划预览入口。 */
  sessionId?: string
  /** 用户在前端选择的模型 ID（优先用于显示名称） */
  sessionModelId?: string
  toolPresentationIndex?: ToolPresentationIndex
...
function findLatestVisibleActivityBlock(blocks: SDKContentBlock[]): SDKContentBlock | undefined {
  for (let index = blocks.length - 1; index >= 0; index -= 1) {
    ...
    if (!block) continue
    if (block.type === 'text' && !(block as SDKTextBlock).text) continue
    if (block.type === 'thinking' && !(block as SDKThinkingBlock).thinking) continue
    return block
  }
  return undefined
}
...
export function AssistantTurnRenderer({ turn, allMessages, basePath, basePaths, onFork, onForkToWorktree, onRewind, onCreateTodo, onRetry, onRetryInNewSession, onCompact, onRelinkProjectRoot, onRestoreProjectRoot, isStreaming, stoppedByUser, sessionId, sessionModelId, toolPresentationIndex }: AssistantTurnRendererProps): React.ReactElement | null {
  ...
  const [expandedImageGroups, setExpandedImageGroups] = React.useState<Set<string>>(() => new Set())
  const handleProcessGroupExpandedChange = React.useCallback((groupId: string, expanded: boolean) => {
    setExpandedImageGroups((current) => {
      const hasGroup = current.has(groupId)
      if (hasGroup === expanded) return current
  ...
  const turnImageGenerationToolUseIds = React.useMemo(
    () => collectAssistantGeneratedImageToolUseIds(turn.assistantMessages),
    [turn.assistantMessages],
  )
  const generatedImagesByToolId = React.useMemo(
    () => collectGeneratedImagesByToolId(turn.turnMessages, turnImageGenerationToolUseIds),
    [turn.turnMessages, turnImageGenerationToolUseIds],
  )
  const imageContainingToolUseIds = React.useMemo(
    () => collectImageContainingToolUseIds(turn.assistantMessages, generatedImagesByToolId),
    [generatedImagesByToolId, turn.assistantMessages],
  )
  const turnGeneratedImages = React.useMemo(
    () => flattenGeneratedImagesForTurn(turn.assistantMessages, generatedImagesByToolId),
    [generatedImagesByToolId, turn.assistantMessages],
  )
  const showTurnGeneratedImages = shouldShowTurnGeneratedImages({
    imageCount: turnGeneratedImages.length,
    isStreaming: !!isStreaming,
    expandedImageGroupCount: expandedImageGroups.size,
  })
  ...
  const hasTextContent = renderableTopLevelBlocks.some(
  ...
  // 如果过滤掉任务浮层记录后只剩错误消息，直接展示完整错误卡。
  if (renderableTopLevelBlocks.length === 0 && hasError && errorContent) {
    return (
      <ErrorMessage
  ...
  const renderTopLevelBlock = (
    block: SDKContentBlock,
    i: number,
    isActivityTail = block === activityTailBlock,
  ): React.ReactNode => {
  ...
  return (
    <Message from="assistant">
      ...
      <MessageContent>
        ...
        <div className={cn('space-y-2')}>
          {renderItems.map((item, itemIndex) => {
            ...
            const processGroupId = `process-${firstIndex}`
            const hasGeneratedImages = groupBlocks.some((block) => (
              block.type === 'tool_use'
              && imageContainingToolUseIds.has((block as SDKToolUseBlock).id)
            ))
            // 流式分段降噪：thinking / 中间正文保持原位，相邻读取和搜索收拢为探索阶段。
            const detailUnits = buildProcessDetailUnits(groupBlocks)
            const indexByBlock = new Map(item.items.map((groupItem) => [groupItem.block, groupItem.index]))
            return (
              <ProcessBlockGroup
                key={processGroupId}
                blocks={groupBlocks}
                ...
                {detailUnits.map((unit) => unit.kind === 'exploration'
                  ? (
                      <ToolRunGroup
                        ...
                        animate={!!isStreaming}
                        renderRow={(block) => renderTopLevelBlock(block, indexByBlock.get(block) ?? 0, false)}
                      />
                    )
                  : unit.blocks.map((block) => renderProcessGroupBlock(block, indexByBlock.get(block) ?? unit.startIndex)))}
              </ProcessBlockGroup>
            )
          })}
        </div>
        {/* 如果有错误但也有内容块，在末尾以 tail 形式挂错误横幅附错误提示 + 重试按钮，保留正文本身的 markdown 排版 */}
        {hasError && errorContent && renderableTopLevelBlocks.length > 0 && (
          <AssistantErrorTail
            message={errorContent}
            onRetry={onRetry}
            onRetryInNewSession={onRetryInNewSession}
      ...
      {/* 操作栏：流式输出完成后显示操作按钮 */}
      {!isStreaming && (() => {
        const textContent = renderableTopLevelBlocks
        ...
        return (
          <MessageActions className="pl-[46px] mt-0.5 min-h-[28px] justify-start">
            ...
            {onFork && lastUuid && (onForkToWorktree ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button type="button" variant="ghost" size="icon-sm" className="w-auto gap-0.5 px-1.5" aria-label="分叉选项" title="分叉选项">
                    <Split className="size-3.5" />
                    <ChevronDown className="size-2.5" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="z-[9999] min-w-44">
                  <DropdownMenuItem onSelect={() => onFork(lastUuid)}>
                    <Split className="size-3.5" />普通分叉
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onForkToWorktree(lastUuid)}>
                    <FolderGit2 className="size-3.5" />Fork 到 managed Worktree
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ) : (
...
export function SDKMessageRenderer({
  ...
  const channels = useAtomValue(channelsAtom)
  const effectiveToolPresentationIndex = toolPresentationIndex ?? buildToolPresentationIndex(allMessages)
...
const SCHEDULED_RUN_MARKER = '<!--DOMI_SCHEDULED_RUN-->'

// stripScheduledRunMarker 已迁移至 @domi/session-core（本文件从该包 import 使用）
...
function ScheduledRunBadge(): React.ReactElement {
  ...
  return (
    <button
      ...
      title="来自 Domi 定时任务，点击查看设置"
    >
      <Clock className="size-3" />
      <span>来自 Domi 定时任务</span>
    </button>
  )
}

function UserAsideList({ asides }: { asides: NonNullable<SDKUserMessage['_asides']> }): React.ReactElement | null {
  const [expanded, setExpanded] = React.useState(false)
  if (asides.length === 0) return null

  return (
    <div className="mb-2 rounded-md border border-border/45 bg-muted/20 text-xs text-muted-foreground">
      <button
        type="button"
      ...
      >
        ...
        <span className="flex-1">附言 {asides.length} 条</span>
        {expanded ? <ChevronUp className="size-3.5" /> : <ChevronDown className="size-3.5" />}
      </button>
      {expanded && (
        <div className="space-y-1.5 border-t border-border/35 px-2.5 py-2">
          {asides.map((aside, index) => (
            <pre key={aside.id || index} className="max-h-48 overflow-auto scrollbar-thin whitespace-pre-wrap break-words rounded bg-background/45 px-2 py-1.5 font-sans leading-5 text-foreground/75">
              {aside.content}
            </pre>
          ))}
        </div>
...
export function getGroupId(group: MessageGroup): string {
  if (group.type === 'user') {
    ...
    const stableKey = (group.message as unknown as Record<string, unknown>)._domiStableKey
  ...
  const stableKey = first ? (first as unknown as Record<string, unknown>)._domiStableKey : undefined
  if (typeof stableKey === 'string') return stableKey
