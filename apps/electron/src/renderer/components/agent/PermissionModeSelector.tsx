...
import type { DomiPermissionMode } from '@domi/shared'
import { DOMI_PERMISSION_MODE_CONFIG, DOMI_PERMISSION_MODE_ORDER } from '@domi/shared'
...
const MODE_ICONS: Record<DomiPermissionMode, React.ComponentType<{ className?: string }>> = {
...
export function PermissionModeSelector({ sessionId }: PermissionModeSelectorProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    setModeMap((prev: Map<string, DomiPermissionMode>) => {
  ...
  const cycleMode = React.useCallback(async () => {
    const currentIndex = DOMI_PERMISSION_MODE_ORDER.indexOf(displayMode)
    const nextIndex = (currentIndex + 1) % DOMI_PERMISSION_MODE_ORDER.length
    const nextMode = DOMI_PERMISSION_MODE_ORDER[nextIndex]!
    const prevMode = mode
  ...
  const config = DOMI_PERMISSION_MODE_CONFIG[displayMode]
