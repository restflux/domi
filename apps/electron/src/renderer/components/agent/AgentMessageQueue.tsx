...
import { getAgentQueueKindLabel } from '@/lib/agent-queue-enter-kind'
import {
...
interface AgentMessageQueueProps {
  ...
  onAdjustDirection: (messageId: string) => void
...
export function AgentMessageQueue({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-1 pb-1 text-[12px] text-muted-foreground">
        <Clock3 className="size-3.5 shrink-0" />
        <span>{items.length} 条待处理消息</span>
      </div>
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                ...
                ) : getAgentQueueKindLabel(item.kind)}
              </span>
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                {item.kind === 'followUp' && (
                  <Button
                    ...
                    disabled={!canAdjustDirection}
                    onClick={() => onAdjustDirection(item.id)}
                    title="提升为调整方向，优先影响当前任务"
                  >
