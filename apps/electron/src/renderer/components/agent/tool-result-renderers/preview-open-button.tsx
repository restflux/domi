...
import { getToolPreviewBasePaths } from '@/components/diff/preview-open-path'
...
export function PreviewOpenButton({ filePath, basePath, basePaths, className }: PreviewOpenButtonProps): React.ReactElement | null {
  ...
  const handleOpen = () => {
    const candidateBasePaths = getToolPreviewBasePaths(basePath, basePaths)
    openPreview(sessionId, {
      filePath,
      previewOnly: true,
      readOnly: true,
      basePaths: candidateBasePaths.length > 0 ? candidateBasePaths : undefined,
    })
  }
