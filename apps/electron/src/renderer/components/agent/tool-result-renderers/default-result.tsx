...
export function DefaultResultRenderer({ result, isError }: DefaultResultRendererProps): React.ReactElement {
  if (isError) {
    return (
      <pre className="rounded-md p-3 text-[12px] font-mono text-destructive/80 bg-destructive/5 whitespace-pre-wrap break-all overflow-x-auto scrollbar-thin">
        {result}
      </pre>
    )
  }
  ...
  return (
    <CollapsibleResult
      content={result}
      renderContent={(text) => (
        <pre className="rounded-md p-3 text-[12px] font-mono text-foreground/60 bg-muted/30 whitespace-pre-wrap break-all overflow-x-auto scrollbar-thin max-h-[400px] overflow-y-auto">
          {text}
        </pre>
      )}
    />
