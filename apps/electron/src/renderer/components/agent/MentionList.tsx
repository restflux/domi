...
function MentionListInner<T>(
  { items, onSelect, emptyText, keyExtractor, renderItem, groupLabel, headerLabel }: MentionListProps<T>,
  ...
  React.useEffect(() => {
    ...
    if (!container) return
    const item = container.querySelectorAll<HTMLElement>('[data-mention-option]')[localIndex]
  ...
  return (
    <div className="rounded-lg border bg-popover shadow-lg overflow-hidden w-[280px]">
      <MentionHeader label={headerLabel} />
      <div ref={containerRef} className="overflow-y-auto max-h-[240px] scrollbar-thin" role="listbox" aria-label={headerLabel ?? '引用选项'}>
        {items.map((item, index) => {
          const currentGroup = groupLabel?.(item)
          const previousGroup = index > 0 ? groupLabel?.(items[index - 1]!) : undefined
          const showGroup = currentGroup !== undefined && currentGroup !== previousGroup
          return (
            <React.Fragment key={keyExtractor(item)}>
              {showGroup && (
                <div
                  className={cn(
                    'px-2.5 py-1 text-[10px] font-medium text-muted-foreground/70',
                    index > 0 && 'mt-1 border-t border-border/50 pt-1.5',
                  )}
                  aria-hidden="true"
                >
                  {currentGroup}
