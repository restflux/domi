  ...
  return (
    <div className="space-y-3 p-1">
      ...
      {progress.summary && !isRunning && (
        <p className="max-h-28 overflow-y-auto scrollbar-thin rounded-md bg-muted/50 px-2.5 py-2 text-xs leading-5 text-muted-foreground">
          {progress.summary}
        </p>
      )}
    </div>
...
export function TaskProgressOverlay({ activities, streaming, contextCompaction }: TaskProgressOverlayProps): React.ReactElement | null {
  ...
  if (!showTaskProgress) {
    return (
      <Button
        className="absolute bottom-[26px] left-1/2 size-10 -translate-x-1/2 rounded-lg border border-border/60 bg-background/85 shadow-sm backdrop-blur-sm transition-[background-color,transform] duration-200 hover:bg-accent/80 active:scale-[0.96]"
  ...
  return (
    <div className={cn(
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              'pointer-events-auto flex min-h-10 max-w-[min(460px,calc(100vw-9rem))] items-center gap-2 rounded-lg border border-border/60 bg-background/85 py-2 pr-3 pl-2.5 text-left shadow-sm backdrop-blur-sm',
          ...
          >
            {displayCompaction ? (
              <>
                {displayCompaction.status === 'running' && <AgentActivityOrb state="solving" size={20} aria-hidden="true" />}
        ...
        <PopoverContent className="w-[min(420px,calc(100vw-2rem))] rounded-lg border-border/60 bg-background/95 p-2 backdrop-blur-sm" side="top" align="center">
          {displayCompaction
      ...
      {!isAtBottom && (
        <Button
          aria-label="回到最下方"
          className="pointer-events-auto size-10 rounded-lg border border-border/60 bg-background/85 shadow-sm backdrop-blur-sm transition-[background-color,transform] duration-200 hover:bg-accent/80 active:scale-[0.96]"
          onClick={() => scrollToBottom()}
          type="button"
          variant="ghost"
        >
