...
import { groupIntoTurns } from './SDKMessageRenderer'
import {
  buildVisibleConversationGroups,
  getContextCompactionProgress,
  isCompactionControlHistoryGroup,
} from './AgentMessages'
...
describe('context compaction progress overlay state', () => {
  test('hides /compact control messages from conversation history', () => {
    for (const text of ['/compact', '  /compact 保留 API 决策  ', '/compact\n保留 API 决策']) {
      expect(isCompactionControlHistoryGroup({
        type: 'user',
        message: { type: 'user', message: { content: [{ type: 'text', text }] } },
      } as never)).toBe(true)
    }
    expect(isCompactionControlHistoryGroup({
      type: 'user',
      message: { type: 'user', message: { content: [{ type: 'text', text: '/compactness' }] } },
    } as never)).toBe(false)
    expect(isCompactionControlHistoryGroup({
      type: 'system',
  ...
  test('同一次任务被隐藏的压缩生命周期切开后仍展示为一个 assistant turn', () => {
    const groups = groupIntoTurns([
      {
        type: 'user',
        message: { content: [{ type: 'text', text: '继续完成当前任务' }] },
      ...
      systemMessage({ subtype: 'compacting' }),
      systemMessage({ subtype: 'compact_boundary' }),
      {
        type: 'assistant',
        message: {
          model: 'gpt-5.6-sol',
          content: [{ type: 'tool_use', id: 'grep-1', name: 'Grep', input: { pattern: 'TODO' } }],
        },
        parent_tool_use_id: null,
      },
    ] as SDKMessage[])

    const visibleGroups = buildVisibleConversationGroups(groups)

    expect(visibleGroups.map((group) => group.type)).toEqual(['user', 'assistant-turn'])
    expect(visibleGroups[1]).toMatchObject({
      type: 'assistant-turn',
      assistantMessages: [{ message: { model: 'gpt-5.6-sol' } }, { message: { model: 'gpt-5.6-sol' } }],
    })
  })
