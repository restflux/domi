...
/**
 * 缩短过长的展示名（如时间戳会话 JSONL 文件名、深层路径），
 * 保留扩展名与尾部特征，前部以 … 代替；完整信息由行级 tooltip 承载。
 */
export function shortenDisplayName(name: string, max = 26): string {
  if (name.length <= max) return name
  const dot = name.lastIndexOf('.')
  const ext = dot > 0 && dot > name.length - 8 ? name.slice(dot) : ''
  const stem = ext ? name.slice(0, dot) : name
  return `…${stem.slice(-(max - 1 - ext.length))}${ext}`
}

/**
 * 从 Domi Skill 入口文件路径中提取 Skill 名称。
 *
  ...
  switch (toolName) {
    case 'Read': {
      ...
      if (typeof fp === 'string') {
        ...
        const name = shortenDisplayName(filename(fp))
        const offset = typeof input.offset === 'number' ? input.offset : undefined
        const limit = typeof input.limit === 'number' ? input.limit : undefined
        if (offset !== undefined && limit !== undefined) {
          return phrase(`读取 ${name} 第 ${offset}-${offset + Math.max(0, limit - 1)} 行`)
        }
        if (offset !== undefined) {
    ...
    case 'Edit': {
      const fp = input.file_path ?? input.filePath ?? input.path
      const name = typeof fp === 'string' ? shortenDisplayName(filename(fp)) : '文件'
    ...
    case 'MultiEdit': {
      ...
      return phrase(editCount > 0 ? `批量编辑 ${name} · ${editCount} 处` : `批量编辑 ${name}`)
    }
    ...
    case 'Grep': {
      ...
      if (typeof pattern === 'string') {
        const parts = [`搜索内容 /${truncate(pattern, 44)}/`]
        const path = input.path
        const glob = input.glob
        if (typeof glob === 'string') {
          parts.push(`in ${truncate(glob, 32)}`)
        } else if (typeof path === 'string') {
          parts.push(`in ${truncate(path, 32)}`)
        }
    ...
    case 'Glob': {
      const pattern = input.pattern
      if (typeof pattern === 'string') {
        const parts = [`搜索文件 ${truncate(pattern, 44)}`]
        const path = input.path
    ...
    case 'LS': {
      ...
      if (typeof path === 'string') return phrase(`查看目录 ${shortenDisplayName(filename(path))}`)
      return phrase('查看目录')
    }
    ...
    case 'BrowserOpen':
      return phrase('打开内置浏览器')
    case 'BrowserNavigate':
      return phrase(typeof input.url === 'string' ? `浏览 ${truncate(input.url, 60)}` : '导航浏览器')
    case 'BrowserSnapshot':
      return phrase('观察浏览器页面')
    case 'BrowserClick':
      return phrase(typeof input.ref === 'string' ? `点击页面元素 ${input.ref}` : '点击页面元素')
    case 'BrowserType':
      // 网页输入可能包含隐私内容；工具活动永不回显正文。
      return phrase(typeof input.ref === 'string' ? `向页面元素 ${input.ref} 输入文本` : '向页面输入文本')
    case 'BrowserScroll':
      return phrase('滚动浏览器页面')
    case 'BrowserExtract':
      return phrase(typeof input.ref === 'string' ? `提取页面元素 ${input.ref}` : '提取页面文本')
    case 'BrowserClose':
      return phrase('关闭内置浏览器')
    ...
    case 'RequestDirectWorkflow': {
      return phrase('请求切换为 Direct')
    }
