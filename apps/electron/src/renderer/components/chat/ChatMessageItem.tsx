...
export const ChatMessageItem = React.memo(function ChatMessageItem({
  ...
  /** 失败 assistant 消息之前的最后一条用户消息，作为重试目标 */
  const retryTargetUserMessage = React.useMemo(() => {
    if (message.role !== 'assistant' || !allMessages) return null
  ...
  const handleRetry = React.useCallback(() => {
    if (!onResendMessage || !retryTargetUserMessage) return
    void onResendMessage(retryTargetUserMessage)
  }, [onResendMessage, retryTargetUserMessage])
            ...
            <>
              ...
              {message.error && (
                <div className="mt-1 px-3 py-2 rounded-md bg-destructive/10 text-destructive text-sm flex items-center gap-2">
                  ...
                  <span className="min-w-0 flex-1 break-all">{message.error}</span>
                  {retryTargetUserMessage && onResendMessage && (
                    <button
                      type="button"
                      onClick={handleRetry}
