...
import type { SystemPrompt, SystemPromptCreateInput, SystemPromptUpdateInput } from '@domi/shared'
...
export function PromptEditorSidebar(): React.ReactElement {
  ...
  const chatPrompts = React.useMemo(
    () => config.prompts.filter((prompt) => prompt.scope === 'chat'),
    [config.prompts],
  )
  const selectedPrompt = React.useMemo(
    () => chatPrompts.find((prompt) => prompt.id === selectedId),
    [chatPrompts, selectedId]
  )
