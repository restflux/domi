...
import type { ChatMessage, ChatToolActivity } from '@domi/shared'
...
interface ChatMessagesProps {
  ...
  /** 是否正在流式生成 */
  streaming: boolean
  /** 新会话 Hero 布局：以自然高度渲染空状态，由 ChatView 外层 spacer 负责垂直居中 */
  hero?: boolean
  /** 流式累积内容 */
...
/**
 * 首次加载骨架屏 — 模拟一轮"用户消息 + assistant 回复"的形状占位。
 * 在 messagesLoaded 之前替代空白滚动区，加载完成后由既有淡入逻辑接管。
 */
function ConversationSkeleton(): React.ReactElement {
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-7 px-4 py-8" aria-hidden="true">
      <div className="flex justify-end">
        <Skeleton className="h-9 w-2/5 rounded-2xl" />
      </div>
      <div className="flex flex-col gap-2.5">
        <div className="flex items-center gap-2.5">
          <Skeleton className="size-[35px] rounded-[25%]" />
...
export function ChatMessages({
  ...
  // 首次消息加载未完成：先显示骨架屏，避免空白滚动区（加载完成后走既有淡入逻辑）
  if (!messagesLoaded && messages.length === 0 && !streaming) {
