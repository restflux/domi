...
function ChatViewInner({ conversationId }: ChatViewProps): React.ReactElement {
  ...
  // 新会话 Hero 布局：空对话时问候语与输入框整体垂直居中；等待首次加载完成后再判定，避免闪烁
  const isChatSessionEmpty = messagesLoaded
    && messages.length === 0
    && !isStreaming
    && !parallelMode
