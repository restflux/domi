/**
 * BuiltinMcpDetailSheet — Domi 内置 MCP 托管详情
 */
...
function getConfigInfo(server: BuiltinMcpServerSummary): BuiltinMcpConfigInfo {
  ...
  if (server.id === 'chrome-devtools') {
    return {
      ...
      description: '启用后 Domi 会通过 npx 启动 chrome-devtools-mcp，让 Agent 可以打开真实浏览器页面、截图和检查 DOM。首次使用可能需要下载 npm 包，并要求本机安装 Chrome。',
    }
  }
  ...
  if (server.id === 'automation') {
    return {
      source: 'Domi 本地自动任务',
      description: '自动任务 MCP 直接使用 Domi 本地任务服务，无需填写额外凭据。',
    }
  }
  return {
    source: 'Domi 运行时',
    description: '该内置 MCP 由 Domi 运行时托管。',
  }
}
