...
import { Sparkles, RefreshCw, ShieldCheck, ArrowDownToLine, Globe2 } from 'lucide-react'
import { Switch } from '@/components/ui/switch'
import { Tooltip, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip'
import { cn } from '@/lib/utils'
import type { SkillMeta, SkillUsageStats } from '@domi/shared'
...
export function SkillCard({ skill, isBuiltin, updating, usage, onOpen, onToggle, onUpdate }: SkillCardProps): React.ReactElement {
  return (
    <div
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="mt-auto flex items-center gap-2">
        {isBuiltin ? (
          <span className="flex items-center gap-1 rounded-md bg-blue-500/10 px-1.5 py-0.5 text-[11px] font-medium text-blue-600 dark:text-blue-400">
            <ShieldCheck size={12} /> Domi 内置
          </span>
        ) : skill.origin ? (
          <span className="flex items-center gap-1 rounded-md bg-violet-500/10 px-1.5 py-0.5 text-[11px] font-medium text-violet-600 dark:text-violet-400">
            <Globe2 size={12} /> {skill.origin === 'pi-global' ? 'Pi 全局' : skill.origin === 'agents-global' ? 'Agent Skills 全局' : 'Claude 全局'}
          </span>
        ) : skill.importSource ? (
      ...
      {usage && usage.triggerCount > 0 && (
        <div className="flex items-center gap-1 text-[11px] text-muted-foreground/60">
          <span>⚡ 已触发 {usage.triggerCount} 次</span>
          {usage.lastTriggeredAt != null && (
            <span>· 最近 {formatRelativeTime(usage.lastTriggeredAt)}</span>
          )}
        </div>
      )}
    </div>
