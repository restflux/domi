...
import { Blocks, ChevronDown, ChevronRight, Search, Plus, Store, FolderOpen, Check, Sparkles, Loader2, ShieldCheck } from 'lucide-react'
...
import type { BuiltinMcpServerSummary, GlobalMcpServerSummary, McpServerEntry, SkillMeta, SkillUsageStats } from '@domi/shared'
...
import { ExtensionTrustDialog } from './ExtensionTrustDialog.tsx'
import { GlobalCapabilitiesBanner } from './GlobalCapabilitiesBanner'
...
function buildSkillClassificationPrompt(input: {
  ...
  return `请帮我整理当前项目在 Domi 工作区中保存的 Skills 的分组。
...
export function AgentSkillsView(): React.ReactElement {
  ...
  const [showExtensionTrust, setShowExtensionTrust] = React.useState(false)
  ...
  const globalSkills = filteredSkills.filter((skill) => skill.origin)
  const customSkills = filteredSkills.filter((skill) => !skill.origin && !data.defaultSkillSlugs.has(skill.slug))
  const builtinSkills = filteredSkills.filter((skill) => !skill.origin && data.defaultSkillSlugs.has(skill.slug))
  const localSkills = data.skills.filter((skill) => !skill.origin)
  const updateCount = localSkills.filter((skill) => skill.hasUpdate).length

  // source:slug → 使用统计查找表（工作区与全局来源分开计数）
  const skillUsageLookup = React.useMemo(() => {
    const lookup = new Map<string, SkillUsageStats>()
    for (const usage of data.skillUsage) {
      lookup.set(`${usage.source}:${usage.skillSlug}`, usage)
    }
    return lookup
  }, [data.skillUsage])
  ...
  const globalMcpEntries = React.useMemo(() => {
    if (!data.globalCapabilities?.mcpEnabled) return []
    const workspaceNames = new Set(Object.keys(data.mcpConfig.servers ?? {}))
    return data.globalCapabilities.detectedMcpServers
      .filter((server) => !workspaceNames.has(server.name))
      .filter((server) => !q || server.name.toLowerCase().includes(q))
  }, [data.globalCapabilities, data.mcpConfig, q])
  ...
  const mcpCount = React.useMemo(
    () => Object.keys(data.mcpConfig.servers ?? {}).filter((n) => n !== 'memos-cloud').length +
      (data.globalCapabilities?.mcpEnabled ? data.globalCapabilities.detectedMcpServers.filter((server) => !Object.hasOwn(data.mcpConfig.servers, server.name)).length : 0) +
      data.builtinMcpServers.length,
    [data.mcpConfig, data.globalCapabilities, data.builtinMcpServers],
  )
  ...
  const selectedIsBuiltin = selectedSkill ? !selectedSkill.origin && data.defaultSkillSlugs.has(selectedSkill.slug) : false

  const openSkillFolder = (slug: string): void => {
    if (data.skillsDir) window.electronAPI.systemOpenFile(`${data.skillsDir}/${slug}`)
  }
  ...
  const handleClassifySkills = React.useCallback(async (): Promise<void> => {
    ...
    if (!data.skillsDir) {
      toast.error('无法定位当前项目的 Domi 工作区 Skills 目录')
      return
    }
  ...
  }, [classifyingSkills, createAgent, data.skillsDir, data.workspaceName, localSkills, setPendingPrompt])
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
        ...
        <div className="max-w-sm text-[13px] text-foreground/50">
          请先在 Work 模式下选择或创建一个项目，再来管理它的 Skills 与 MCP。
        </div>
      </div>
    )
  }
    ...
    <div className="flex h-full flex-col overflow-hidden">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  disabled={classifyingSkills || localSkills.length === 0}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin">
        <div className="mx-auto w-full max-w-6xl px-8 pb-10">
          {!data.loading && data.globalCapabilities && (
            <div className="mb-6">
              <GlobalCapabilitiesBanner
                capabilities={data.globalCapabilities}
                onSkillsEnabledChange={(enabled) => void data.setGlobalSkillsEnabled(enabled)}
                onMcpEnabledChange={(enabled) => void data.setPiGlobalMcpEnabled(enabled)}
              />
            </div>
          )}
          {data.loading ? (
            ...
            <div className="space-y-6">
              <button
                type="button"
                onClick={() => setShowExtensionTrust(true)}
                className="flex w-full items-center gap-4 rounded-xl border border-amber-500/25 bg-amber-500/[0.05] p-4 text-left transition-colors hover:bg-amber-500/[0.09]"
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-foreground">Extension Trust</div>
                  <div className="mt-1 text-xs text-muted-foreground">
                    预览摘要与风险，显式批准、检查或撤销当前项目的 Pi Extension。
                  </div>
                </div>
              ...
              <SkillsTab
                ...
                onOpen={(skill) => {
                  if (skill.origin && skill.sourcePath) {
                    window.electronAPI.systemOpenFile(skill.sourcePath)
                    return
                  }
            ...
            <McpTab
              ...
              globalEntries={globalMcpEntries}
      ...
      {currentWorkspaceId ? (
        <ExtensionTrustDialog
          open={showExtensionTrust}
          workspaceId={currentWorkspaceId}
          onOpenChange={setShowExtensionTrust}
        />
      ) : null}
    </div>
  )
...
interface SkillsTabProps {
  ...
  usageLookup: ReadonlyMap<string, SkillUsageStats>
...
function SkillsTab({
  ...
  if (total === 0) {
    return <EmptyState icon={<Blocks className="size-8 text-foreground/30" />} title="暂无 Skill" hint="可以在 Work 模式下让 Domi 帮你联网查找并安装 Skill，或从其他项目导入。" />
  }
  if (customSkills.length === 0 && builtinSkills.length === 0 && globalSkills.length === 0) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {customSkills.length > 0 && (
        <SkillSection title="我的 Skills" skills={customSkills} isBuiltin={isBuiltin} updatingSkill={updatingSkill} usageLookup={usageLookup} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} />
      )}
      {builtinSkills.length > 0 && (
        <SkillSection title="Domi 内置" skills={builtinSkills} isBuiltin={isBuiltin} updatingSkill={updatingSkill} usageLookup={usageLookup} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} />
      )}
      {globalSkills.length > 0 && (
        <SkillSection title="外部全局（只读）" skills={globalSkills} isBuiltin={isBuiltin} updatingSkill={updatingSkill} usageLookup={usageLookup} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} readOnly />
      )}
    </div>
  )
}
...
function SkillSection({ title, skills, isBuiltin, updatingSkill, usageLookup, onOpen, onToggle, onUpdate, readOnly = false }: SkillSectionProps): React.ReactElement {
              ...
              {!collapsed && (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {group.skills.map((skill) => (
                    <SkillCard
                      key={`${skill.origin ?? 'workspace'}:${skill.slug}`}
                      skill={skill}
                      isBuiltin={isBuiltin(skill.slug)}
                      updating={updatingSkill === skill.slug}
                      usage={usageLookup.get(`${skill.origin ?? 'workspace'}:${skill.slug}`)}
                      onOpen={() => onOpen(skill)}
                      {...(!readOnly && {
                        onToggle: (enabled: boolean) => onToggle(skill.slug, enabled),
...
interface McpTabProps {
  ...
  globalEntries: GlobalMcpServerSummary[]
...
function McpTab({ userEntries, globalEntries, builtinServers, total, onOpen, onOpenGlobal, onOpenBuiltin, onToggle, onToggleBuiltin, onRequestDelete, onAdd }: McpTabProps): React.ReactElement {
  if (total === 0) {
    return (
      <EmptyState
        ...
        title="还没有 MCP 服务器"
        hint="点击右上角「添加服务器」开始，或在 Work 模式下让 Domi 帮你查找并配置。"
        action={
          <button
            type="button"
            onClick={onAdd}
  ...
  if (userEntries.length === 0 && globalEntries.length === 0 && builtinServers.length === 0) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {globalEntries.length > 0 && (
        <McpSection title="Pi 全局 MCP（只读）" count={globalEntries.length}>
          {globalEntries.map((server) => (
            <McpCard
              key={`pi-global:${server.name}`}
              name={server.name}
              entry={{
                type: server.type,
              ...
              statusLabel={server.enabled ? 'Pi 全局' : '已关闭'}
              statusTone={server.enabled ? 'success' : 'muted'}
              readOnly
              onOpen={() => onOpenGlobal(server.sourcePath)}
            />
          ))}
        </McpSection>
      )}
