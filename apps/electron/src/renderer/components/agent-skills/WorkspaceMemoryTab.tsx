...
import type { SkillFileNode, WorkspaceMemorySummary } from '@domi/shared'
...
function buildWorkspaceMemoryInitPrompt(historyRange: MemoryHistoryRange): string {
  ...
  return `请为当前项目初始化并沉淀长期记忆。这里的“项目”指系统提示中的“项目根目录”及其关联的 Agent 工作会话；不要把 Domi 工作区笼统当作项目。
...
路径与职责边界：
- 系统提示中的“Domi 工作区目录”是 Domi 管理配置与隔离资料的位置，存放 MCP、Skills、Domi 管理的 AGENTS.md 与 Auto Memory；它不是用户项目根目录。必须按系统提示给出的绝对路径操作，不得猜测或替换路径。
- “项目根目录”是用户项目资料的边界，并不一定等于实际 cwd：新会话通常从项目根目录运行，历史会话可能仍从会话工作台运行。允许从项目级 Context 及明确关联的长期项目资料读取证据；只使用系统提示已激活的受信项目指令，不要自行发现、创建、迁移或修改项目根内的 AGENTS.md / CLAUDE.md、.claude、MCP 或 Skills 配置，除非用户明确要求。
...
2. 只更新系统提示明确给出的“Domi 工作区 AGENTS.md”绝对路径。这里是 Domi 管理的项目指令文件；内容仅限稳定、跨会话有效的项目规则、入口和工作方法，不得混入临时调试、聊天记录或长篇资料。
3. 只更新系统提示明确给出的“Domi 工作区 Auto Memory 目录”中的 \`MEMORY.md\`、必要的主题文件和 \`user-profile.md\`，不要在其他目录创建记忆文件。\`MEMORY.md\` 保持简短的主题索引与路由，主题细节拆分到主题文件。
...
export function WorkspaceMemoryTab({ workspaceSlug, search }: WorkspaceMemoryTabProps): React.ReactElement {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {(summary.instructionMigration.status === 'conflict'
        || summary.instructionMigration.status === 'blocked'
        || summary.instructionMigration.status === 'failed') && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/8 px-4 py-3 text-sm text-foreground">
          <div className="font-medium">AGENTS.md 迁移需要处理</div>
          <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
            {summary.instructionMigration.message ?? '旧工作区指令已保留，Domi 没有覆盖或删除原文件。'}
          </div>
        </div>
      )}
      ...
      <SettingsCard divided={false}>
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
              新建一个 Agent 会话，读取当前项目{historyRangeLabel}的工作会话，沉淀并更新 Domi 工作区中的 AGENTS.md 与 auto memory 文件。
            </div>
          </div>
