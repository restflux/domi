...
/** 明确的本地路径中可能包含任意 Mention 字符，都不应触发快捷菜单。 */
function isMentionTriggerInsidePath(paragraphText: string, triggerOffset: number): boolean {
  const token = getMentionToken(paragraphText, triggerOffset)
  if (!token) return false
...
export function shouldAllowMentionTrigger({
  ...
  if (
    isCodeContext
    || isMentionTriggerInsideUrl(paragraphText, triggerOffset)
    || isMentionTriggerInsidePath(paragraphText, triggerOffset)
  ) return false
