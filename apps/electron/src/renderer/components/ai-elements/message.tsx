 ...
 * AI Elements - 消息组件原语
 *
 * 简化迁移自 domi-frontend 的 ai-elements/message.tsx，
...
import { CodeBlock, MermaidBlock } from '@domi/ui'
import { detectLanguage } from '@domi/core'
import { FilePathChip, isAbsoluteFilePath, isImageFilePath, isRelativeFilePath } from './file-path-chip'
import type { HTMLAttributes, ComponentProps, ReactNode } from 'react'
import type { FileAttachment } from '@domi/shared'
...
function MentionChip({ type, value }: { type: MentionType; value: string }): React.ReactElement {
  ...
  // 图片 @file 引用沿用统一文件预览入口；FilePathChip 点击时会按当前 session 重新授权。
...
const ManagedBrowserLinkContext = React.createContext<((url: string) => void | Promise<void>) | null>(null)

/** 为 Work 消息中的 HTTP/HTTPS 链接提供当前会话的内置浏览器入口。 */
export function ManagedBrowserLinkProvider({
  onOpen,
  children,
}: {
  ...
  return <ManagedBrowserLinkContext.Provider value={onOpen}>{children}</ManagedBrowserLinkContext.Provider>
}
...
/**
 * react-markdown 会把 Windows 路径中的反斜杠当作 Markdown 转义符。
 * 例如 `](C:\\Users\\A\\.domi\\文档.md)` 在进入链接组件前已经丢失部分分隔符，
 * 最终得到一个看似可点击、实际无法预览的损坏路径。
 *
 * 在 Markdown 解析前，仅规范化链接目标中的 Windows 绝对路径；代码块和行内代码保持原文。
 * 统一改为带尖括号的正斜杠形式，同时兼容中文和包含空格的文件名。
 */
export function normalizeWindowsMarkdownFileLinks(markdown: string): string {
  ...
  for (let partIndex = 0; partIndex < parts.length; partIndex += 2) {
    ...
    while (cursor < line.length) {
      ...
      let pathStart = cursor + 2
      while (line[pathStart] === ' ' || line[pathStart] === '\t') pathStart++
      const angleWrapped = line[pathStart] === '<'
      if (angleWrapped) pathStart++

      if (!/^[A-Za-z]:\\/.test(line.slice(pathStart))) {
        output += line[cursor]
        cursor++
        continue
      }
      ...
      let closingParen = -1
      if (angleWrapped) {
        pathEnd = line.indexOf('>', pathStart)
        if (pathEnd !== -1) {
          let afterAngle = pathEnd + 1
          while (line[afterAngle] === ' ' || line[afterAngle] === '\t') afterAngle++
          if (line[afterAngle] === ')') closingParen = afterAngle
        }
      } else {
        let nestedParens = 0
        for (let i = pathStart; i < line.length; i++) {
          if (line[i] === '(') {
            nestedParens++
          } else if (line[i] === ')') {
            if (nestedParens === 0) {
      ...
      if (pathEnd === -1 || closingParen === -1) {
        output += line[cursor]
        cursor++
        continue
      }

      const rawPath = line.slice(pathStart, pathEnd).trim()
      const normalizedPath = rawPath.replace(/\\/g, '/')
      output += `](<${normalizedPath}>)`
      cursor = closingParen + 1
    }

    parts[partIndex] = output
  }
...
export type HttpMessageLinkTarget = 'managed-browser' | 'system-browser'

export function resolveHttpMessageLinkTarget(input: {
  hasManagedBrowser: boolean
  ctrlKey: boolean
  metaKey: boolean
  shiftKey?: boolean
  altKey?: boolean
}): HttpMessageLinkTarget {
  return input.hasManagedBrowser && !input.ctrlKey && !input.metaKey && !input.shiftKey && !input.altKey
    ? 'managed-browser'
    : 'system-browser'
}
...
const MarkdownLink = React.memo(function MarkdownLink({
  ...
  const openManagedBrowser = React.useContext(ManagedBrowserLinkContext)
  ...
  return (
    <a
      ...
      onClick={(event) => {
        ...
        const target = resolveHttpMessageLinkTarget({
          hasManagedBrowser: openManagedBrowser !== null,
          ctrlKey: event.ctrlKey,
          metaKey: event.metaKey,
          shiftKey: event.shiftKey,
          altKey: event.altKey,
        })
        if (target === 'managed-browser' && openManagedBrowser) {
          void openManagedBrowser(href)
          return
        }
        window.electronAPI.openExternal(href)
      }}
      onAuxClick={(event) => {
        if (event.button !== 1 || !href || !href.startsWith('http://') && !href.startsWith('https://')) return
        event.preventDefault()
        window.electronAPI.openExternal(href)
      }}
      title={openManagedBrowser && href?.startsWith('http')
        ? `${href}\n点击在内置浏览器打开；Ctrl/Cmd 或中键点击在系统浏览器打开`
        : href}
    >
      {linkChildren}
    </a>
...
export const MessageResponse = React.memo(
  function MessageResponse({ children, className, basePath, basePaths, remarkPlugins }: MessageResponseProps): React.ReactElement {
    ...
    const renderedMarkdown = normalizeWindowsMarkdownFileLinks(
      (remarkPlugins?.includes(remarkMentions)
        ? normalizeNamedReferenceDelimiters(children)
        : children
      ).replace(/<!--DOMI_AUTOMATION:[\s\S]*?-->/g, '').trim()
    )
...
/** 文件附件展示（标签样式，teal 色调；点击在系统文件管理器中显示） */
function MessageAttachmentFile({ attachment }: MessageAttachmentFileProps): React.ReactElement {
  ...
  const handleShowInFolder = React.useCallback((): void => {
    void window.electronAPI.showItemInFolder(attachment.localPath).then((shown) => {
      if (!shown) {
        toast.warning('未能定位文件', { description: attachment.filename })
      }
    })
  }, [attachment.localPath, attachment.filename])
  ...
  return (
    <button
      ...
      title={`在文件夹中显示：${attachment.filename}`}
      className="flex items-center gap-2 rounded-lg bg-[#37a5aa]/10 border border-[#37a5aa]/20 px-3 py-1.5 text-[13px] text-[#37a5aa] shrink-0 cursor-pointer transition-colors hover:bg-[#37a5aa]/20"
    >
