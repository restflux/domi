...
import { FileText, StickyNote, X, Clock, GitBranch, MessageSquareText } from 'lucide-react'
...
export function TabBarItem({
  ...
  const handleMouseDown = (e: React.MouseEvent): void => {
    // 固定入口与当前活动 Work 会话不可中键关闭。
    if (type === 'scratch' || !closable) return
    if (e.button === 1) {
      e.preventDefault()
      onMiddleClick()
    }
  ...
  if (isScratch) {
    return (
      <div
        className="chrome-tab-slot chrome-tab-slot-scratch relative flex-shrink-0 titlebar-no-drag"
      ...
      >
        <button
          ...
          type="button"
          className={cn(
            'chrome-tab group relative flex items-center justify-center gap-1.5 min-w-[82px] px-3 h-[34px]',
  ...
  return (
    <div
      className="chrome-tab-slot relative min-w-[132px] max-w-[240px] flex-[1_0_132px] titlebar-no-drag"
      onMouseEnter={onHoverEnter}
      onMouseLeave={onHoverLeave}
    >
      <button
        ...
        type="button"
        className={cn(
          'chrome-tab group relative flex items-center gap-1.5 px-3 h-[34px] w-full',
          isClassic ? 'rounded-t-lg' : 'rounded-none',
          'text-xs transition-colors select-none cursor-pointer',
          'border-t border-l border-r border-transparent',
          isActive
      ...
      >
        ...
        {isNarrow ? (
          ...
          <span className="flex-1 min-w-0 truncate text-left flex items-center gap-1">
            {semanticIcon && (
              <span
                data-tab-semantic-icon={semanticIcon}
                className="tab-semantic-icon inline-flex size-3.5 shrink-0 items-center justify-center"
                aria-hidden="true"
              >
                {semanticIcon === 'running' ? (
                  <Spinner size="sm" className="text-primary/70" />
                ) : semanticIcon === 'automation' ? (
                  <Clock className="size-3" />
                ) : semanticIcon === 'delegation' ? (
            ...
            {workspaceName && (
              <span
                className="workspace-tab-meta hidden min-w-0 max-w-[86px] shrink-0 items-center gap-1 text-[10px] font-normal leading-4"
                title={workspaceName}
              >
                <span aria-hidden="true" className="workspace-tab-meta-separator shrink-0">·</span>
        ...
        {workspaceName && !isNarrow && (
          <span className="workspace-tab-legacy-meta shrink-0 px-1.5 py-0 rounded-full bg-primary/10 text-[10px] leading-4 workspace-badge font-medium truncate max-w-[86px]">
            {workspaceName}
          </span>
        )}
