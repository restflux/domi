...
export function BrowserPanelV2({ ownerSessionId, browserSessionId }: BrowserPanelProps): React.ReactElement {
  ...
  return (
    <section className="flex h-full min-w-0 flex-col bg-background" aria-label="内置浏览器">
      <BrowserToolbar
        page={page}
        selecting={selectingElement}
        responsive={responsive}
        onToggleResponsive={() => setResponsive((current) => !current)}
        ...
        onSelectElement={handleToggleElementSelection}
      ...
      {error && (
        <div className="flex shrink-0 items-center gap-2 bg-destructive/10 px-3 py-2 text-xs text-destructive" role="alert">
