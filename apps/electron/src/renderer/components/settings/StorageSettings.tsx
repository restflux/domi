...
export function StorageSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsSection
        title="Worktrees"
        description="查看 Agent 创建的临时修改环境、保留期限和需要处理的清理状态；不设置数量配额。"
      >
        <SettingsCard divided={false}>
          <div className="flex items-center justify-between gap-4 px-4 py-3">
            <div>
              <div className="text-sm font-medium">管理 Worktrees</div>
              <p className="mt-1 text-xs text-muted-foreground">正在工作的环境不会按数量、时间或磁盘占用自动删除。</p>
            </div>
            <Button size="sm" variant="outline" onClick={() => setWorktreeManager({ open: true, scope: 'all' })}>打开管理</Button>
          </div>
        </SettingsCard>
      </SettingsSection>
      ...
      >
        <AlertDialogContent className="max-w-2xl">
          ...
          <div className="max-h-72 overflow-auto scrollbar-thin rounded-md bg-muted/50 p-2">
