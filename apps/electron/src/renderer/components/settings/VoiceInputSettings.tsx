...
const OUTPUT_OPTIONS = [
  { value: 'auto', label: '自动：Domi 激活时写入对话框，否则写入当前光标' },
  { value: 'clipboard', label: '仅复制到剪贴板' },
  { value: 'domi-input', label: '仅写入 Domi 输入框' },
]
...
export function VoiceInputSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      <SettingsSection
        title="豆包流式语音输入"
        description="通过全局快捷键唤起浮窗，实时识别语音，停止后写入 Domi 输入框或当前光标位置。"
        action={
          <Button
            variant="outline"
            size="sm"
      ...
      >
        ...
        <SettingsCard>
          ...
          <SettingsSelect
            label="输出方式"
            description="默认写入当前光标位置；如果唤起时 Domi 是当前激活窗口，会写入当前 Chat 或 Work 输入框。自动粘贴失败时会保留到剪贴板。"
