 ...
 *
 * 支持两种模式：
 * - Personal 备份（.domi-backup）：全量导出，含解密 API Key
 * - Share 分发（.domi-share）：自由选择组件，凭据自动剥离
 *
 * Share 模式支持多工作区导出：
...
import type { LocalProjectRootStatus, MigrationFileMode } from '@domi/shared'

type MigrationMode = MigrationFileMode
...
export function MigrationSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection
        title="导入备份"
        description="支持 Domi 迁移文件，也可显式导入兼容的旧版迁移文件"
      >
        <button
