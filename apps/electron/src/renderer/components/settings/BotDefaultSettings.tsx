...
export function BotDefaultSettings(): React.ReactElement {
  ...
  React.useEffect(() => {
    window.electronAPI.getSettings().then((settings) => {
      setDefaultWorkspaceId(settings.agentRemoteDefaultWorkspaceId ?? settings.agentWorkspaceId ?? '')
      setLoading(false)
  ...
  const handleSave = React.useCallback(async () => {
    try {
      await window.electronAPI.updateSettings({
        agentRemoteDefaultWorkspaceId: defaultWorkspaceId || undefined,
      })
      ...
      >
        <SettingsCard>
          {workspaceOptions.length > 0 ? (
            <SettingsSelect
              label="默认项目"
              description="机器人创建新会话时自动使用；不随桌面端最近会话变化"
        ...
        <SettingsCard divided={false}>
          <div className="px-4 py-3 space-y-2 text-sm text-muted-foreground">
            ...
            <p className="pt-2 text-xs">
              首次对话前可先发送 /workspace，再用序号选择项目；直接发送文本会自动创建新会话或发送到当前绑定的会话。
            </p>
          </div>
        </SettingsCard>
      </SettingsSection>
