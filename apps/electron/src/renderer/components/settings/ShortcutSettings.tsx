...
import {
  agentFollowUpModeAtom,
  agentSteeringModeAtom,
  shortcutOverridesAtom,
  sendWithCmdEnterAtom,
} from '@/atoms/shortcut-atoms'
import type { AgentQueueDeliveryMode } from '@/lib/agent-queue-delivery-mode'
import {
  DEFAULT_SHORTCUTS,
...
interface QueueDeliveryModeRowProps {
  label: string
  mode: AgentQueueDeliveryMode
  onChange: (mode: AgentQueueDeliveryMode) => void
}

function QueueDeliveryModeRow({ label, mode, onChange }: QueueDeliveryModeRowProps): React.ReactElement {
  return (
    <div className="flex items-center justify-between gap-4 rounded-md px-1 py-1.5">
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium text-foreground">{label}</div>
        <div className="mt-0.5 text-xs text-muted-foreground">
          每次 Agent turn 消费此队列中的一条或全部消息
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-1 rounded-lg bg-muted/60 p-0.5">
        <button
          ...
          onClick={() => onChange('one-at-a-time')}
        >
          逐条处理（默认）
        </button>
        <button
...
export function ShortcutSettings(): React.ReactElement {
  ...
  const [agentSteeringMode, setAgentSteeringMode] = useAtom(agentSteeringModeAtom)
  const [agentFollowUpMode, setAgentFollowUpMode] = useAtom(agentFollowUpModeAtom)
  ...
  const handleAgentSteeringModeChange = React.useCallback((next: AgentQueueDeliveryMode) => {
    if (next === agentSteeringMode) return
    const previous = agentSteeringMode
    setAgentSteeringMode(next)
    window.electronAPI
      .updateSettings({ agentSteeringMode: next })
      .then(() => {
        toast.success('Steering 队列消费设置已保存', { id: 'shortcut-save-success' })
      })
      .catch((error) => {
        setAgentSteeringMode(previous)
        console.error(error)
        toast.error('Steering 队列消费设置保存失败', { id: 'shortcut-save-error' })
      })
  }, [agentSteeringMode, setAgentSteeringMode])

  const handleAgentFollowUpModeChange = React.useCallback((next: AgentQueueDeliveryMode) => {
    if (next === agentFollowUpMode) return
    const previous = agentFollowUpMode
    setAgentFollowUpMode(next)
    window.electronAPI
      .updateSettings({ agentFollowUpMode: next })
      .then(() => {
        toast.success('Follow-up 队列消费设置已保存', { id: 'shortcut-save-success' })
      })
      .catch((error) => {
        setAgentFollowUpMode(previous)
        console.error(error)
        toast.error('Follow-up 队列消费设置保存失败', { id: 'shortcut-save-error' })
      })
  }, [agentFollowUpMode, setAgentFollowUpMode])
      ...
      <div>
        ...
        <div className="mt-3 rounded-lg border border-border/60 bg-muted/20 p-3">
          <div className="mb-2 text-xs text-muted-foreground">
            控制两类队列一次消费几条；新一轮 Agent 运行生效。
          </div>
          <div className="space-y-2">
            <QueueDeliveryModeRow
              label="Steering 队列消费方式"
              mode={agentSteeringMode}
              onChange={handleAgentSteeringModeChange}
            />
            <QueueDeliveryModeRow
              label="Follow-up 队列消费方式"
              mode={agentFollowUpMode}
              onChange={handleAgentFollowUpModeChange}
            />
          </div>
        </div>
      </div>
