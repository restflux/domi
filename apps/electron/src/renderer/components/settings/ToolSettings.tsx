...
import type { ChatToolInfo } from '@domi/shared'
...
import { updateChatToolWithLinkedCapabilities } from '@/lib/linked-image-tool-ui-sync'
  ...
  const handleToggle = async (checked: boolean): Promise<void> => {
    try {
      await updateChatToolWithLinkedCapabilities({
        toolId: 'nano-banana',
        enabled: checked,
        updateToolState: window.electronAPI.updateChatToolState,
        updateCapabilityVersion: bumpCapabilitiesVersion,
      })
      setEnabled(checked)
      await refreshChatTools(setChatTools)
    } catch (error) {
...
/** GPT Image 生图工具设置区域（OpenAI Images API，模型 gpt-image-2） */
function GptImageSettings(): React.ReactElement {
  ...
  return (
    <SettingsSection
      title="GPT Image"
      description="启用后 AI 可以生成和编辑图片（基于 OpenAI GPT Image，模型 gpt-image-2）"
      action={
        <Switch
          checked={enabled}
          onCheckedChange={handleToggle}
    ...
    >
      <SettingsCard divided={false}>
        <div className="space-y-4 p-4">
          {/* 引导说明 */}
          <div className="rounded-lg bg-muted/50 p-3 space-y-2 text-sm text-muted-foreground">
            <p>GPT Image 基于 <span className="font-medium text-foreground">OpenAI Images API</span>（模型 gpt-image-2）提供 AI 图片生成与编辑能力，与 Codex 生图实现对齐。</p>
            <p className="text-xs">配置步骤：</p>
            <ol className="text-xs list-decimal list-inside space-y-1">
              <li>
                访问{' '}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onBlur={handleBlurSave}
            />
            <p className="text-xs text-muted-foreground">留空则使用 OpenAI 官方地址，可配置为其他 OpenAI 兼容端点</p>
          </div>
...
function CustomToolCredentialEditor({
  tool,
  onSaved,
}: {
  tool: ChatToolInfo
  ...
  const keys = tool.credentialKeys ?? []
  const [expanded, setExpanded] = React.useState(false)
  const [saving, setSaving] = React.useState(false)
  const [credentialUpdates, setCredentialUpdates] = React.useState<Record<string, string>>({})
  const configuredKeys = new Set(tool.configuredCredentialKeys ?? [])
  ...
  const persistCredentials = async (
    ...
    setSaving(true)
    try {
      await window.electronAPI.updateChatToolCredentials(tool.meta.id, updates)
      setCredentialUpdates({})
      await onSaved()
      toast.success(successMessage)
    } catch (error) {
      console.error('[自定义工具] 保存凭据失败:', error)
      toast.error('保存工具凭据失败')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleSave = async (): Promise<void> => {
    const updates = Object.fromEntries(
      Object.entries(credentialUpdates).filter(([, value]) => value.trim().length > 0),
    )
    if (Object.keys(updates).length === 0) {
      toast.error('请至少输入一个要更新的凭据')
      return
    }
    await persistCredentials(updates, `已保存凭据: ${tool.meta.name}`)
  }

  const handleClear = async (): Promise<void> => {
    await persistCredentials(
      Object.fromEntries(keys.map((key) => [key, ''])),
      `已清除凭据: ${tool.meta.name}`,
    )
  }
  ...
  return (
    <div className="mt-3 border-t border-border/60 pt-3">
      ...
      >
        <KeyRound size={13} />
        {expanded ? '收起凭据' : tool.available ? '更新凭据' : '配置凭据'}
      </Button>
      {!tool.available && !expanded && (
        <span className="ml-2 text-xs text-amber-600 dark:text-amber-400">凭据未配置完整</span>
      )}
      {expanded && (
        <div className="mt-3 space-y-3 rounded-lg bg-muted/40 p-3">
          {keys.map((key) => (
            <div key={key} className="space-y-1">
              <label className="flex items-center justify-between text-xs font-medium font-mono">
                <span>{key}</span>
                <span className="font-sans font-normal text-muted-foreground">
                  {configuredKeys.has(key) ? '已配置' : '未配置'}
                </span>
              </label>
              <Input
                type="password"
                autoComplete="new-password"
                value={credentialUpdates[key] ?? ''}
                onChange={(event) => setCredentialUpdates((current) => ({
                  ...current,
                  [key]: event.target.value,
                }))}
                placeholder={configuredKeys.has(key) ? '留空则保持不变' : `输入 ${key}`}
              />
            </div>
          ))}
          <div className="flex justify-end gap-2">
            {configuredKeys.size > 0 && (
              <Button size="sm" variant="outline" disabled={saving} onClick={handleClear}>
                清除全部
              </Button>
            )}
            <Button size="sm" disabled={saving} onClick={handleSave}>
              {saving ? <><Loader2 size={13} className="mr-1.5 animate-spin" />保存中...</> : '保存凭据'}
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">
            凭据单独保存，不会写入工具 URL、headers 或 body 模板，也不会随自定义工具迁移导出。
          </p>
        </div>
      )}
    </div>
...
function CustomToolsSection(): React.ReactElement | null {
  ...
  return (
    <SettingsSection
      title="自定义工具"
      description="通过 Work 模式创建的 HTTP API 工具"
    >
      <SettingsCard divided>
        {customTools.map((tool) => (
          <div key={tool.meta.id} className="p-4">
            <div className="flex items-center justify-between">
            ...
            <CustomToolCredentialEditor
              tool={tool}
              onSaved={() => refreshChatTools(setChatTools)}
            />
          </div>
        ))}
      </SettingsCard>
