...
function NowledgeMemSection(): React.ReactElement {
  ...
  const handleCopy = async (): Promise<void> => {
    ...
    try {
      ...
      toast.success('已复制配置提示词，请粘贴到 Work 模式输入框执行')
    } catch (error) {
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="space-y-2">
            ...
            <ul className="text-xs text-muted-foreground list-disc list-inside space-y-1">
              ...
              <li>Domi 已切换到 <span className="font-medium text-foreground">Work 模式</span>（此提示词只能在 Agent 中执行）</li>
            </ul>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground pt-1">
              ⚠️ 提示词执行完成后需要 <span className="font-medium text-foreground">完全退出并重启 Domi</span>，MCP 与 Hooks 才会生效
            </p>
          </div>
