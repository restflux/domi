...
import { ChevronLeft, ChevronRight, Copy, Download, Pencil, X } from 'lucide-react'
...
export function ImageLightbox({
  ...
  const handleCopy = async (): Promise<void> => {
    ...
    if (result.success) {
      ...
      toast.error(result.message ?? '复制图片失败')
    }
  }
            ...
            <div style={{ gridArea: 'layer', visibility: mode === 'editing' ? 'hidden' : 'visible' }}>
              <div className="flex flex-col items-center">
                <ContextMenu>
                  ...
                  <ContextMenuContent className="z-[220] min-w-[9rem]">
                    <ContextMenuItem onSelect={() => void handleCopy()} className="gap-2">
                ...
                <div className={cn(
                  ...
                  <button type="button" onClick={() => void handleCopy()} className={cn('rounded-full p-1.5 text-white/80 transition-colors duration-150', 'hover:bg-white/15 hover:text-white', 'focus:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 focus-visible:ring-offset-2 focus-visible:ring-offset-black')} title="复制图片">
