...
// Domi macOS Agent Island native host.
...
struct CompactIslandView: View {
  ...
  private var compactLabel: String {
    if let session = primarySession {
      return "Domi · \(phaseText(session.phase))"
    }
    if snapshot.state.idleDashboard {
      return snapshot.state.recentSessions.isEmpty ? "Domi · 额度概览" : "Domi · 最近会话"
    }
...
struct ExpandedIslandView: View {
  ...
  private var headerEyebrow: String {
    switch primaryPhase {
    case "needs-interaction": return "DOMI · HANDOFF"
    case .some: return "DOMI · AGENT"
    case .none: return "DOMI · REMINDER"
    }
  }
