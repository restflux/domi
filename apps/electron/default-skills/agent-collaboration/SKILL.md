...
description: Domi 协作子 Agent Skill。仅当用户明确要求多 Agent、至少两个真正独立且明显耗时的方向可并行，或核心安全/权限/并发/数据一致性方案需要一次限时独立审查时触发。用于调用 Domi collaboration 创建、等待和停止真实子会话。普通 coding、局部修复、短调研、一次性代码审查、修复后的常规复核，以及只为获得“最终 Yes”的重复审查必须由父会话直接完成。
group: Domi
version: "1.1.5"
---

# Domi Agent Collaboration

你负责判断何时把复杂任务交给 Workflow / Skill 工作流，或拆给真实可见的 Domi 协作子 Agent 会话。

Domi 已提供内置 `collaboration` MCP 工具。你必须通过这些工具创建、等待、查看和停止协作子会话，不要用 Bash、脚本或直接修改 `~/.domi/agent-sessions.json` 的方式创建会话。
...
- `collaboration.list_delegations`：恢复丢失的委派 ID 或用户明确要求查看状态时使用；默认只返回运行中委派。
- `collaboration.get_delegation_results`：按已知委派 ID 读取一个或多个子会话结果摘要。
...
- 普通 coding、局部修复和一次性代码审查不创建协作子会话；父会话直接完成实现、测试和自检。
- 创建工具返回后必须保存 `delegationId`。后续直接按 ID 调用 `wait_for_delegations` 或 `get_delegation_results`，不要例行调用 `list_delegations`。
- 只检查当前运行状态时，`list_delegations` 必须传 `includeCompleted: false`；恢复历史时显式传 `includeCompleted: true, limit: 20`。
- 对抗式审查默认只创建一个 reviewer、只进行一轮，并传 `maxRuntimeSeconds: 600`。
- reviewer 只报告 Blocking / High 级正确性问题；Medium / Low 由父会话修复并通过测试验证，不续审。
- 只有首轮发现 Blocking / High，且修复改变核心语义时，才允许一次 focused re-review，并传 `maxRuntimeSeconds: 300`。
- 审查等待最多 600 秒；等待超时后调用 `stop_delegation`，不要反复延长等待。
- 不为获得“最终 Yes”、风格确认或低风险边界说明调用 `continue_delegation`。
...
**流程**：
1. 父 Agent 完成方案，并确认它涉及核心安全、权限、并发、数据一致性或不可逆正确性风险
2. 创建一个 review 子 Agent，传 `maxRuntimeSeconds: 600`；只说明审查目标、明确文件/符号范围和产出，不扩展为全仓扫描
3. 子 Agent 只检查 Blocking / High：挑战关键假设、寻找会造成错误结果或安全问题的边界条件
4. 子 Agent 只返回审查报告，**不修改文件**
5. 父 Agent 评估并修复；Medium / Low 用测试收口，不续审
6. 仅当 Blocking / High 修复改变核心语义时，允许一次 `maxRuntimeSeconds: 300` 的 focused re-review

**触发条件**：
- 核心算法、权限门禁、安全机制、并发时序或数据一致性存在较高正确性风险
- 错误可能导致数据破坏、越权、重复副作用或不可恢复状态
- 用户明确要求独立对抗式审查

**约束**：
- 子 Agent 只审查不修改——所有文件变更由父 Agent 执行
- 默认一个 reviewer、一轮；不并行创建多个内容高度重叠的 reviewer
- 不审查代码风格、命名和可由 typecheck / lint / focused tests 确定的问题
- 不为获取“最终 Yes”复审；首轮没有 Blocking / High 时立即结束审查流程
...
5. 保存创建工具返回的 `delegationId`，按 ID 调用 `collaboration.wait_for_delegations` 收敛结果；几十个并行任务可以先用 `mode=any` 等一部分完成。只有父会话恢复后丢失 ID，或用户明确要求查看历史时才调用 `list_delegations`。
...
- 输出：最终回复的结构、最多报告数量和严重级别。
- 预算：review 必须写明只查 Blocking / High，并在创建工具传入运行时限。

示例：

```text
父任务：实现 Domi 协作子 Agent 能力。
...
### Scenario 2：独立并行任务应使用 Domi 协作编排
...
Then Agent 应使用普通搜索或读文件工具，不创建真实 Domi 子会话。
...
### Scenario 9：普通多文件 coding 不创建 reviewer

Given 用户要求修改三个相关文件并补 focused tests。

When 改动不涉及安全、权限、并发一致性或不可逆数据风险。

Then 父 Agent 直接实现、测试和自检，不创建对抗式审查子会话。

### Scenario 10：高风险审查必须限时单轮

Given 修改涉及权限门禁和并发副作用顺序。

When 父 Agent 判断需要独立正确性审查。

Then 只创建一个 role=review 的子会话，并传 `maxRuntimeSeconds: 600`。

And reviewer 只报告 Blocking / High。

### Scenario 11：Medium / Low 不触发续审

Given reviewer 只报告命名建议、测试补充或低风险 UX 边界。

When 父 Agent 完成必要修复和 focused tests。

Then 不调用 `continue_delegation`，不追求 reviewer 再给“最终 Yes”。

### Scenario 12：Blocking 修复仅允许一次 focused re-review

Given 首轮 reviewer 发现 Blocking / High，且父 Agent 的修复改变了核心执行语义。

When 需要确认该问题是否关闭。

Then 仅追加一次聚焦原问题的审查，传 `maxRuntimeSeconds: 300`，不得重新全量扫描。

### Scenario 13：已知委派 ID 时不扫描历史

Given 创建工具已经返回 delegationId。

When 父 Agent需要等待或读取结果。

Then 直接按 ID 调用 `wait_for_delegations` 或 `get_delegation_results`。

And 不调用 `list_delegations`。

When 父会话恢复后确实丢失 ID。

Then 调用 `list_delegations`；只查运行中传 `includeCompleted: false`，恢复历史传 `includeCompleted: true, limit: 20`。
