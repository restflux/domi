...
本技能是 Domi CLI 的薄封装。本文件给维护者解释**底层格式**与**CLI 行为**，便于排查问题。
真正的解析逻辑在 `@domi/session-core`（仓库内唯一真源），不要在技能里重抄。

## 会话存储

```
~/.domi/agent-sessions.json        会话索引（{ version, sessions: AgentSessionMeta[] }）
~/.domi/agent-sessions/<id>.jsonl   单会话消息，JSONL（一行一条 JSON）
```

开发模式（`DOMI_DEV=1` 或 Domi 未打包）数据在 `~/.domi-dev/`。CLI 用 `--dev` 或 `--config-dir` 切换。

## 两种会话格式（CLI 自动识别，无需关心）

`@domi/session-core` 的 `readSessionMessages` 在读取时统一归一，下游不需要区分：
...
```
@domi/session-core   解析 / 快照去重 / outline / search / select / render —— 真源
  └─ /node 子入口      readSessionMessages（文件 IO，含 node:fs）
apps/cli (domi)       命令路由薄壳，调用 core
default-skills/session-cleaner  本技能，教 Agent 调 CLI
```

改 bug / 改格式 → 改 `@domi/session-core`。CLI 和技能都不应包含解析逻辑。
