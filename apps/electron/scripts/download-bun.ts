...
import type { PlatformArch, BunDownloadInfo } from '@domi/shared'
...
async function downloadFile(url: string, destPath: string): Promise<void> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'Domi-Build-Script/1.0',
    },
    redirect: 'follow',
  })
...
async function getBunVersion(): Promise<string> {
  ...
  const version = pkg.domi?.bun?.version
  if (!version) {
    throw new Error('package.json 中未配置 domi.bun.version')
  }
