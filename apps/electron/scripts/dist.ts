...
function main(): void {
  ...
  // 打印配置信息
  console.log(`\n${color.bgBlue}${color.bold} Domi 打包工具 ${color.reset}\n`)
  ...
  // ── 步骤 4: 编译 domi CLI 二进制 ──
  step++
  printStepStart(step, totalSteps, '编译 domi CLI (bun --compile)')
  results.push(
    runStep('编译 domi CLI', 'bun', ['run', 'build:cli'], { verbose: opts.verbose })
  )
  printStepResult(results[results.length - 1])
  if (!results[results.length - 1].success) return printSummary(results)
  ...
  // ── 步骤 6: 同步并清理 external runtime 依赖 ──
  step++
  printStepStart(step, totalSteps, '同步 Pi Runtime 依赖')
  results.push(
    runStep('同步 Pi Runtime 依赖', 'bun', ['run', 'sync:runtime-deps'], { verbose: opts.verbose })
  )
  printStepResult(results[results.length - 1])
  if (!results[results.length - 1].success) return printSummary(results)
