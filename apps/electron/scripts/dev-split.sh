...
if [ -n "$TMUX" ]; then
  ...
  # 创建新的 tmux 会话
  tmux new-session -d -s domi-dev "cd $(pwd) && bun run dev:vite"

  # 创建左右分屏并运行 Electron
  tmux split-window -h -t domi-dev "cd $(pwd) && bun run dev:electron"

  # 选择左侧窗格（Vite）
  tmux select-pane -t domi-dev:0.0

  # 附加到会话
  tmux attach-session -t domi-dev
