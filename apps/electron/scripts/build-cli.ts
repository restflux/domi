...
import { CLI_EXECUTABLE_BASENAME } from '@domi/shared/config'
...
const binName = isWindows ? `${CLI_EXECUTABLE_BASENAME}.exe` : CLI_EXECUTABLE_BASENAME
const outDir = join(electronDir, 'resources/bin')
const outFile = join(outDir, binName)
const legacyOutFile = join(outDir, isWindows ? 'proma.exe' : 'proma')
...
if (legacyOutFile !== outFile && existsSync(legacyOutFile)) {
  unlinkSync(legacyOutFile)
}

console.log(`${color.cyan}[build:cli]${color.reset} 编译 Domi CLI → ${color.dim}${outFile}${color.reset}`)
