/**
 * domi session export — 把会话(或其 turn 子集)渲染为干净 Markdown。
 *
...
import { readSessionMessages } from '@domi/session-core/node'
import { groupIntoTurns, toTranscript, selectTurns, renderTranscriptMarkdown } from '@domi/session-core'
  ...
  run: (ctx) => {
    ...
    if (!hasWindow && !forceStdout && bytes > maxBytes) {
      ...
      const result = {
        ...
        written: fallback,
        hint: '输出过大已落盘。用 --turns A-B / --head N / --tail N 取片段，或 --stdout 强制全量，或 --out 指定路径。先跑 `domi session outline <id>` 看结构。',
      }
      if (ctx.json) {
        emitJson(result)
      } else {
