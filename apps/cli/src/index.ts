 ...
 *
 * 用法：
 *   domi <command> [subcommand] [args] [--flags]
 *   domi session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 Domi 配置目录（默认 ~/.domi，DOMI_DEV=1 → ~/.domi-dev）
 *   --dev             使用 ~/.domi-dev
 *
...
import { APP_NAME, CLI_EXECUTABLE_BASENAME } from '@domi/shared/config'
...
function printHelp(): void {
  info(`${CLI_EXECUTABLE_BASENAME} — ${APP_NAME} 会话渐进式读取 CLI\n`)
  info(`用法: ${CLI_EXECUTABLE_BASENAME} session <command> [args] [--flags]\n`)
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `domi session <cmd>` 与直接 `domi <cmd>`
  let rest = argv
  ...
  try {
    ...
    if (err instanceof UsageError) {
      ...
      info(`用法: ${CLI_EXECUTABLE_BASENAME} ${command.usage}`)
      return EXIT_USAGE
    }
